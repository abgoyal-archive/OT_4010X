...
PVMFStatus PVFMVideoMIO::GetFrameProperties(uint32& aFrameWidth, uint32& aFrameHeight, uint32& aDisplayWidth, uint32& aDisplayHeight, uint32& aCropWidth, uint32& aCropHeight, uint32& aPmemSrcBufVA)
{
    ...
    aCropWidth = iCropWidth;
    aCropHeight = iCropHeight;

    // [20100423] Jackal
    aPmemSrcBufVA = iPmemSrcBufVA;

    return PVMFSuccess;
}

PVMFStatus PVFMVideoMIO::GetFrameFormatType(PVMFFormatType& aVideoSubFormat)
{
    if (iVideoSubFormat == PVMF_MIME_FORMAT_UNKNOWN)
    {
        return PVMFErrNotReady;
    }

    aVideoSubFormat = iVideoSubFormat;
                                       ...
                                       const PvmiMediaXferHeader& data_header_info, OsclAny* aContext)
{
    ...
    //Jackal
    iCropWidth = iVideoDisplayWidth;
    iCropHeight = iVideoDisplayHeight;
    //Jackal
...
#ifdef MT6516_VIDEO_DECODER_OUTPUT_FORMAT
                                    fScale = 1.0;
#endif
...
#if 0
    LOGD("CopyVideoFrameData in");
    LOGD("aSrcBuffer = 0x%08x",aSrcBuffer);
    LOGD("aSrcSize = %d",aSrcSize);
    LOGD("aDestBuffer = 0x%08x",aDestBuffer);
    LOGD("aDestSize = %d",aDestSize);
    LOGD("aSrcWidth = %d",aSrcWidth);
    LOGD("aSrcHeight = %d",aSrcHeight);
    LOGD("aDestWidth = %d",aDestWidth);
    LOGD("aDestHeight = %d",aDestHeight);
#endif
...
#else
            if (NULL == oscl_pmem_vir2phy(aSrcBuffer))
            {
                //LOGE("aSrcBuffer is not pmem");
                oscl_memcpy(aDestBuffer, aSrcBuffer, aSrcSize);
                iPmemSrcBufVA = 0;
            }
            else
            {
                //LOGE("aSrcBuffer is pmem");
                iPmemSrcBufVA = (uint32)aSrcBuffer;
            }
#endif
        }
        aDestSize = aSrcSize;
