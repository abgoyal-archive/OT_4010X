...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "PVFrameAndMetadataUtility"
#if 0
...
// Ryan Lin!!!!
#define PVFMUTIL_ERRORHANDLINGTIMEOUT_VALUE 1
        ...
        PVErrorEventObserver *aErrorObserver, PVInformationalEventObserver *aInfoObserver)
{
    ...
    iPlayer = PVPlayerFactory::CreatePlayer(this, this, this, NULL, iHwAccelerated);
...
void PVFrameAndMetadataUtility::HandleErrorEvent(const PVAsyncErrorEvent& aEvent)
{
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVFrameAndMetadataUtility::HandleErrorEvent() state: %d", pstate));
    switch (pstate)
    {
        case PVP_STATE_ERROR:
        {
            ...
            // !!!!! Using Timeout Mechanism to Detect Error Occur !!!!!
...
void PVFrameAndMetadataUtility::HandleFrameReadyEvent(PVMFStatus aEventStatus)
{
            ...
            iCurrentCmd[0].GetCmdType() == PVFM_UTILITY_COMMAND_GET_FRAME_UTILITY_BUFFER)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVFrameAndMetadataUtility::HandleFrameReadyEvent() Frame In"));
            ...
            uint32 ch = 0;
            // [20100423] Jackal
            uint32 PmemSucBufVA = 0;
            if (iVideoMIO->GetFrameProperties(fw, fh, dw, dh, cw, ch, PmemSucBufVA) == PVMFSuccess)
            {
                ...
                iVideoFrameBufferProp->iCropWidth= cw;
                iVideoFrameBufferProp->iCropHeight= ch;
                // [20100423] Jackal
                iVideoFrameBufferProp->iPmemSrcBufVA = PmemSucBufVA;
            }
            else
            {
                ...
                iVideoFrameBufferProp->iCropWidth= 0;
                iVideoFrameBufferProp->iCropHeight= 0;
                // [20100423] Jackal
                iVideoFrameBufferProp->iPmemSrcBufVA = 0;
            }

            // add by Ryan
#if defined(MT6516_MP4_HW_DECODER_V2) || defined(MT6573_MFV_HW)
            PVMFFormatType format = PVMF_MIME_YUV420_PLANAR;
            if (iVideoMIO->GetFrameFormatType(format) == PVMFSuccess)
            {
                iVideoFrameBufferProp->mVideoSubFormat = format;
            }
#endif            
        }
...
PVMFStatus PVFrameAndMetadataUtility::DoPlayerShutdownRestart(void)
{
    ...
    OSCL_TRY(leavecode, iPlayer = PVPlayerFactory::CreatePlayer(this, this, this, NULL, iHwAccelerated));
    OSCL_FIRST_CATCH_ANY(leavecode,
