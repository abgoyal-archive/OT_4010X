...
class PVAuthorEngineNodeFactoryUtility
{
    public:
        ...
        static bool QueryRegistry(const PvmfMimeString& aMimeType, PVUuid& aUuid)
        {
            ...
            else if (CompareMimeTypes(aMimeType, OSCL_HeapString<OsclMemAllocator>(KPcmEncMimeType)) ||
				     CompareMimeTypes(aMimeType, OSCL_HeapString<OsclMemAllocator>(KAdcmEncMimeType)))
            {
#if USE_OMX_ENC_NODE
				//by HP Cheng
                // replace amr encoder node with omx encoder node
...
#endif
	     // HP Cheng add KWavComposerMimeType
                     ...
                     CompareMimeTypes(aMimeType, OSCL_HeapString<OsclMemAllocator>(KWavComposerMimeType)) ||
