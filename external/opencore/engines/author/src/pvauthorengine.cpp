...
PVMFStatus PVAuthorEngine::DoAddMediaTrack(PVEngineCommand& aCmd)
{
	...
	// add by HP Cheng
	// 1. first judge if source node could connect to composer node directly?
	// 2. judge if it is a PCM16 type encoder, if yes, we do connect source node to composer node directly.
	// note: while doing the connection, composer node is the master, it will ask for 'sampling rate' and 'channel num'
	// from source node while doing connect

	// for the judgement of Source node data type equal to Composer node
	bool srcDataEqual2Compser = false;

	// Get source node capability
    PVMFNodeCapability srcNodeCapability;
    if (inputNodeContainer->iNode->GetCapability(srcNodeCapability) != PVMFSuccess)
    {
        LOG_ERR((0, "PVAuthorEngine::IsCompressedFormatDataSource: Error - GetCapability failed"));
        return PVMFFailure;
    }

	// Already has composer node capability, see 'composerNodeCapability'

	// Check if match
	for (uint32 i = 0; i < srcNodeCapability.iOutputFormatCapability.size(); i++)
	{
		for ( uint32 j = 0 ; j < composerNodeCapability.iInputFormatCapability.size(); j++)
		{

			PVMFFormatType formatSrc = (srcNodeCapability.iOutputFormatCapability[i]);
			PVMFFormatType formatCom = (composerNodeCapability.iInputFormatCapability[j]);

			if (formatSrc == formatCom)
			{
				srcDataEqual2Compser = true;
			}
		}
	}

	// if Source and Composer node has same data type, and also now it is a PCM16 encoder
	// we could directly connect them
	srcDataEqual2Compser = srcDataEqual2Compser && ( compressedFormatMimeType == PVMF_MIME_PCM16 );


	// modified by HP Cheng for handling src data = composer data case
   if (compressedDataSrc || srcDataEqual2Compser)
   //if (compressedDataSrc)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "hphp: PVAuthorEngine::DoAddMediaTrack, connect directly"));
...
PVMFStatus PVAuthorEngine::GetPvmfFormatString(PvmfMimeString& aMimeType, const PvmfMimeString& aNodeMimeType)
{
	...
	else if (aNodeMimeType == KPcmEncMimeType)		//added by HP Cheng
    {
        aMimeType = PVMF_MIME_PCM16;
    }
	else if (aNodeMimeType == KAdcmEncMimeType)		//added by HP Cheng
    {
        aMimeType = PVMF_MIME_ADPCM;
    }
    ////////////////////////////////////////////////////////////////////////////
