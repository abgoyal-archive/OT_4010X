...
static bool bUsingTraceTool = false;
#define LOGP(...) \
{\
    if (bShowLog)\
    {\
...
PVPlayerEngine* PVPlayerEngine::New(PVCommandStatusObserver* aCmdStatusObserver,
                                    ...
                                    bool aHwAccelerated, uint32* pUserData)
{
    PVPlayerEngine* engine = NULL;
    engine = OSCL_NEW(PVPlayerEngine, (aHwAccelerated, pUserData));
    if (engine)
    {
...
#if USING_AVSYNC_CONTROL
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
        if (iDatapathList[i].iSinkNode != NULL)
        {
            iDatapathList[i].iSinkNode->SetAVSyncCtrl(NULL);
        }
    }
    iPlaybackClock.setAVSyncCtrl(NULL);
#endif    
...
PVPlayerEngine::PVPlayerEngine(bool aHwAccelerated, uint32* pUserData) :
        ...
        iOverflowFlag(false),
        iPlayerDriverUserData(pUserData),
        iHttpRepositionBeforeStart(false),
        iIsHttpStreaming(false),
    // <--- Morris Yang ALPS00135233
        iIgnoreMoovDataReady(false)
    // --->
{
    bShowLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"log_playerengine");
    bUsingTraceTool = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"enable_ftrace");
    iDebugTest = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"debug_test");
    if (Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"log_profile") | 
        Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"save_profile"))
    {
        OsclProfile::Start();
    }
...
#if USING_AVSYNC_CONTROL
    iPlaybackClock.setAVSyncCtrl(&mAVSyncCtrl);
#endif
}
...
bool PVPlayerEngine::FindNodeTypeByNode(PVMFNodeInterface* aUnknownNode, PVPlayerNodeType& aNodeType, int32& aDatapathListIndex)
{
    ...
    bool bDecNodeAssgine = false;
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
        if (iDatapathList[i].iDecNode != NULL)
        {
            bDecNodeAssgine = true;
        }
    }
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVPlayerEngine::NodeCommandCompleted() In Tick=%d", OsclTickCount::TickCount()));
    LOG_PROFILE("NodeCommandCompleted() In, %lld", OsclTickCount::getTimeMs());
    ...
    if (context_uint32 == &iNumberCancelCmdPending)
    {
        ...
        if (iNumberCancelCmdPending == 0)
        {
			...
			if(!iSourceNode)
			{
			    LOGE("NodeCommandCompleted: iSourceNode NULL");
			}
			// ]
...
void PVPlayerEngine::HandleNodeInformationalEvent(const PVMFAsyncEvent& aEvent)
{
    ...
    else if (FindNodeTypeByNode(nodeorigin, nodetype, datapathindex) == true)
    {
        ...
        else
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_WARNING, (0, "PVPlayerEngine::HandleNodeInformationalEvent() Info event from unknown node type Event type 0x%x Context 0x%x Data 0x%x",
...
void PVPlayerEngine::HandlePlayerDatapathEvent(int32 /*aDatapathEvent*/, PVMFStatus aEventStatus, OsclAny* aContext, PVMFCmdResp* aCmdResp)
{
    ...
    if (context_uint32 == &iNumberCancelCmdPending)
    {
        ...
        if (iNumberCancelCmdPending == 0)
        {
			...
			if(!iSourceNode)
			{
			    LOGE("HandlePlayerDatapathEvent: iSourceNode NULL");
			}
			// ]
            OSCL_TRY(leavecode, cmdid = iSourceNode->Reset(iSourceNodeSessionId, (OsclAny*)context));
            OSCL_FIRST_CATCH_ANY(leavecode,
...
void PVPlayerEngine::RecognizeCompleted(PVMFFormatType aSourceFormatType, OsclAny* aContext)
{
    ...
    if (context_uint32 == &iNumberCancelCmdPending)
    {
        ...
        if (iNumberCancelCmdPending == 0)
        {
            ...
            {
            	// Honda add for debug
            	LOGE("RecognizeCompleted: iSourceNode NULL");
            }
            SetEngineState(PVP_ENGINE_STATE_RESETTING);
        }
        return;
        ...
        const PVUuid* aUuid, bool aAPICommand, PVCommandId* aId)
{
    ...
    LOG_PROFILE("AddCommandToQueue() Type=%d, %lld", aCmdType, OsclTickCount::getTimeMs());
    return cmd.GetCmdId();
}
...
PVPlayerState PVPlayerEngine::GetPVPlayerState(void)
{
    switch (iState)
    {
        ...
        case PVP_ENGINE_STATE_STOPPING:
        {
        // <--- Morris Yang ALPS00133034 For suspend/resume or MT call in http streaming
            if ((PVP_ENGINE_STATE_AUTO_PAUSED == iState) && iIsHttpStreaming && iHttpRepositionBeforeStart)
            	return PVP_STATE_PREPARED;
        // --->
...
void PVPlayerEngine::EngineCommandCompleted(PVCommandId aId, OsclAny* aContext, PVMFStatus aStatus, PVInterface* aExtInterface, OsclAny* aEventData, int32 aEventDataSize)
{
    ...
    LOG_PROFILE("EngineCommandCompleted() Type=%d, %lld",
                     completedcmd.GetCmdType(), OsclTickCount::getTimeMs());
    ...
    switch (completedcmd.GetCmdType())
    {
        ...
        // Add by Ryan for profiling
        case PVP_ENGINE_COMMAND_QUERY_INTERFACE:
            // Reset Tickcount for profiling                        
            //LOGV("Reset Tickcount: %d", OsclTickCount::TickCount(true));
            break;
    ...
    if (iCmdStatusObserver)
    {
        if (aId != -1 && completedcmd.IsAPICommand())
        {
            ...
            LOG_PROFILE("iCmdStatusObserver->CommandCompleted(), %lld", OsclTickCount::getTimeMs());
    ...
    else
    {
        ...
        if (iSourceNode)
        {
            ...
            if (iSourceNode->GetState() != EPVMFNodeCreated)
            {
			    ...
			    if(!iSourceNode)
			    {
			        LOGE("DoCancelAllCommands: iSourceNode NULL");
			    }
			    // ]
                OSCL_TRY(leavecode, cmdid = iSourceNode->Reset(iSourceNodeSessionId, (OsclAny*)context));
                OSCL_FIRST_CATCH_ANY(leavecode,
...
void PVPlayerEngine::DoCancelCommandBeingProcessed(void)
{
    ...
    switch (iCmdToCancel[0].GetCmdType())
    {
        ...
        case PVP_ENGINE_COMMAND_STOP:
        {
		    ...
		    if(!iSourceNode)
		    {
		        LOGE("DoCancelCommandBeingProcessed: iSourceNode NULL");
		    }
		    // ]
            OSCL_TRY(leavecode, cmdid = iSourceNode->Reset(iSourceNodeSessionId, (OsclAny*)context));
            OSCL_FIRST_CATCH_ANY(leavecode,
...
PVMFStatus PVPlayerEngine::DoCancelPendingNodeDatapathCommand()
{
    ...
    for (uint32 i = 0; i < iCurrentContextList.size(); ++i)
    {
        if (iCurrentContextList[i]->iNode)
        {
            ...
            {
				// modified by donghui wang to fix ALPS00122138
...
PVMFStatus PVPlayerEngine::DoAddDataSource(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoAddDataSource(), %lld", OsclTickCount::getTimeMs());
    ...
    char originalFile[1024] = {0};

    oscl_UnicodeToUTF8( iDataSource->GetDataSourceURL().get_cstr(),
		iDataSource->GetDataSourceURL().get_size(),
		originalFile,
		1024);
    LOGD("!!!!!**************************************************!!!!!");
    LOGD("!!!!!          DataSource url(%s)(%d)        ", originalFile, gettid());
    // (mg) For rollover reset to first available alternate
    iAlternateSrcFormatIndex = 0;
    iDataReadySent = false;

    // <--- Morris Yang ALPS00126974
    if (oscl_strncmp(iDataSource->GetDataSourceFormatType().getMIMEStrPtr(), PVMF_MIME_DATA_SOURCE_HTTP_URL, oscl_strlen(PVMF_MIME_DATA_SOURCE_HTTP_URL)) == 0)
    {
        iIsHttpStreaming = true;
        if ((oscl_strlen(originalFile) > 11) && (oscl_strstr(originalFile, ".mp3") != NULL))
        {
            // check if we are (possibly) streaming a mp3 clip, if yes set the RequestReadCapacityNotification size to PVMF_MP3_REC_PLUGIN_FILE_SCAN_SIZE
            iPlayerRecognizerRegistry.SetMaxRequiredSizeForRecognitionOverride(8*1024);  //PVMF_MP3_REC_PLUGIN_FILE_SCAN_SIZE
        }
    }
    // --->
...
PVMFStatus PVPlayerEngine::DoQuerySourceFormatType(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoQuerySourceFormatType(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSetupSourceNode(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSetupSourceNode(), %lld", OsclTickCount::getTimeMs());
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVPlayerEngine::DoSetupSourceNode() In"));

    OSCL_ASSERT(iDataSource != NULL);

    LOGD("!!!!!          SourceFormatType(%s)           ", iSourceFormatType.getMIMEStrPtr());
    LOGD("!!!!!**************************************************!!!!!");
    if (iSourceNode == NULL)
    {
        ...
        if (iPlayerNodeRegistry.QueryRegistry(iSourceFormatType, outputformattype, foundUuids) == PVMFSuccess)
        {
            ...
            if (iSourceNode)
            {
                iSourceNode->SetPlayerDriverUserData(iPlayerDriverUserData);
            }                     
...
PVMFStatus PVPlayerEngine::DoSourceNodeQueryTrackSelIF(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSourceNodeQueryTrackSelIF(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSourceNodeQueryInterfaceOptional(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSourceNodeQueryInterfaceOptional(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoGetMetadataKey(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoGetMetadataKey(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoGetMetadataValue(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoGetMetadataValue(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoReleaseMetadataValues(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoReleaseMetadataValues(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoInit(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoInit(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSourceNodeInit(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSourceNodeInit(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoAcquireLicense(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoAcquireLicense(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::DoCancelAcquireLicense(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoCancelAcquireLicense(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSourceNodeGetLicense(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSourceNodeGetLicense(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoAddDataSink(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoAddDataSink(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSetPlaybackRange(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoSetPlaybackRange(), %lld", OsclTickCount::getTimeMs());
    ...
    // HP cheng:
    // Originally added by Ryan for solving un-confortable sound of 2 seek very close to each other
    // this happens due to music apk behavior
    // The modification has some side effect:
    // Selecting 'repeat current song', when repeat, the beginning of the song will not be played
    // The solution to this is to remove this clear buffer, and modify apk behavior
    ...
    if (isVideoSource())
    {
        if (iPlaybackClock.getAVSyncCtrl() != NULL)
        {
            iPlaybackClock.getAVSyncCtrl()->StartSeekTo();

#if 1    
            LOGP("***** pause audio output *****");
            for (uint32 i = 0; i < iDatapathList.size(); ++i)
            {
                if (iDatapathList[i].iSinkNode != NULL)
                {
                    iDatapathList[i].iSinkNode->ResetNodeParam();
                }
            }
#endif  
        }
        ...
        bool aSFR)
{
    ...
    LOG_PROFILE("DoSinkNodeSkipMediaDataDuringPlayback(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoPrepare(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoPrepare(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSinkNodeInit(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSinkNodeInit(), %lld", OsclTickCount::getTimeMs());
    ...
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
        ...
        {
#if USING_AVSYNC_CONTROL
            iDatapathList[i].iSinkNode->SetAVSyncCtrl(&mAVSyncCtrl);
#endif
                        ...
                        if (iPlayerNodeRegistry.QueryRegistry(iSrcFormat, iSinkFormat, foundUuids) == PVMFSuccess)
                        {
                            if (!foundUuids.empty())
                            {
                                ...
                                if (iTrackSelectionList[i].iTsDecNode != NULL)
                                {
                                    ...
                                    // add by Ryan for return all buffer
                                    iTrackSelectionList[i].iTsDecNode->SetSinkNode(iTrackSelectionList[i].iTsSinkNode);
                                ...
                                else
                                {
                                    ...
                                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_WARNING, (0, "PVPlayerEngine::DoDecNodeQueryCapConfigIF() Dec node creation failed"));
                                }
                            }
                            else
                            {
                                // No matching node found with the given Sinkformat, check with the next Sink Format
                                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_WARNING, (0, "PVPlayerEngine::DoDecNodeQueryCapConfigIF() No matching decoder node found"));
                            }
                        }
                        else
                        {
                            // Registry query failed with the given Sinkformat, check with the next Sink Format.
                            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_WARNING, (0, "PVPlayerEngine::DoDecNodeQueryCapConfigIF() Registry query for dec node failed"));
                        }
                    }
...
PVMFStatus PVPlayerEngine::DoDecNodeInit(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoDecNodeInit(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSinkNodeDecNodeReset(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSinkNodeDecNodeReset(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSinkDecCleanupSourcePrepare(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSinkDecCleanupSourcePrepare(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSourceNodePrepare(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSourceNodePrepare(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSinkNodeQueryInterfaceOptional(PVPlayerEngineDatapath &aDatapath, PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSinkNodeQueryInterfaceOptional(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoDecNodeQueryInterfaceOptional(PVPlayerEngineDatapath &aDatapath, PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoDecNodeQueryInterfaceOptional(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoDatapathPrepare(PVPlayerEngineDatapath &aDatapath, PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoDatapathPrepare() for %s, %lld",
                     aDatapath.iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSourceNodeStart(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSourceNodeStart(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoDatapathStart(PVPlayerEngineDatapath &aDatapath, PVCommandId aCmdId, OsclAny* aCmdContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::DoDatapathStart() for %s Tick=%d",
                     aDatapath.iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("DoDatapathStart() for %s, %lld",
                     aDatapath.iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSinkNodeSkipMediaData(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    LOG_PROFILE("DoSinkNodeSkipMediaData(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoStart(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoStart(), %lld", OsclTickCount::getTimeMs());
    
// <--- Morris Yang ALPS00133034
    if (iIsHttpStreaming && iHttpRepositionBeforeStart)
    	iHttpRepositionBeforeStart = false;
// --->
    ...
    // Added by HP Cheng for making sure that pure audio cannot activate this AVSync lock.
    // if we find there is only one data path and sink node input type is PCM16 or PCM8, 
    // we can say that we do not use this lock.
    // if we use this lock on pure audio mp4 files, it will wait until the lock time out (1 sec.)
    bool flagSingleAudioPath = false;
    if (iDatapathList.size() == 1)
    {
        PVMFNodeCapability nodeCapability;
        iDatapathList[0].iSinkNode->GetCapability(nodeCapability);
        //LOGD(" sink node input camability : %s", nodeCapability.iInputFormatCapability[0].getMIMEStrPtr());
        
        if ( pv_mime_strcmp(nodeCapability.iInputFormatCapability[0].getMIMEStrPtr(), PVMF_MIME_PCM16) == 0 || 
              pv_mime_strcmp(nodeCapability.iInputFormatCapability[0].getMIMEStrPtr(), PVMF_MIME_PCM8) == 0 )
        {
             	//LOGD("flagSingleAudioPath = true");
             	flagSingleAudioPath = true;
        }
    }

    if (isVideoSource() && !flagSingleAudioPath)
    {
        if (iPlaybackClock.getAVSyncCtrl() != NULL)
        {
            iPlaybackClock.getAVSyncCtrl()->Init();
        }
    }
#endif    
    
    // <--- Donglei Ji ALPS00131944 ALPS00136233(isVideoSource is not incorrect, so change method)
    uint32 i;
    iPlaybackClock.EnableTimeAdjustment(true);
    // check data sinks if it is video
    for (i = 0; i < iDatapathList.size(); ++i)
    {
        if (iDatapathList[i].iSinkFormat.isVideo())
        {
        	  iPlaybackClock.EnableTimeAdjustment(false);
        }
    }
    // --->
...
PVMFStatus PVPlayerEngine::DoPause(PVPlayerEngineCommand& aCmd)
{
    ...
    LOG_PROFILE("DoPause(), %lld", OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoDatapathPause(PVPlayerEngineDatapath& aDatapath, PVCommandId aCmdId, OsclAny* aCmdContext, bool aSinkPaused)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::DoDatapathPause() for %s Tick=%d",
                     aDatapath.iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("DoDatapathPause() for %s, %lld",
                     aDatapath.iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
PVMFStatus PVPlayerEngine::DoSourceUnderflowAutoPause(PVPlayerEngineCommand& aCmd)
{
    ...
    switch (iState)
    {
        ...
        // <--- Morris Yang ALPS00133034
        if (iIsHttpStreaming)
            iHttpRepositionBeforeStart = true;
        // --->
...
void PVPlayerEngine::DoEngineDatapathCleanup(PVPlayerEngineDatapath& aDatapath)
{
    ...
    if (aDatapath.iDatapath)
    {
        ...
        // Honda add
        aDatapath.iSinkThreadCreated = false;
    }
...
void PVPlayerEngine::HandleSourceNodeQueryInitIF(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodeQueryInitIF(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodeQueryTrackSelIF(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodeQueryTrackSelIF(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodeQueryInterfaceOptional(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodeQueryInterfaceOptional(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodeInit(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodeInit(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodeGetDurationValue(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodeGetDurationValue(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSinkNodeQueryCapConfigIF(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSinkNodeQueryCapConfigIF(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSinkNodeInit(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSinkNodeInit(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDecNodeQueryCapConfigIF(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleDecNodeQueryCapConfigIF(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDecNodeInit(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleDecNodeInit(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodePrepare(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodePrepare(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSinkNodeQueryInterfaceOptional(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSinkNodeQueryInterfaceOptional(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSinkNodeDecNodeReset(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSinkNodeDecNodeReset(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDecNodeQueryInterfaceOptional(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleDecNodeQueryInterfaceOptional(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodeStart(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSourceNodeStart(), %lld", OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSinkNodeSkipMediaData(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    ...
    LOG_PROFILE("HandleSinkNodeSkipMediaData() for %s, %lld",
                     aNodeContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iReposLogger, PVLOGMSG_INFO,
...
void PVPlayerEngine::HandleSinkNodeSkipMediaDataDuringPlayback(PVPlayerEngineContext& aNodeContext, const PVMFCmdResp& aNodeResp)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleSinkNodeSkipMediaDataDuringPlayback() for %s Tick=%d",
                     aNodeContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleSinkNodeSkipMediaDataDuringPlayback() for %s, %lld",
                     aNodeContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iReposLogger, PVLOGMSG_INFO,
    ...
    if ((iNumPendingSkipCompleteEvent == 0) && (iNumPVMFInfoStartOfDataPending == 0))
    {
        ...
        //Zhihui Zhang ALPS00136562 work around for app to prevent from playbackclock start when auto play mp3 after power on
...
void PVPlayerEngine::HandleDatapathPrepare(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    ...
    LOG_PROFILE("HandleDatapathPrepare() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDatapathStart(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleDatapathStart() for %s Tick=%d",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleDatapathStart() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDatapathPause(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleDatapathPause() for %s Tick=%d",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleDatapathPause() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDatapathResume(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleDatapathResume() for %s Tick=%d",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleDatapathResume() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
    ...
    if (iNumPendingDatapathCmd == 0)
    {
        ...
        {
       // <--- Morris Yang ALPS00126917 
            if ((iPlaybackClock.GetState() == PVMFMediaClock::STOPPED) && (iPlaybackClock.IsRtspStreamingClock())) {
                LOGE ("Oops, force RTSP clock start to run!!!!");
            }
            ...
            if ((iPlaybackClock.GetState() == PVMFMediaClock::PAUSED) || 
            	((iPlaybackClock.GetState() == PVMFMediaClock::STOPPED) && (iPlaybackClock.IsRtspStreamingClock())))
       // --->
            {
                StartPlaybackClock();
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO, (0, "PVPlayerEngine::HandleDatapathResume() Report command as completed"));
        EngineCommandCompleted(aDatapathContext.iCmdId, aDatapathContext.iCmdContext, PVMFSuccess);
    }
...
void PVPlayerEngine::HandleDatapathStop(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleDatapathStop() for %s Tick=%d",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleDatapathStop() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDatapathTeardown(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleDatapathTeardown() for %s Tick=%d",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleDatapathTeardown() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleDatapathReset(PVPlayerEngineContext& aDatapathContext, PVMFStatus aDatapathStatus, PVMFCmdResp* aCmdResp)
{
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF, iPerfLogger, PVLOGMSG_INFO,
                    (0, "PVPlayerEngine::HandleDatapathReset() for %s Tick=%d",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::TickCount()));
    LOG_PROFILE("HandleDatapathReset() for %s, %lld",
                     aDatapathContext.iEngineDatapath->iTrackInfo->getTrackMimeType().get_cstr(), OsclTickCount::getTimeMs());
...
void PVPlayerEngine::HandleSourceNodeInfoEvent(const PVMFAsyncEvent& aEvent)
{
    ...
    switch (event)
    {
        ...
        // <--- Morris Yang ALPS00135233
            if (iIsHttpStreaming && iIgnoreMoovDataReady)
            {
                LOGD ("Ignore PVMFInfoDataReady for moov!!!");
                iIgnoreMoovDataReady = false;
                break;
            }
        // --->
    ...
    // <--- Morris Yang ALPS00006721 [
        ...
        {
        	SendInformationalEvent(PVMFInfoNotSeekable);
        	break;
        }
    // --->
    // <--- sam sun for aac seek table gen [
        ...
        {
        	SendInformationalEvent(PVMFInfoSeekable);
        	break;
        }
    // --->
    ...
    // <--- Morris Yang ALPS00135233
        case PVMFInfoDownloadingMoov:
        {
              iIgnoreMoovDataReady = true;
        	break;
        }
    // --->
...
void PVPlayerEngine::HandleSinkNodeInfoEvent(const PVMFAsyncEvent& aEvent, int32 aDatapathIndex)
{
    ...
    {
		// Honda add
    	case PVMFInfoSinkThreadCreate:
    	{
    		if (iDatapathList[aDatapathIndex].iDatapath)
    		{
    		    iDatapathList[aDatapathIndex].iSinkThreadCreated = true;
    		}
    	}
    	break;
	...
	//<---weiguo Li. alps00129892. sometimes PVMFInfrEndOfData is received after stop Command,
	//if this happen, just ignore this event or when prepare again prepare will fail. 
	if( iState == PVP_ENGINE_STATE_STOPPING )
	{
	    LOGW("PVPlayerEngine::HandleSinkNodeInfoEvent() PVMFInfoEndOfData event received with istate=%d",iState);
	    break;
	}
        //--->
            if (iDatapathList[aDatapathIndex].iDatapath && iDatapathList[aDatapathIndex].iEndOfDataReceived == false)
            {
                ...
                {
                    LOGP("PVPlayerEngine::HandleSinkNodeInfoEvent() Issue Pause due to end of clip");                    
...
bool PVPlayerEngine::AllDatapathReceivedEndOfData()
{
    ...
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
        if ((iDatapathList[i].iDatapath) && (iDatapathList[i].iSinkThreadCreated == true) &&
                (iDatapathList[i].iEndOfDataReceived == false))
        {
            return false;
        }
...
void PVPlayerEngine::SendEndOfClipInfoEvent(PVMFStatus aStatus, PVInterface* aExtInterface)
{
    ...
    {
        LOGP("!!!!! PVPlayerEngine::SendEndOfClipInfoEvent !!!!!");
...
void PVPlayerEngine::SendEndTimeReachedInfoEvent(PVMFStatus aStatus, PVInterface* aExtInterface)
{
    ...
    {
        LOGP("!!!!! PVPlayerEngine::SendEndTimeReachedInfoEvent !!!!!");
        PVUuid puuid = PVPlayerErrorInfoEventTypesUUID;
...
void PVPlayerEngine::StartPlaybackClock()
{
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO, (0, "PVPlayerEngine::StartPlaybackClock() StartNPT %d StartTS %d", iStartNPT, iStartMediaDataTS));
}
...
void PVPlayerEngine::ResetEngineForLooping()
{
    LOGP("***** ResetEngineForLooping *****");
    if (iSourceNode)
    {
        iSourceNode->ResetNodeParam();
    }
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
        if (iDatapathList[i].iDecNode)
        {
            iDatapathList[i].iDecNode->ResetNodeParam();
        }
...
void PVPlayerEngine::ClearAllNodesBuffer()
{
    LOGP("***** ClearAllNodesBuffer *****");
    if (iSourceNode)
    {
        iSourceNode->ClearBuffer();
    }
    for (uint32 i = 0; i < iDatapathList.size(); ++i)
    {
        if (iDatapathList[i].iDecNode)
        {
            iDatapathList[i].iDecNode->ClearBuffer();
        }
        if (iDatapathList[i].iSinkNode)
        {
            iDatapathList[i].iSinkNode->ClearBuffer();
        }
    }
}
...
PVMFStatus PVPlayerEngine::DoErrorHandling()
{
    ...
    if (iSourceNode)
    {
        ...
        if (iSourceNode->GetState() != EPVMFNodeCreated)
        {
		    ...
		    if(!iSourceNode)
		    {
		        LOGE("DoErrorHandling: iSourceNode NULL");
		    }
		    // ]
            OSCL_TRY(leavecode, cmdid = iSourceNode->Reset(iSourceNodeSessionId, (OsclAny*)context));
            OSCL_FIRST_CATCH_ANY(leavecode,
...
bool PVPlayerEngine::isVideoSource()
{
    return iSourceFormatType.isVideo();
}
