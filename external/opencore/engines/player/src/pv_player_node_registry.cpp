...
//LOGD("PVPlayerNodeRegistry::AddLoadableModules --- libList size: %d", libList.Size());
    for (unsigned int i = 0; i < libList.Size(); i++)
    {
        ...
        {
//LOGD("PVPlayerNodeRegistry::AddLoadableModules --- lib name: %s", libList.GetLibraryPathAt(i).get_cstr());           
...
PVMFNodeInterface* PVPlayerNodeRegistry::CreateNode(PVUuid& aUuid, bool aHwAccelerated)
{
    ...
    if (iFoundFlag)
    {
        ...
        if (KPVMFOMXVideoDecNodeUuid == aUuid)
        {
            ...
            //priority = OsclActiveObject::EPriorityHigh;
        }
#endif
...
//LOGD("PVPlayerRecognizerRegistry::AddLoadableModules --- libList size: %d", libList.Size());
    for (unsigned int i = 0; i < libList.Size(); i++)
    {
        OsclSharedLibrary* lib = OSCL_NEW(OsclSharedLibrary, ());
        if (lib->LoadLib(libList.GetLibraryPathAt(i)) == OsclLibSuccess)
        {
//LOGD("PVPlayerRecognizerRegistry::AddLoadableModules --- lib name: %s", libList.GetLibraryPathAt(i).get_cstr());              
            OsclAny* interfacePtr = NULL;
...
void PVPlayerRecognizerRegistry::SetMaxRequiredSizeForRecognitionOverride(int32 iMaxSizeOverride)
{
    PVMFRecognizerRegistry::SetMaxRequiredSizeForRecognitionOverride(iMaxSizeOverride);
}
// --->
