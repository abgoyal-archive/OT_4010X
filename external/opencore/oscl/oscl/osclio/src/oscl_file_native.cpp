...
static pthread_mutex_t gFileReadMutex = PTHREAD_MUTEX_INITIALIZER;
#endif
//  ]
...
OsclNativeFile::OsclNativeFile()
{
    ...
    //bSaveReadTime = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "save_fread");
    if (bSaveReadTime)
    {
        if (pf == NULL)
        {
            pf = fopen("/fread.txt", "w+");
        }
    }
}
                           ...
                           , Oscl_FileServer& fileserv)
{
...

    {
        ...
        LOGD("OsclNativeFile(0x%x)::Open: %s", (uint32)this, convfilename);
...

    {
        if (iOpenFileHandle)
        {
            ...
            if (iWriteDirty == 1)
            {
                ...
                //[ALPS00006015][Camera]Can not save video just record
                fsync(fileno(iFile));
            }
        }
        else if (iFile != NULL)
    ...
    if (iFile)
    {
        ...
        if (bSaveReadTime)
        {
            if (pf != NULL)
            {
                ...
                fprintf(pf, "read in, %lld, bytes, %d\n", _start, size * numelements);
            }
        }
        _size = fread(buffer, OSCL_STATIC_CAST(int32, size), OSCL_STATIC_CAST(int32, numelements), iFile);
        if (bSaveReadTime)
        {
            if (pf != NULL)
            {
