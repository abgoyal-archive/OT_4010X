...
// Ryan Lin !!
#undef LOG_TAG
#define LOG_TAG "OsclFileAsyncRead"
        ...
        iReadPtr(0, iReadPtrDummyLen, 0)
{
    ...
    //from the async reads.  It must be <= iKCacheBufferCount * iTotalCacheSize;
     //LOGE ("OAF(0x%x)::OAF  iTotalCacheSize=%u\n", this, iTotalCacheSize);
...
void OsclAsyncFile::ConstructL()
{
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::ConstructL bufferSize %d numBuffers %d readSize %d readAhead %d"
                     , (uint32)this, iTotalCacheSize, iKCacheBufferCount, iKAsyncReadBufferSize, iKMinBytesReadAhead));
}
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Open mode %d ", (uint32)this, mode));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Open filename '%s' mode %d ", (uint32)this, filename, mode));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Close", (uint32)this));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::StartAsyncRead %d", (uint32)this, aStartAsyncRead));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Read size %d numelements %d", (uint32)this, aDataSize, aNumElements));
   //uint32 StartTime = OsclTickCount::TicksToMsec(OsclTickCount::TickCount());
   //LOGE ("+OAF(0x%x)::Read size=%d numelements=%d, bytesToRead=%d, tick=%u\n", this, aDataSize, aNumElements, aDataSize*aNumElements, StartTime);
    ...
    //uint32 EndTime = OsclTickCount::TicksToMsec(OsclTickCount::TickCount());
    //LOGE ("-OAF(0x%x)::Read size=(%d) numelements=(%d) bytesToRead=(%d) return %d, tick=%u ms\n", this, aDataSize, aNumElements, aDataSize*aNumElements, bytesRead, EndTime);
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::doRead size %d numelements %d offset %d", (uint32)this, aDataSize, aNumElements, (int32)aOffset));
    ...
    else
    {
        ...
        if (ret != 0)
        {
            // This is not good
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OsclAsyncFile(0x%x)::doRead Seek Failed returned %d offset %d", (uint32)this, ret, (int32)aOffset));
            return 0;
        }
...
//        LOGE ("OAF(0x%x):: +iNFD->Read(), aOffset=%d, tick=%u\n", this, aOffset, OsclTickCount::TicksToMsec(OsclTickCount::TickCount()));
        bytesRead = iNativeFileDuplicate->Read(ptrCurrentBuffer.Ptr(), 1, iTotalCacheSize);
 //       LOGE ("OAF(0x%x):: -iNFD->Read(), tick=%u, bytesRead=%u\n", this, OsclTickCount::TicksToMsec(OsclTickCount::TickCount()), bytesRead);
...
void OsclAsyncFile::Run()
{
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                        (0, "OsclAsyncFile(0x%x)::Run Error!!!", (uint32)this));
        iNumOfRunErr++;
        return;
    }
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Run numBytes %d ", (uint32)this, iReadPtr.Length()));
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                        (0, "OsclAsyncFile(0x%x)::Run Reached end of file ", (uint32)this));
        return;
    }
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                        (0, "OsclAsyncFile(0x%x)::Run Reached the read-ahead count", (uint32)this));
        return;
    }
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::StartNextRead pos %d ", (uint32)this, (int32)aPosToReadFrom));
    ...
    else if (aPosToReadFrom == iSyncFilePosition)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OsclAsyncFile(0x%x)::StartNextRead Seek failed, return %d, offset %d ", (uint32)this, result, (int32)aPosToReadFrom));
            return;
        }
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::StartNextRead Issuing async read, size %d ", (uint32)this, iKAsyncReadBufferSize));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Seek offset %d origin %d", (uint32)this, (int32)offset, origin));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                    (0, "OsclAsyncFile(0x%x)::Tell pos %d ", (uint32)this, (int32)iFilePosition));
...
void OsclAsyncFile::LaunchAsyncReadThread()
{
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                        (0, "OsclAsyncFile(0x%x)::LaunchAsyncReadThread starting thread ", (uint32)this));
...
void OsclAsyncFile::StopAsyncReadThread()
{
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                        (0, "OsclAsyncFile(0x%x)::StopAsyncReadThread stopping thread ", (uint32)this));
...
{
    OSCL_SET_THREAD_NAME("OAF_InThread");
    
#ifdef USES_ASYNC_FILE_IO
    LOGD("!!!!!**************************************************!!!!!");
    // add by Honda
    ...
    {
        LOGE("[%s] OsclAsyncFile:: failed, errno: %d", __func__, errno);
    }
    else
    {
        sched_p.sched_priority = 0;
        sched_getparam(0, &sched_p);
        LOGP("!!!!!  (0x%x) OsclAsyncFile:: sched_setscheduler ok, SCHED_RR, priority: %d", (uint32)this, sched_p.sched_priority);
    }
    LOGP("!!!!!**************************************************!!!!!");
#endif
