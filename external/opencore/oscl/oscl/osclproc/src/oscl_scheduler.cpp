...
// Ryan Lin !!
#undef LOG_TAG
#define LOG_TAG "OsclScheduler"

static bool bSaveAOStatus = false;
...
OsclExecSchedulerCommonBase::~OsclExecSchedulerCommonBase()
{
    ...
    if (IsInstalled())
    {
        LOGE("OsclExecSchedulerCommonBase::~OsclExecSchedulerCommonBase()");
...
{
    bLog_Profile = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"log_ao");
    bSaveAOStatus = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, (char*)"save_ao");
    if (pf == NULL && bSaveAOStatus)
    {
        pf = fopen("/ao.txt", "w+");
    }
...
void OsclExecSchedulerCommonBase::UninstallScheduler()
{
    ...
    if (iBlockingMode)
    {
        ...
        if (IsStarted())
        {
            LOGE("OsclExecSchedulerCommonBase::UninstallScheduler");
            OsclError::Leave(OsclErrInvalidState);//scheduler not stopped
        }
    }
    else if (IsStarted())
...
//called before entering scheduling loop.
{
    ...
    if (IsStarted())
    {
        LOGE("OsclExecSchedulerCommonBase::BeginScheduling(");
...
//Will leave if any AO leaves.
{
    ...
    LOGSTATS((0, "PVSCHED:Scheduler '%s', Thread 0x%x: Err: %d to leave", iName.Str(), PVThreadContext::Id(), err));

    EndScheduling();

    if (err)
    {
        LOGERROR((0, "PVSCHED:Scheduler '%s', Thread 0x%x: Err: %d to leave", iName.Str(), PVThreadContext::Id(), err));
        OsclError::Leave(err);
    }
...
//Will leave if any AO leaves.
{
    ...
    {
        LOGE("OsclExecScheduler::RunSchedulerNonBlocking");
...
//Add an AO to the pending timer queue.
{
    ...
    {
        LOGE("OsclExecSchedulerCommonBase::AddToExecTimerQ");
...
//Calling context can be any thread.
{
    ...
    if (err != OsclErrNone)
    {
        LOGE("!!!!! OsclExecSchedulerCommonBase::PendComplete - error code: %d !!!!!", err);
    }
...
//Run a PV AO.
{
    ...
    int32 err;
    long long _starttime = OsclTickCount::getTimeMs();
    LOGV("pvactive(%s)", pvactive->iName.Str());
    OSCL_TRY_NO_TLS(iErrorTrapImp, err, pvactive->Run(););
    long long _endtime = OsclTickCount::getTimeMs();
    LOG_PROFILE("%s (%d), %lld, %lld, %lld\n", pvactive->iName.Str(), gettid(), _starttime, _endtime, _endtime - _starttime); 
...
//Will leave if any AO leaves.
{
    ...
    while (!iDoStop)
    {
        ...
        {
            LOGE("OsclExecSchedulerCommonBase::BlockingLoopL");
