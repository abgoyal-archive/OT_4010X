...
OSCL_IMPORT_REF void PrintTime(long long ms);

OSCL_COND_EXPORT_REF OSCL_INLINE uint32 OsclTickCount::TickCount(bool bResetTickCount)
{
...
#define ROLLBACK_THRESHOLD 0x80000000
    ...
    if (((0 == stv.tv_sec) && (0 == stv.tv_usec)) || bResetTickCount)
    ...
    if ((clk_val - prev_val) > ROLLBACK_THRESHOLD)
    {
        ...
        gettimeofday(&stv, NULL);
        gettimeofday(&tv, NULL);
        clk_val = (tv.tv_sec - stv.tv_sec) * 1000 + (tv.tv_usec - stv.tv_usec) / 1000;
        //clk_val = prev_val;
        // --->
    }
    prev_val = clk_val;
...
OSCL_COND_EXPORT_REF OSCL_INLINE long long OsclTickCount::getTimeMs()
{
#if   PV_USE_GETTIMEOFDAY
    struct timeval t1;
    long long ms;
    ...
    ms = (t1.tv_sec & 255) * 1000LL + t1.tv_usec / 1000;
...
OSCL_COND_EXPORT_REF OSCL_INLINE long long OsclTickCount::getTimeUs()
{
#if   PV_USE_GETTIMEOFDAY
    struct timeval t1;
    long long us;
    ...
    gettimeofday(&t1, NULL);
    us = t1.tv_sec * 1000000LL + t1.tv_usec;
