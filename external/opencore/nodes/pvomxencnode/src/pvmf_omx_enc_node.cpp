...
static const char LOG_ID_AUDIO_WAV[]  = "Audio_WAV";			//added by HP Cheng
...
#include <utils/Log.h>
#define LOG_TAG "OmxEnc_Node"
#undef PVLOGGER_LOGMSG
#define PVLOGGER_LOGMSG(IL, LOGGER, LEVEL, MESSAGE)
//#define PVLOGGER_PROFILING(IL, LOGGER, LEVEL, MESSAGE) MSG MESSAGE
#define PVLOGGER_PROFILING(IL, LOGGER, LEVEL, MESSAGE)
        ...
        ipFixedSizeBufferAlloc(NULL)
{
    ...
    OSCL_TRY(err,
             ...
             iCapability.iOutputFormatCapability.push_back(PVMF_MIME_ADPCM);			//by HP Cheng             
...
bool PVMFOMXEncNode::NegotiateAudioComponentParameters()
{
	...
	else if (iOutFormat == PVMF_MIME_ADPCM)		// added by HP Cheng
    {
        DesiredPortFormat = OMX_AUDIO_CodingADPCM;
    }
    ...
    {
        iParamPort.format.audio.eEncoding = OMX_AUDIO_CodingADPCM;
    }
    ...
    {
        status = SetADPCMEncoderParameters();
    }
    else if ((iOutFormat == PVMF_MIME_ADTS) ||
             (iOutFormat == PVMF_MIME_ADIF) ||
             (iOutFormat == PVMF_MIME_MPEG4_AUDIO))
...
// added by hp cheng
bool PVMFOMXEncNode::SetADPCMEncoderParameters()
{

    OMX_ERRORTYPE Err = OMX_ErrorNone;
    OMX_AUDIO_PARAM_ADPCMTYPE AdpcmType;

    CONFIG_SIZE_AND_VERSION(AdpcmType);
    AdpcmType.nPortIndex = iOutputPortIndex;

    Err = OMX_GetParameter(iOMXEncoder, OMX_IndexParamAudioAdpcm, &AdpcmType);
    if (Err != OMX_ErrorNone)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                        (0, "PVMFOMXEncNode-%s::SetPCMEncoderParameters - Problem getting PCM parameters in output port %d ", iNodeTypeId, iOutputPortIndex));
        return false;
    }

	AdpcmType.nChannels = iAudioEncodeParam.iOutputNumChannels; // must be 1

	AdpcmType.nSampleRate = iAudioEncodeParam.iOutputSamplingRate;            /**< Sampling rate of the source data.  Use 0 for variable or unknown sampling rate. */


    CONFIG_SIZE_AND_VERSION(AdpcmType);
    AdpcmType.nPortIndex = iOutputPortIndex;

    Err = OMX_SetParameter(iOMXEncoder, OMX_IndexParamAudioAdpcm, &AdpcmType);
    if (Err != OMX_ErrorNone)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                        (0, "PVMFOMXEncNode-%s::SetADPCMEncoderParameters - Problem setting ADPCM parameters in output port %d ", iNodeTypeId, iOutputPortIndex));
        return false;
    }
...
bool PVMFOMXEncNode::SendInputBufferToOMXComponent()
{
    ...
    do
    {
        ...
        PVLOGGER_PROFILING(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "2. To OMX %d", OsclTickCount::TicksToMsec(OsclTickCount::TickCount())));
        ...
        OMX_OUT OMX_BUFFERHEADERTYPE* aBuffer)
{
    ...
    PVLOGGER_PROFILING(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "2.5. %s::Emp %d", iNodeTypeId, OsclTickCount::TicksToMsec(OsclTickCount::TickCount())));
        ...
        OMX_OUT OMX_BUFFERHEADERTYPE* aBuffer)
{
    ...
    PVLOGGER_PROFILING(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "3. %s::Fil %d", iNodeTypeId, OsclTickCount::TicksToMsec(OsclTickCount::TickCount())));
...
bool PVMFOMXEncNode::QueueOutputBuffer(OsclSharedPtr<PVMFMediaDataImpl> &mediadataimplout, uint32 aDataLen)
{
    ...
    if (leavecode == 0)
    {
        ...
        if (sendYuvFsi)
        {
			...
			else if (iOutFormat == PVMF_MIME_ADPCM)		//by HP Cheng
			{
				// Save default VOL header
				uint32 wavParaSize = sizeof(PV_AudioEncodeParam);
				uint refCounterSize = oscl_mem_aligned_size(sizeof(OsclRefCounterDA));
				uint32 totalWavParaSize = refCounterSize + wavParaSize;

				uint8 *memBufferWavHeader = NULL;
				memBufferWavHeader = (uint8*)iAlloc.allocate(totalWavParaSize);
				oscl_memset(memBufferWavHeader, 0, totalWavParaSize);

				OsclMemoryFragment wavParam;
				OsclRefCounter* refCounterWavHeader = new(memBufferWavHeader) OsclRefCounterDA(memBufferWavHeader,	(OsclDestructDealloc*)&iAlloc);

				memBufferWavHeader += refCounterSize;
				wavParam.ptr = memBufferWavHeader;

				// place wav header here
				oscl_memcpy(wavParam.ptr, (OsclAny*)&iAudioEncodeParam, wavParaSize);
				wavParam.len = wavParaSize;

				//create a OsclRefCounterMemFrag using header
				OsclRefCounterMemFrag wavRCParam;
				wavRCParam = OsclRefCounterMemFrag(wavParam, refCounterWavHeader, wavParaSize);


				mediaDataOut->setFormatSpecificInfo(wavRCParam);
            }
...
void PVMFOMXEncNode::DoPrepare(PVMFOMXEncNodeCommand& aCmd)
{
    ...
    switch (iInterfaceState)
    {
        case EPVMFNodeInitialized:
        {
            ...
            // added by HP Cheng
            else if (iOutFormat == PVMF_MIME_ADPCM)
            {
                    Role = (OMX_STRING)"audio_encoder.wav";
                    iNodeTypeId = LOG_ID_AUDIO_WAV;
            }
...
PVMFStatus PVMFOMXEncNode::SetCodecType(PVMFFormatType aCodec)
{
    ...
    else if (aCodec == PVMF_MIME_ADPCM)		// Added by HP Cheng
    {
        iOutFormat = aCodec;
    }
...
uint32 PVMFOMXEncNode::ConvertOMXTicksIntoTimestamp(const OMX_TICKS &src)
{
    ...
    LOGV("ConvertOMXTicksIntoTimestamp value:%lld, current_ts=%d",value,current_ts);
