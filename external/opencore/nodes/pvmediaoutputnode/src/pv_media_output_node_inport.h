...
#define THRESHOLD_FOR_DROPPED_VIDEO_FRAMES_TIMES 5000
        ...
        , public PVMFMediaClockNotificationsObs
{
    ...
    private:
        ...
        //<-- Honda modify
        void SendMediaData(uint32 aDelta);
        //void SendMediaData();
        //-->
    ...
    private:
        uint32 iConsecutiveFramesDroppedTime;
        
        // add by Ryan for av sync
        uint32 mCompensateClock;

        uint32 frame_count;
        uint32 late_frame_count;
        uint32 clock_msec_start;
        uint32 clock_sec32_start;
        uint32 lock_TS;
        
        uint32 m_CurOutDataTs;
        uint32 m_TooFastFrameCount;
        uint32 m_DropFrameTimeOffset;

        uint8* GetFrameAddr();

        bool bLogFrameInfo;
        bool bDisableCheckTimeStamp;
        uint32 iCheckFPSInterval;
        //<-- Honda add
        uint32 iLateCompensate;
        uint32 m_startTimeStamp;
        //-->

        // <--- Morris Yang ALPS00122159
    	uint32 iAudioSamplingRate;
        uint32 iAudioChannels;
        uint32 iAudioBitsPerSample;
        uint32 iPcmOutputBufferSize;
        uint32 iPcmOutputBufferDurationInMs;
        uint32 iMediaToSkipInBytes;
        bool bRecalculateAudioSkipSize;
        // --->
};
