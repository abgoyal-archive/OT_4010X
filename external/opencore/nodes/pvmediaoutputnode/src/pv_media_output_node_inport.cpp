...
#define PVMF_MOPORT_LOGWARNING(x) PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_WARNING, x);
...
PVMediaOutputNodePort::PVMediaOutputNodePort(PVMediaOutputNode* aNode)
        ...
        , lock_TS(0)
        , m_CurOutDataTs(0)
        , m_TooFastFrameCount(0)
        , m_DropFrameTimeOffset(0)
        , bLogFrameInfo(false)
        , bDisableCheckTimeStamp(false)
{
    bShowLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_outputnodeinputport");
    bLogFrameInfo = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_frame_info");
    bDisableCheckTimeStamp = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "disable_check_timestamp");
    iCheckFPSInterval = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_fps_interval");
    if (iCheckFPSInterval == 0)
    {
        iCheckFPSInterval = CHECK_INTERVAL;
    }
    ...
    iConsecutiveFramesDroppedTime = 0;
...
// <--- Morris Yang ALPS00122159
    iAudioSamplingRate = 44100;
    iAudioChannels = 2;
    iAudioBitsPerSample = 16;
    iPcmOutputBufferSize = 0;
    iPcmOutputBufferDurationInMs = 0;
    iMediaToSkipInBytes = 0;
    bRecalculateAudioSkipSize = false;
// --->
...
OSCL_EXPORT_REF PVMFStatus PVMediaOutputNodePort::ClearMsgQueues()
{
    ...
    if (iNode != NULL)
    {
        if (iNode->iMIOControl != NULL)
        {
            iNode->iMIOControl->ClearBuffer();
        }
    }
...
//for PvmiMediaTransfer
{
    ...
    if (&iWriteAsyncEOSContext == (uint32*)aContext)
    {
        ...
        else
        {
            ...
            LOGV("writeComplete For EOS - Fmt=%s", iSinkFormatString.get_str());
...
#if 1        
            if (iClock->getAVSyncCtrl() != NULL)
            {
                iClock->getAVSyncCtrl()->UnlockSyncMutex();
            }
#endif
    ...
    else
    {
        ...
        {
            PVMF_MOPORT_LOGWARNING((0, "PVMediaOutputNodePort::writeComplete - CmdId Not Found - Fmt=%s, FIdx=%d, ClnUpQSize=%d",
                                  iSinkFormatString.get_str(),
                                  iFragIndex,
                                  iCleanupQueue.size()));
        }
...
{
    LOGDATAPATH((0, "PVMediaOutputNodePort::SendData called"));
    ...
    if (iCurrentMediaMsg->getFormatID() == PVMF_MEDIA_CMD_EOS_FORMAT_ID)
    {
        ...
        else if (iFrameStepMode == false)
        {
            ...
            {
                LOGV("video[%d] EOS early, timeStamp= %d", iCurrentMediaMsg->getSeqNum(), iCurrentMediaMsg->getTimestamp());
	...
	// ALPS00017094
                        ...
                        //return;
				    	PVMFStatus status_1 =
	                    iClockNotificationsInf->SetCallbackDeltaTime(1000, //delta time in clock when callBack should be called
                                toleranceWndForCallback,
                                this, //observer object to be called on timeout
                                false, //no threadLock
                                NULL, //no context
                                iDelayEarlyFrameCallBkId); //ID used to identify the timer for cancellation
	                   	if (PVMFSuccess != status_1)
	                   	{
							PVMF_MOPORT_LOGERROR((0, "PVMediaOutputNodePort::SendData(1) - Could not set callback notification for send data %d delta %u", status, delta));
                            iNode->ReportErrorEvent(PVMFErrCorrupt, NULL, PVMFMoutNodeErr_Unexpected);
                            return;
	                   	}
	                   	else
    ...
    else if (iCurrentMediaMsg->getFormatID() < PVMF_MEDIA_CMD_FORMAT_IDS_START)
    {
        if (oActiveMediaOutputComp)
        {
            ...
            if (iClock->getAVSyncCtrl() != NULL)
            {
                LOGV("***** Audio Data In, SeqNum = %d, TS = %d *****", iCurrentMediaMsg->getSeqNum(), iCurrentMediaMsg->getTimestamp());
                bool ret = iClock->getAVSyncCtrl()->CheckBufIn(PVMFAVSyncbase::AVSYNC_TAG_AUDIO);                
                if (iSinkFormat.isAudio() && ret)
                {
                    iClock->getAVSyncCtrl()->SetAudioInTs(iCurrentMediaMsg->getTimestamp());
                    LOGV("***** Set NPT In, SeqNum = %d, TS = %d *****", iCurrentMediaMsg->getSeqNum(), iCurrentMediaMsg->getTimestamp());
                }
            }
#endif            
            SendMediaData(0);
        ...
        else if (iFrameStepMode == false)
        {
            ...
            {
                LOGV("video[%d] onTime, timeStamp= %d", iCurrentMediaMsg->getSeqNum(), iCurrentMediaMsg->getTimestamp());
                // Ryan Lin !!!                
                if (iSinkFormat.isVideo())
                {
                    LOGV("***** Video Data In, TS = %d *****", iCurrentMediaMsg->getTimestamp());
                    uint32 _iSeqNum = iCurrentMediaMsg->getSeqNum();
                    OsclProfile::Push(OsclProfile::iSendMediaDataInTime, _iSeqNum);
                }
                SendMediaData(delta);
            }
            else if (status == PVMF_MEDIAOUTPUTNODEPORT_MEDIA_LATE)
            {
                LOGV("video[%d] late, timeStamp= %d", iCurrentMediaMsg->getSeqNum(), iCurrentMediaMsg->getTimestamp());
                iCurrentMediaMsg.Unbind();
                iFragIndex = 0;
            }
            else if (status == PVMF_MEDIAOUTPUTNODEPORT_MEDIA_EARLY)
            {
                LOGV("video[%d] early, timeStamp= %d", iCurrentMediaMsg->getSeqNum(), iCurrentMediaMsg->getTimestamp());
                OSCL_ASSERT(false == oClockCallBackPending);
                //stop processing input, since we are not done with the current media msg
                oProcessIncomingMessage = false;
                oClockCallBackPending = false;
	...
	//<-- Honda
	// ALPS00017026, ALPS00017029, ALPS00017100
	                    //iNode->ReportErrorEvent(PVMFErrCorrupt, NULL, PVMFMoutNodeErr_Unexpected);
	                    //return;
				    	PVMFStatus status_1 =
	                    iClockNotificationsInf->SetCallbackDeltaTime(1000, //delta time in clock when callBack should be called
	                   	...
	                   	if (PVMFSuccess != status_1)
	                   	{
							PVMF_MOPORT_LOGERROR((0, "PVMediaOutputNodePort::SendData(2) - Could not set callback notification for early frame %d delta %u", status, delta));
	                   	    iNode->ReportErrorEvent(PVMFErrCorrupt, NULL, PVMFMoutNodeErr_Unexpected);
	                       	return;
	                   	}
	                   	else
...
////////////////////////////////////////////////////////////////////////////
void PVMediaOutputNodePort::SendMediaData(uint32 aDelta)
//send media data to the MIO componenent.
{
    ...
    for (uint32 fragindex = iFragIndex; fragindex < mediaData->getNumFragments();)
    {
        ...
        // <--- Morris Yang ALPS00122159
        if ((fragindex == 0) && iSinkFormat.isAudio() && bRecalculateAudioSkipSize && (iClock->getAVSyncCtrl() != NULL) && (iClock->getAVSyncCtrl()->AccurateMediaSkipControlForAudio()))
        {
            // double check msg timestamp
            uint32 _media_to_skip_ms = iSkipTimestamp - mediaData->getTimestamp();
	          iMediaToSkipInBytes = 0;
	          if (_media_to_skip_ms < iPcmOutputBufferDurationInMs)
 	          {
 	              iMediaToSkipInBytes = (uint32)((double)(iAudioSamplingRate * iAudioChannels * (iAudioBitsPerSample/8)) * _media_to_skip_ms / 1000);
 	              iMediaToSkipInBytes = ((iMediaToSkipInBytes + 3) & ~3);
 	              LOGE ("SendMediaData frag=%u, fseq=%u, ts=%u, iSkipTimestamp=%u, MemFragSize=%u, iMediaToSkipInBytes=%u", fragindex, mediaxferhdr.seq_num, mediaxferhdr.timestamp, iSkipTimestamp, frag.getMemFragSize(), iMediaToSkipInBytes);
 	          }
        }
        // --->
        ...
        //<-- Honda add
        mediaxferhdr.aDelat = aDelta;
        //-->
        err = WriteDataToMIO(cmdId, mediaxferhdr, frag);

        // <--- Morris Yang ALPS00122159
        if ((fragindex == 0) && iSinkFormat.isAudio() && bRecalculateAudioSkipSize)
        {
            // reset recalc flag
            bRecalculateAudioSkipSize = false;
        }
...
uint8* PVMediaOutputNodePort::GetFrameAddr()
{
    // get data pointer for drop mm queue of q-table
    PVMFSharedMediaDataPtr mediaData;
    convertToPVMFMediaData(mediaData, iCurrentMediaMsg);
    ...
    if (mediaData != NULL)
    {
        ...
        if (mediaDataImpl != NULL)
        {
            for (uint32 fragindex = iFragIndex; fragindex < mediaData->getNumFragments();fragindex++)
            {
                OsclRefCounterMemFrag frag;        
                mediaData->getMediaFragment(fragindex, frag);
                return (uint8*)frag.getMemFragPtr();
            }
        }
    }
...
// Ryan Lin !!
#ifndef _WIN32
#define CHECK_TIME_STAMP 1
#define SHOW_VIDEO_FPS
...
#define SYNC_VIDEO_TO_AUDIO 1
#define THE_FRAME_RATE_LIMITATION 33
#define DECREASE_LATENCY 10
#define _DROP_FRAME_TIME_OFFSET_ 0
#define FORCE_DISPLAY_THRESHOLD  4
#define LATE_COMPENSATE 50
#define LATE_COMPENSATE_DECREASE 5
...
PVMediaOutputNodePort::CheckMediaTimeStamp(uint32& aDelta)
{
    ...
    if (iClock != NULL)
    {
        uint32 clock_msec32 = 0;
        bool overflowFlag = false;

        // Ryan Lin!!
        uint32 iAudioInTs = 0;

//        if (clock_msec_start == 0)
//        {
//            clock_msec_start = clock_msec32;
//            clock_sec32_start = aTimeStamp / 1000;
//        }
...
//      iClock->GetCurrentTime32(clock_msec32, overflowFlag, PVMF_MEDIA_CLOCK_MSEC);
        ...
        if (iSinkFormat.isVideo())
        {
            if (iCurrentMediaMsg->getSeqNum() == 0)
            {
                LOGV("***** Reset Playback Clock *****");
                frame_count = 0;
                late_frame_count = 0;
                //clock_msec_start = clock_msec32;
                //clock_sec32_start = aTimeStamp / 1000; 
                m_CurOutDataTs = aTimeStamp;
                m_TooFastFrameCount = 0;
                m_DropFrameTimeOffset = 0;
                mCompensateClock = 0;
                lock_TS = 0;
...
//<-- Honda
// ALPS00017026, ALPS00017029, ALPS00017100
// Remove this for CR: ALPS00006116
//				if (iClock->getAVSyncCtrl() != NULL)
//				{
//					uint32 u4AudioFirstTs = iClock->getAVSyncCtrl()->GetAudioFirstTs();
//					if (aTimeStamp > u4AudioFirstTs)
//					{
//						iClock->adjustStartTimebaseTickValue(aTimeStamp - u4AudioFirstTs);
//					}
//				}
//<--
            }
        ...
        if (clock_msec_start == 0)
        {
            clock_msec_start = clock_msec32;
            clock_sec32_start = aTimeStamp / 1000;
            m_startTimeStamp = aTimeStamp;
        }
#if SYNC_VIDEO_TO_AUDIO        
		if (iSinkFormat.isVideo())
		{
			if (iClock->getAVSyncCtrl() != NULL)
			{
		        if (iClock->getAVSyncCtrl()->CheckBufIn(PVMFAVSyncbase::AVSYNC_TAG_VIDEO))
		        {
		            iAudioInTs = iClock->getAVSyncCtrl()->GetAudioInTs();
		            if (iClock->getAVSyncCtrl()->isMutexLock())
		            {
		                lock_TS = iAudioInTs;
		            }
		            frame_count = 0;
		            if (clock_msec32 > iAudioInTs)
		            {
		                mCompensateClock = clock_msec32 - iAudioInTs;
		            }
		            LOGD("***** audio in ts = %d *****", iAudioInTs);
		            LOGD("***** video in ts = %d *****", aTimeStamp);
		            LOGD("***** playback clock = %d *****", clock_msec32);
		            LOGD("***** compensate playback time = %d *****", mCompensateClock);
		        }
		    }
		}
#endif 
        ...
        // Ryan Lin !!
        // add for av sync
        // drop the time stamp late than lock time stamp
        if (aTimeStamp < lock_TS)
        {
            aDelta = lock_TS - aTimeStamp;

            LOGV("drop not sync frame TS: %d", aTimeStamp);

            //[20100528] Jackal
            //Mark this because the mechanism of mm_queue is changed!!
            //uint8* _ptr = GetFrameAddr();
            //LOGV("PopFrameByAddr - 0x%X", _ptr);
            //Oscl_MM_Queue::PopFrameByAddr(_ptr);

            return PVMF_MEDIAOUTPUTNODEPORT_MEDIA_LATE;
        }
        
        if (mCompensateClock != 0)
        {
            LOGV("mCompensateClock = %d", mCompensateClock);
            aTimeStamp += mCompensateClock;
            if (mCompensateClock >= DECREASE_LATENCY)
            {
                mCompensateClock -= DECREASE_LATENCY;
            }
            else
            {
                mCompensateClock = 0;
...
#if 1    
        // check for high frame rate video
        if (bDisableCheckTimeStamp == false)
        {
            uint32 _threshold = (uint32)(1000 / THE_FRAME_RATE_LIMITATION);
            uint32 _delta = aTimeStamp - m_CurOutDataTs;
            if (((_delta < _threshold && m_CurOutDataTs != aTimeStamp) ||
                (_delta == _threshold && m_TooFastFrameCount != 0)) & 
                (iConsecutiveFramesDropped < FORCE_DISPLAY_THRESHOLD))
            {
	#if _DROP_FRAME_TIME_OFFSET_
				uint32 _delta_1 = _delta + m_DropFrameTimeOffset;
				if (_delta_1 < _threshold)
				{
	#endif
	...
	#if _DROP_FRAME_TIME_OFFSET_
	                m_DropFrameTimeOffset += _threshold - _delta;
	#endif
	                aDelta = _delta;

	                LOGV("drop too closed frame: %d", aTimeStamp);
                    ...
                    if (iConsecutiveFramesDroppedTime == 0)
                    {
                        iConsecutiveFramesDroppedTime = aTimeStamp;
                    }
	                return PVMF_MEDIAOUTPUTNODEPORT_MEDIA_LATE;
	#if _DROP_FRAME_TIME_OFFSET_
				}
        ...
        uint32 ts_adjforlatemargin = aTimeStamp + iLateMargin + iLateCompensate;

        uint32 cur_clock_sec = aTimeStamp / 1000;   
        if (cur_clock_sec - clock_sec32_start >= iCheckFPSInterval)
        {
            float fps = 1000.0 * frame_count / (clock_msec32 - clock_msec_start);
            
#ifdef SHOW_VIDEO_FPS 
            char* _str = NULL;
            if (iSinkFormat.isVideo())
            {
                _str = "Video";
            }
            else if (iSinkFormat.isAudio())
            {
                _str = "Audio";
            }
            ...
            if (frame_count != 0)
            {
                if (_str != NULL)
                {
                    LOGD("(%s) profiling: ts = %d sec, FPS = %f", _str, (uint32)(aTimeStamp / 1000), fps);
                }
            }
            //LOGD("frame_count: %d, start: %d, end: %d", frame_count, clock_msec_start, clock_msec32);
            if (late_frame_count != 0)
            {
                LOGE("(%s) drop frame info - time: %d s, count: %d ", _str, cur_clock_sec, late_frame_count);
                if (cur_clock_sec > 0 && (cur_clock_sec % 30) == 0)
                {
                    //late_frame_count = 0;
                }
            }
            ...
            frame_count = 0;
            clock_sec32_start = cur_clock_sec;
            clock_msec_start = clock_msec32;
        }
...
//<-- Honda
        if (iSinkFormat.isVideo())
        {
            if (0 == iCurrentMediaMsg->getSeqNum())
            {
            	if ((clock_adjforearlymargin - aTimeStamp) > WRAP_THRESHOLD && bDisableCheckTimeStamp == false)
            	{
            		// Avoid Seq=0, ((clock_adjforearlymargin - aTimeStamp) < 0)
            		// This will cause always PVMF_MEDIAOUTPUTNODEPORT_MEDIA_EARLY
            		// ALPS00017022, ALPS00017023, ALPS00017024, ALPS00017033,
            		// ALPS00017054, ALPS00017066, ALPS00017074, ALPS00017080,
            		// ALPS00017096, ALPS00017099
            		clock_adjforearlymargin = aTimeStamp;
            	}
            }
            bool bForceDisplay = false;
            if (((aTimeStamp - m_startTimeStamp) <= 2000) || (iConsecutiveFramesDropped >= FORCE_DISPLAY_THRESHOLD))
            {
                bForceDisplay = true;
            }
            if (((ts_adjforlatemargin - clock_msec32) > WRAP_THRESHOLD) && bForceDisplay)
            {
                ts_adjforlatemargin = clock_msec32;
            }
        }
//-->
        if ((clock_adjforearlymargin - aTimeStamp) > WRAP_THRESHOLD && bDisableCheckTimeStamp == false)
        {
            ...
            if (iClockRate > 1)
            {
                ...
                LOG_INFO("Early - Fmt=%s, Seq=%d, Ts=%d, Clock=%d, DeltaInMS=%d, ClkRate=%d, DeltaInClkUnits=%d",
                                         iSinkFormatString.get_str(),
                                         iCurrentMediaMsg->getSeqNum(),
                                         aTimeStamp,
                                         clock_msec32,
            ...
            else
            {
                ...
                PVMF_MOPORT_LOGDATAPATH((0, "PVMediaOutputNodePort::CheckMediaTimeStamp - Early - Fmt=%s, Seq=%d, Ts=%d, Clock=%d, Delta=%d",
                                         ...
                                         clock_msec32,
                                         aDelta));
                LOG_INFO("Early - Fmt=%s, Seq=%d, Ts=%d, Clock=%d, Delta=%d",
                                         iSinkFormatString.get_str(),
                                         iCurrentMediaMsg->getSeqNum(),
                                         aTimeStamp,
                                         clock_msec32,
            ...
            if (iLateCompensate > LATE_COMPENSATE_DECREASE)
            {
                iLateCompensate -= LATE_COMPENSATE_DECREASE;
            }
            else
            {
                iLateCompensate = 0;
        ...
        else if ((ts_adjforlatemargin - clock_msec32) > WRAP_THRESHOLD)
        {
            ...
            PVMF_MOPORT_LOGDATAPATH((0, "PVMediaOutputNodePort::CheckMediaTimeStamp - Late - Fmt=%s, Seq=%d, Ts=%d, Clock=%d, Delta=%d",
                                     ...
                                     clock_msec32,
                                     aDelta));
            LOG_INFO("Late - Fmt=%s, Seq=%d, Ts=%d, Clock=%d, Delta=%d",
                                     iSinkFormatString.get_str(),
                                     iCurrentMediaMsg->getSeqNum(),
                                     aTimeStamp,
                                     clock_msec32,
...
#else
                if ((aTimeStamp - iConsecutiveFramesDroppedTime >= THRESHOLD_FOR_DROPPED_VIDEO_FRAMES_TIMES) && (iLateFrameEventSent == false))
#endif
                {
                    iLateFrameEventSent = true;
                    LOGE("report consecutive drop frames");
                    iNode->ReportInfoEvent(PVMFInfoVideoTrackFallingBehind, (OsclAny*)NULL);
                }
#endif
            }
            //<-- Honda add
            iLateCompensate = LATE_COMPENSATE;
            //-->
            return PVMF_MEDIAOUTPUTNODEPORT_MEDIA_LATE;
        }
        else
        {
            ...
            PVMF_MOPORT_LOGDATAPATH((0, "PVMediaOutputNodePort::CheckMediaTimeStamp - OnTime - Fmt=%s, Seq=%d, Ts=%d, Clock=%d",
                                     ...
                                     aTimeStamp,
                                     clock_msec32));
            LOG_INFO("OnTime - Fmt=%s, Seq=%d, Ts=%d, Clock=%d",
                                     iSinkFormatString.get_str(),
                                     iCurrentMediaMsg->getSeqNum(),
                                     aTimeStamp,
                                     clock_msec32);
            ...
            aDelta = ((aTimeStamp > clock_msec32) ? (aTimeStamp - clock_msec32) : 0);
...
// ALPS00017026, ALPS00017029, ALPS00017100
           	if (iSinkFormat.isAudio())
            {
                if (iClock->getAVSyncCtrl() != NULL)
                {
            		iClock->getAVSyncCtrl()->SetAudioFirstTs(iCurrentMediaMsg->getTimestamp());
            	}
           	}
#if _DROP_FRAME_TIME_OFFSET_
           	else
           	{   // Video frame, reset m_DropFrameTimeOffset
           	    m_DropFrameTimeOffset = 0;
           	}
#endif
//-->
...
#if 1                        
                        uint32 _iSeqNum = iCurrentMediaMsg->getSeqNum();
                        OsclProfile::Push(OsclProfile::iReceiveYUVDataInTime, _iSeqNum);
#endif                        
                    }
                }
            }
        ...
        case PVMF_MEDIA_UNCOMPRESSED_AUDIO:
        {
            ...
            if (pv_mime_strcmp(aParameters->key, PVMF_FORMAT_SPECIFIC_INFO_KEY) == 0)
            {
              ...
              // <--- Morris Yang ALPS00122159
                ...
                iAudioChannels = pcm16Info->desiredChannels;
                iAudioBitsPerSample = pcm16Info->bitsPerSample;
                iPcmOutputBufferSize = pcm16Info->buffer_size;
              // --->
...
bool PVMediaOutputNodePort::DataToSkip(PVMFSharedMediaMsgPtr& aMsg)
{
    ...
    if (aMsg->getFormatID() != PVMF_MEDIA_CMD_EOS_FORMAT_ID)
    {
        ...
        if (iSendStartOfDataEvent == true)
        {
         ...
         // <--- Morris Yang ALPS00122159
                if (iSinkFormat.isAudio() && (iClock->getAVSyncCtrl() != NULL) && (iClock->getAVSyncCtrl()->AccurateMediaSkipControlForAudio()))
                {
                    // <--- Donglei ALPS00134148
                    // calculate PCM data duration in each output buffer
                    //iPcmOutputBufferDurationInMs = (uint32)((double)(iPcmOutputBufferSize*1000)/(iAudioSamplingRate * iAudioChannels * (iAudioBitsPerSample/8)));
		      		
		      		uint32 fragindex = 0;
		            uint32 memFragSize = 0;
                    ...
                    OsclRefCounterMemFrag frag;
                    mediaData->getMediaFragment(fragindex, frag);
                    memFragSize = frag.getMemFragSize();
                    
                    // calculate PCM data duration in each output buffer
                    iPcmOutputBufferDurationInMs = (uint32)((double)(memFragSize*1000)/(iAudioSamplingRate * iAudioChannels * (iAudioBitsPerSample/8)));
		      		// ---> 
                    // see if SkipMediaTs is located in this msg, if yes, don't skip it. left it to SendMediaData
                    if ((iSkipTimestamp - aMsg->getTimestamp()) < iPcmOutputBufferDurationInMs)
                    {
                        bRecalculateAudioSkipSize = true;
                        LOGE ("PVMediaOutputNodePort::DataToSkip left to SendMediaData aMsg->getTimestamp()=%u, iSkipTimestamp=%u, isAudio=%d, iPcmOutputBufferDurationInMs=%u, iPcmOutputBufferSize=%u", aMsg->getTimestamp(), iSkipTimestamp, iPortFormat.isAudio(), iPcmOutputBufferDurationInMs, iPcmOutputBufferSize);            		    	
                        return false;
                    }
                }
         // --->
...
int32 PVMediaOutputNodePort::WriteDataToMIO(int32 &aCmdId, PvmiMediaXferHeader &aMediaxferhdr, OsclRefCounterMemFrag &aFrag)
{
    int32 leavecode = OsclErrNone;
	 ...
	 // <--- Morris Yang ALPS00122159
	 if (bRecalculateAudioSkipSize && iSinkFormat.isAudio())
	 {
		LOGE ("WriteDataToMIO !!! (iMediaToSkipInBytes=%u)", iMediaToSkipInBytes);
    ...
    OSCL_TRY_NO_TLS(iOsclErrorTrapImp, leavecode,
                    aCmdId = iMediaTransfer->writeAsync(PVMI_MEDIAXFER_FMT_TYPE_DATA,  /*format_type*/
                                                        ...
                                                        (uint8*)aFrag.getMemFragPtr() + iMediaToSkipInBytes,
                                                        aFrag.getMemFragSize() - iMediaToSkipInBytes,
                                                        aMediaxferhdr,
