...
PVMediaOutputNode::~PVMediaOutputNode()
{
    ...
    if (iMIOControl)
    {
        iMIOControl->SetAVSyncCtrl(NULL);
...
OSCL_EXPORT_REF void PVMediaOutputNode::ReportErrorEvent(PVMFEventType aEventType, PVInterface* aExtMsg)
{
    ...
    // <--- Demon Deng, ALPS00029598, report this to engine
    if (aEventType == PVMFErrResourceConfiguration)
        PVMFNodeInterface::ReportErrorEvent(aEventType);
    // --->
}
OSCL_EXPORT_REF void PVMediaOutputNode::ReportInfoEvent(PVMFEventType aEventType, PVInterface* aExtMsg)
{
    ...
    // Honda add
    if (PVMFInfoSinkThreadCreate == aEventType)
    {
        PVMFNodeInterface::ReportInfoEvent(aEventType, (OsclAny*)NULL);
    }
}
...
{
    bShowLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_outputnode");
}
...
PVMFStatus PVMediaOutputNode::ResetNodeParam()
{
    LOGV("***** ResetNodeParam() *****");    
    iMIOControl->ResetMIO();
    return PVMFSuccess;
}

PVMFStatus PVMediaOutputNode::ClearBuffer()
{
    LOGV("***** ClearBuffer() *****");
    uint32 i;
    for (i = 0; i < iInPortVector.size(); i++)
    ...
    iMIOControl->ClearBuffer();
    
    return PVMFSuccess;
}

void PVMediaOutputNode::SetAVSyncCtrl(PVMFAVSyncbase* aAVSyncCtrl)
{
    if (iMIOControl)
    {
        iMIOControl->SetAVSyncCtrl(aAVSyncCtrl);
    }
}
