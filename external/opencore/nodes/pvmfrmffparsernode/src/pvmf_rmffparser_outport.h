 ...
 * Filename:
 * ---------
 *   pvmf_rmffparser_node_outport.h
 *
 * Project:
 * --------
 *   YUSU
...
#define PVMF_RMFFPARSER_OUTPORT_H_INCLUDED
...
#define PVMF_RMFFPARSERNODE_LOGERROR(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iLogger,PVLOGMSG_ERR,m);
#define PVMF_RMFFPARSERNODE_LOGWARNING(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iLogger,PVLOGMSG_WARNING,m);
#define PVMF_RMFFPARSERNODE_LOGINFOHI(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG,iLogger,PVLOGMSG_INFO,m);
#define PVMF_RMFFPARSERNODE_LOGINFOMED(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG,iLogger,PVLOGMSG_INFO,m);
#define PVMF_RMFFPARSERNODE_LOGINFOLOW(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG,iLogger,PVLOGMSG_INFO,m);
#define PVMF_RMFFPARSERNODE_LOGINFO(m) PVMF_RMFFPARSERNODE_LOGINFOMED(m)
#define PVMF_RMFFPARSERNODE_LOGDATATRAFFIC(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iDataPathLogger,PVLOGMSG_INFO,m);
#define PVMF_RMFFPARSERNODE_LOGDATATRAFFIC_AVC(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iAVCDataPathLogger,PVLOGMSG_INFO,m);
#define PVMF_RMFFPARSERNODE_LOGCLOCK(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iClockLogger,PVLOGMSG_INFO,m);
#define PVMF_RMFFPARSERNODE_LOGBIN(iPortLogger, m) PVLOGGER_LOGBIN(PVLOGMSG_INST_LLDBG, iPortLogger, PVLOGMSG_ERR, m);
#define PVMF_RMFFPARSERNODE_LOGDIAGNOSTICS(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_PROF,iDiagnosticsLogger,PVLOGMSG_INFO,m);
...
class PVRMFFParserNodeTrackPortInfo : public OsclMemPoolFixedChunkAllocatorObserver,
        public OsclMemPoolResizableAllocatorObserver
{
    public:
        ...
        PVRMFFParserNodeTrackPortInfo()
        {
            ...
            iLogger = PVLogger::GetLoggerObject("datapath.sourcenode.mtkrawsource");
            //PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0,"PVRMFFParserNodeTrackPortInfo::PVRMFFParserNodeTrackPortInfo"));

        }

        PVRMFFParserNodeTrackPortInfo(const PVRMFFParserNodeTrackPortInfo& aSrc) :
        ...
        virtual ~PVRMFFParserNodeTrackPortInfo()
        {
...
class PVRMFFParserNodeTrackOMA2DRMInfo
{
    public:
        PVRMFFParserNodeTrackOMA2DRMInfo()
        {
            iTrackId = 0;
            iDRMInfoSize = 0;
            iDRMInfo = NULL;
        ...
        PVRMFFParserNodeTrackOMA2DRMInfo(const PVRMFFParserNodeTrackOMA2DRMInfo& aSrc)
        {
            Copy(aSrc);
        };


        virtual ~PVRMFFParserNodeTrackOMA2DRMInfo()
        {
            if (iDRMInfo != NULL)
            {
        ...
        void Copy(const PVRMFFParserNodeTrackOMA2DRMInfo& aSrc)
        {
            iTrackId = aSrc.iTrackId;
...
class PVMFRMFFParserNode;

class PVMFRMFFParserOutPort : public PvmfPortBaseImpl,
        public PvmiCapabilityAndConfigPortFormatImpl
{
    public:
        PVMFRMFFParserOutPort(int32 aTag, PVMFNodeInterface* aNode, const char*);
        PVMFRMFFParserOutPort(int32 aTag
    ...
    private:
        ...
        friend class PVMFRMFFParserNode;
        PVMFRMFFParserNode* iRMFFParserNode;
};

#endif // PVMF_RMFFPARSER_OUTPORT_H_INCLUDED
