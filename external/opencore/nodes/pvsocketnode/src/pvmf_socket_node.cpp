        ...
        uint32& aHigherPortNum, uint32& aStartPortNum)
{
    ...
    for (int maxNumOfBind = UDP_PORT_RANGE; maxNumOfBind >= 0; maxNumOfBind--)
    {
        ...
        LOGD ("AllocateConsecutivePorts RTP(%d), RTCP(%d), MIME=%s", lower_sock_config->iAddr.iLocalAdd.port, higher_sock_config->iAddr.iLocalAdd.port, lower_sock_config->iMime.get_cstr());
