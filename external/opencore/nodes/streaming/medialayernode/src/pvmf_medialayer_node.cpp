    ...
    if (msgFormatID > PVMF_MEDIA_CMD_FORMAT_IDS_START)
    {
        ...
        else if (msgFormatID == PVMF_MEDIA_CMD_EOS_FORMAT_ID)
        {
            ...
            for (uint i = 0; i < pinputPort->vCounterPorts.size(); i++)
            {
                ...
                // <--- Morris Yang 20100914 Set the EOS TS from JB EOS CMD msg (ALPS00125050, ALPS00125146, ALPS00125063)
                    poutPort->iContinuousTimeStamp = msgIn->getTimestamp();
                // --->
                    msgIn->setTimestamp(poutPort->iContinuousTimeStamp);
                    ...
                    {
                        LOGE ("PVMFMediaLayerNode::ProcessInputMsg_OneToN: Sending EOS - MimeType=%s, StreamId=%d, TS=%u", pinputPort->iMimeType.get_cstr(), msgIn->getStreamID(), msgIn->getTimestamp());
                    }
                }
            }
        }
