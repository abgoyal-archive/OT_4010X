...
#define LOG_TAG "RTSPUnicastNode"
#include <cutils/log.h>
#include <cutils/properties.h>
#include "m4v_config_parser.h"
#include "getactualaacconfig.h"
#define RTSP_IGNORE_AUDIO (1 << 0)
#define RTSP_IGNORE_VIDEO (1 << 1)
...
PVMFSMRTSPUnicastNode::PVMFSMRTSPUnicastNode(int32 aPriority): PVMFSMFSPBaseNode(aPriority)
{
    ...
    ResetNodeParams(false);
     ...
     property_get("rtsp.ignoreaudio", value, "0");
     bool _res = atoi(value);
     if (_res) _uDbgFlags |= RTSP_IGNORE_AUDIO;

     property_get("rtsp.ignorevideo", value, "0");
     _res = atoi(value);
     if (_res) _uDbgFlags |= RTSP_IGNORE_VIDEO;
...
// <--- Morris Yang ALPS00225213, ALPS00225214
    _ipFirewallPacketExchangeImpl = NULL;
// --->
}
...
// <--- Morris Yang 20101004 for OMA DM
        ...
        uint32 myport = (first % (iMaxUDPPort-iMinUDPPort-3)) + iMinUDPPort;  //start from iMinUDPPort
// --->
//FR331150-yong'an.zhong.hz porting begin
        //(6000,7000)
        char value[PROPERTY_VALUE_MAX];
        property_get("def.rtsp.port.limit",value,"0");
	if(strstr(value,","))
	{
           char * comma=oscl_strstr(value,",");
	   int isport=8192,ieport=60000; //opencore rtsp default port from 8192 to 2^16
	   if(comma){
	     ieport = strtol(comma+1, NULL, 10);
             comma = "\0";
	   }
           isport=strtol(value, NULL, 10);
           if(isport<1024)   //ensure port large than 1024
	   {
		myport = (first & 0x1FFF) + 0x2000;
	   }else{
		myport=( first % ( ieport - isport - 2 ) ) + isport;
	   }

           PVMF_SM_RTSP_LOGERROR((0, "customzie port number %d %d %d ",isport,ieport, (ieport-isport)));
	}else{
	   myport = (first % (iMaxUDPPort-iMinUDPPort-3)) + iMinUDPPort;  //start from iMinUDPPort
	}
        //FR331150-yong'an.zhong.hz porting end
...
bool PVMFSMRTSPUnicastNode::GraphConnect()
{
    if (iGraphConnectComplete == false)
    {
        ...
        for (uint32 i = 0; i < iTrackInfoVec.size(); i++)
        {
            ...
            LOGE ("GraphConnect - Track MimeType %s", trackInfo.iMimeType.get_cstr());
        ...
        if (pvInterface->queryInterface(sourceContextUuid, sourceDataContext))
        {
            if (sourceDataContext)
            {
                ...
                if (sourceDataContext->queryInterface(streamingContextUuid, streamingDataContext))
                {
                   ...
                   // <--- Morris Yang 20101004 for OMA DM
                        iMinUDPPort = sContext->iMinUDPPort;
                        iMaxUDPPort = sContext->iMaxUDPPort;
                   // --->
                    }
                }
    ...
    for (int32 i = 0; i < numTracks; i++)
    {
        ...
        for (uint32 j = 0; j < minfoVecLen; j++)
        {
            ...
            else
            {
             ...
             // <--- ALPS00225358 Reject error content (The b=AS is 0 and duration is over 138 hours)
		if ((mInfo->getBitrate() == 0) && (iMetaDataInfo->iSessionDuration >= 138 * 60 * 60 * 1000)) // 138 hours
		{
		    LOGD ("Reject error content!!! b=AS: 0 , duration=%lld", iMetaDataInfo->iSessionDuration);
		    return false;
		}
		// --->
            ...
            if ((configSize > 0) && (config != NULL))
            {
                ...
                if (false == IsSupportedFormat(mimeType, (uint8*)memfrag.ptr, memfrag.len, mInfo))
                	continue;    
            }
            else
            {
                // for H.263
                if (false == IsSupportedFormat(mimeType, NULL, 0, mInfo))
                	continue;
            }
...
// <--- Morris Yang ALPS00131435
    SetAllowRegOldPkt((0 == iRepositionRequestedStartNPTInMS));
// --->
...
// <--- Morris Yang ALPS00225213, ALPS00225214
              _ipFirewallPacketExchangeImpl = NULL;
    		PVMFSMFSPChildNodeContainer* _iJitterBufferNodeContainer = getChildNodeContainer(PVMF_SM_FSP_JITTER_BUFFER_NODE);
		if (_iJitterBufferNodeContainer)
		{
			PVMFJitterBufferNode * _jbNode = OSCL_STATIC_CAST(PVMFJitterBufferNode *, _iJitterBufferNodeContainer->iNode);
			_ipFirewallPacketExchangeImpl = _jbNode->GetFireWallPacketExchangerImpl();
			LOGD ("_ipFirewallPacketExchangeImpl=0x%x", _ipFirewallPacketExchangeImpl);
		}
// --->
...
PVMFStatus PVMFSMRTSPUnicastNode::InitMetaData()
{
    ...
    if (sessionInfo != NULL)
    {
        ...
        if (sessionRange->end_is_set == true)
        {
            ...
            LOGE ("Session duration: %u ms", duration);
		...
		if (iJitterBufferNodeContainer)
		{
			PVMFJitterBufferNode * jbNode = OSCL_STATIC_CAST(PVMFJitterBufferNode *, iJitterBufferNodeContainer->iNode);
			jbNode->SetSessionDuration(duration);
			jbNode->SetPlayerDriverUserData(this->GetPlayerDriverUserData());       
		}
        }
...
PVMFSMRTSPUnicastNode::SendSessionControlPrepareCompleteParams()
{
            ...
            (iSessionSourceInfo->_sessionType == PVMF_MIME_DATA_SOURCE_SDP_FILE))
    {
        ...
        // <--- Demon Deng ALPS00028091
        if (rtspServerInfo.iDisableFireWallPackets)
            jbExtIntf->DisableFireWallPackets();
        // --->
...
// <--- Morris Yang ALPS00225213, ALPS00225214 from jb misc to here
// prevent the firewall pkt send operation complete after ipFireWallPacketExchangerImpl (memory allocator) has been destroyed
// which will cause aSockConfig.iPendingSendMediaData.Unbind() exception
    LOGD ("PVMFSMRTSPUnicastNode::CleanUp() _ipFirewallPacketExchangeImpl=0x%x", _ipFirewallPacketExchangeImpl);
    if (_ipFirewallPacketExchangeImpl)
    {
        OSCL_DELETE(_ipFirewallPacketExchangeImpl);
        _ipFirewallPacketExchangeImpl = NULL;
    }
// --->
    iLogger = NULL;
...
bool PVMFSMRTSPUnicastNode::IsSupportedFormat(const char* mimeType, uint8* config, uint32 config_len, mediaInfo* mInfo)
{
    // <--- Demon Deng, support 6573 resolution
#ifdef MT6573_MFV_HW
 	 const int32 _MAX_VIDEO_W_H_ = 864;
     const int32 _MAX_VIDEO_DIM_ = 864*480;  // 864*480
#else
 	 const int32 _MAX_VIDEO_W_H_ = 720;
 	 const int32 _MAX_VIDEO_DIM_ = 345600;  // 720*480
#endif
    // --->
	 int bitrate = -1;      
        if (mInfo)
 	 	bitrate = mInfo->getBitrate();
        LOGE ("mimeType = %s, bitrate = %d", mimeType, bitrate);

    // <--- Morris Yang ALPS00225151 (temp solution)
    // For this URL, the RTP video packets from Helix server comes very slowly if we send RTCP packets.
    // The bitrate info in SDP is also invalid (0)
    // Disable the RTCP feedback temporarily to make the playback smooth
        if (0 == bitrate)
        {
    		PVMFSMFSPChildNodeContainer* iJitterBufferNodeContainer = getChildNodeContainer(PVMF_SM_FSP_JITTER_BUFFER_NODE);
		if (iJitterBufferNodeContainer)
		{
			PVMFJitterBufferNode * jbNode = OSCL_STATIC_CAST(PVMFJitterBufferNode *, iJitterBufferNodeContainer->iNode);
			jbNode->DisableRTCPFeedback();
		}
        }
    // --->
    
        if ((bitrate > 8*1000*1000) || 
        	((iMetaDataInfo->iSessionDuration <= 5000) && (bitrate > 2*1000*1000)) || 
        	((iMetaDataInfo->iSessionDuration <= 7000) && (bitrate > 6*1000*1000)))  // prevent jitter buffer full
        {
        	LOGE ("Unsupported media: bitrate is too high !!!");
        	return false;
        }

        // check H.263
        OSCL_StackString<32> h263(_STRLIT_CHAR("H263"));
        if ((oscl_strstr(mimeType, h263.get_cstr()) != NULL) && (mInfo != NULL))
        {
                Oscl_Vector<PayloadSpecificInfoTypeBase*, SDPParserAlloc> payloadVector;
                payloadVector = mInfo->getPayloadSpecificInfoVector();
                if (payloadVector.size() != 0)
                {
                    PayloadSpecificInfoTypeBase* payloadInfo = payloadVector[0];
                    H263PayloadSpecificInfoType* h263PayloadInfo =  OSCL_STATIC_CAST(H263PayloadSpecificInfoType*, payloadInfo);

                    int32 width, height;
                    width = h263PayloadInfo->getFrameWidth();
                    height = h263PayloadInfo->getFrameHeight();
                    LOGE ("H.263 streaming video, width = %d, height = %d", width, height);
                    if (width > _MAX_VIDEO_W_H_ || height > _MAX_VIDEO_W_H_ || ((width*height) >  _MAX_VIDEO_DIM_) || width <= 0 || height  <= 0)
	 	      {
		          LOGE ("Unsupported H.263 streaming video");
		          return false;
   		      }

                // <--- Morris Yang ALSP00225366 never play this link !!![
                    sessionDescription* sessionInfo = iSdpInfo->getSessionInfo();
                    if (false == h263PayloadInfo->getHasFrameSizeField())
                    {
                        if ((1275548844 == sessionInfo->getOSessionID()) && (1 == sessionInfo->getStreamCount()) && (iMetaDataInfo->iSessionDuration == 462000) && (width == 320) && (height == 240))
                        {
                            LOGE ("Unsupported error file");
                            return false;
                        }
                    }
                // --->
       ...
       // check AVC
	if ((oscl_strncmp(mimeType, PVMF_MIME_H264_VIDEO, oscl_strlen(PVMF_MIME_H264_VIDEO)) == 0) && (config != NULL) && (config_len > 0))
	{
		int32 width, height, display_width, display_height; // discard info
		int32 profile, level = 0;
		int16 status =  iGetAVCConfigInfo((uint8*)config,
                                                                     (int32)config_len,
					                                  (int32*) & width,
					                                  (int32*) & height,
					                                  (int32*) & display_width,
					                                  ...
					                                  (int32*) & profile,
					                                  (int32*) & level);
		LOGE ("H.264 streaming video width=%d, height=%d", display_width, display_height);

		if (status != 0)
		{
		       LOGE ("iGetAVCConfigInfo failed !!!");
			return false;
		}
		  
		if (display_width > _MAX_VIDEO_W_H_ || display_height > _MAX_VIDEO_W_H_ || ((display_width*display_height) >  _MAX_VIDEO_DIM_) || display_width <= 0 || display_height  <= 0)
		{
		      LOGE ("Unsupported H.264 streaming video");
		      return false;
		}
       }

       // check MPEG4
       if ((oscl_strncmp(mimeType, PVMF_MIME_M4V, oscl_strlen(PVMF_MIME_M4V)) == 0) && (config != NULL) && (config_len > 0))
       {
		int32 width, height, display_width, display_height; // discard info
		int16 status =  iGetM4VConfigInfo((uint8*)config,
                                                                     (int32)config_len,
					                                  (int32*) & width,
					                                  (int32*) & height,
					                                  (int32*) & display_width,
		...
		LOGE ("MPEG4 streaming video width=%d, height=%d", display_width, display_height); 

		if (status != 0)
		{
			LOGE ("iGetM4VConfigInfo failed !!!");
			return false;
		}
			  
		if (display_width > _MAX_VIDEO_W_H_ || display_height > _MAX_VIDEO_W_H_ || ((display_width*display_height) >  _MAX_VIDEO_DIM_) || display_width <= 0 || display_height  <= 0)
		{
			LOGE ("Unsupported MPEG4 streaming video");
			return false;
		}
	}

       // check MPEG4 audio
       if ((oscl_strncmp(mimeType, PVMF_MIME_3640, oscl_strlen(PVMF_MIME_3640)) == 0) && (config != NULL) && (config_len > 0))
       {
		uint32 num_channels = 0;
		int32 _config_len = config_len;
		uint8 audioObjectType;
		uint8 sampleRateIndex;
		GetActualAacConfig(config,
                               &audioObjectType,
                               &_config_len,
                               &sampleRateIndex,
                               &num_channels);

		//LOGE ("[Morris] audioObjectType = %d\n", audioObjectType);
		if ((audioObjectType != 2)    // AAC LC (Low Complexity) 
		&& (audioObjectType != 4)      // AAC LTP (Long Term Prediction)
		&& (audioObjectType != 5)      // SBR (Spectral Band Replication) 
		&& (audioObjectType != 29))   // PS (Parametric Stereo)          
		{
			// not supported profile
			LOGE ("Unsupported audio object type: (%d)\n", audioObjectType);
			return false;
		}
	}

        // check LATM
        if ((oscl_strncmp(mimeType, PVMF_MIME_LATM, oscl_strlen(PVMF_MIME_LATM)) == 0) && (config != NULL) && (config_len > 0))
        {
            bool _ret = true;
            PV_LATM_Parser* pLATMParser = OSCL_NEW(PV_LATM_Parser, ());
            int32 _config_len = config_len;
            uint8* pLATMConfigBuffer = pLATMParser->ParseStreamMuxConfig(config, (int32 *)&_config_len);
            if (pLATMConfigBuffer == NULL)
            {
                _ret = false;
            }
            ...
            if (pLATMParser)
            {
                OSCL_DELETE(pLATMParser);
            }

            if (pLATMConfigBuffer)
            {
                oscl_free(pLATMConfigBuffer);
            }
            LOGE ("Checking PVMF_MIME_LATM config %s!!!", (_ret ? "success" : "fail"));
            return _ret;
        }
...
bool PVMFSMRTSPUnicastNode::IgnoreTrack(const char* mimeType)
{
	if ((_uDbgFlags & RTSP_IGNORE_VIDEO) || (_uDbgFlags & RTSP_IGNORE_AUDIO))
	{
		if (_uDbgFlags & RTSP_IGNORE_VIDEO)
		{
			OSCL_StackString<32> h263(_STRLIT_CHAR("H263"));
			if ((oscl_strstr(mimeType, h263.get_cstr()) != NULL))
			{
			...
			if (oscl_strncmp(mimeType, PVMF_MIME_M4V, oscl_strlen(PVMF_MIME_M4V)) == 0)
			{
				return true;
			}

			if (oscl_strncmp(mimeType, PVMF_MIME_H264_VIDEO, oscl_strlen(PVMF_MIME_H264_VIDEO)) == 0)
			{
				return true;
			}
	    }

	    if (_uDbgFlags & RTSP_IGNORE_AUDIO)
	    {
			if (oscl_strncmp(mimeType, PVMF_MIME_3640, oscl_strlen(PVMF_MIME_3640)) == 0)
			{
				return true;
			}

			if (oscl_strncmp(mimeType, PVMF_MIME_AMR, oscl_strlen(PVMF_MIME_AMR)) == 0)
			{
				return true;
			}

			if (oscl_strncmp(mimeType, PVMF_MIME_LATM, oscl_strlen(PVMF_MIME_LATM)) == 0)
			{
				return true;
			}
	    }
...
// <--- Morris Yang ALPS00131435
void PVMFSMRTSPUnicastNode::SetAllowRegOldPkt(bool value)
{
    PVMFSMFSPChildNodeContainer* iJitterBufferNodeContainer = getChildNodeContainer(PVMF_SM_FSP_JITTER_BUFFER_NODE);
    if (iJitterBufferNodeContainer)
    {
        PVMFJitterBufferNode * jbNode = OSCL_STATIC_CAST(PVMFJitterBufferNode *, iJitterBufferNodeContainer->iNode);
        jbNode->SetAllowRegOldPkt(value);
    }
}
// --->
