...
class PVMFSMRTSPUnicastNode: public PVMFSMFSPBaseNode
{
    ...
    // <--- Morris Yang ALPS00131435
        void SetAllowRegOldPkt(bool value);
    // --->
    
        bool IsSupportedFormat(const char* mimeType, uint8* config, uint32 config_len, mediaInfo* mInfo);
        bool IgnoreTrack(const char* mimeType);
        uint32 _uDbgFlags;
    ...
    // <--- Morris Yang ALPS00225213, ALPS00225214
    PVFirewallPacketExchangeImpl* _ipFirewallPacketExchangeImpl;
    // --->
};
#endif
