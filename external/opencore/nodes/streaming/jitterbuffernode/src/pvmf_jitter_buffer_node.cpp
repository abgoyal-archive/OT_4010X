...
// <--- Morris Yang ALPS00224993
    iConsective_zero_buffering_count = 0;
// --->
        
// <--- Morris Yang ALPS00135691
    iIsBuffering = false;
    iLastDelayPercent = 0;
// --->
...
void PVMFJitterBufferNode::SetAllowRegOldPkt(bool value)
{
    //LOGD("PVMFJitterBufferNode::SetAllowRegOldPkt(%d)", value);
    Oscl_Vector<PVMFJitterBufferPortParams*, OsclMemAllocator>::iterator iter;
    for (iter = iPortParamsQueue.begin(); iter != iPortParamsQueue.end(); iter++)
    {
        PVMFJitterBufferPortParams* pPortParams = *iter;
        if (pPortParams && (pPortParams->ipJitterBuffer) && (PVMF_JITTER_BUFFER_PORT_TYPE_INPUT == pPortParams->iTag))
        {
            pPortParams->ipJitterBuffer->SetAllowRegOldPkt(value);
        }
    }
}
// --->
...
void PVMFJitterBufferNode::SetClientPlayBackClock(PVMFMediaClock* aClientClock)
{
    ...
    if (ipClientPlayBackClock)
    	ipClientPlayBackClock->SetRtspStreaming(true);
    // --->
}
...
{
    // <--- Demon Deng, ALPS00139342, start timer when pushing started
    // From now on, duration timer is align with our play time
    // instead of server time which is not good in GPRS or bad network
    ...
    switch (aPortParams->iTag)
    {
        case PVMF_JITTER_BUFFER_PORT_TYPE_INPUT:
        {
            ...
            switch (regStatus)
            {
                ...
                case PVMF_JB_REGISTER_MEDIA_MSG_ERROR_FATAL:
                {
                    ReportErrorEvent(PVMFErrProcessing, NULL);
                    return PVMFFailure;
                }
        ...
        // <--- Morris Yang ALPS00225347
            if (PVMFFailure == status)
            	ReportErrorEvent(PVMFErrProcessing, NULL);
        // --->
        }
        break;
...
void PVMFJitterBufferNode::DoRequestPort(PVMFJitterBufferNodeCommand& aCmd)
{
    ...
    if (pPortParams->iTag == PVMF_JITTER_BUFFER_PORT_TYPE_INPUT)
    {
        PVMFJitterBufferConstructParams jbConstructParams(ipJitterBufferMisc->GetEstimatedServerClock(), *ipClientPlayBackClock, pPortParams->iMimeType, *ipJitterBufferMisc->GetEventNotifier(), iDelayEstablished, iJitterDelayPercent, iJitterBufferState, this, port, ipJitterBufferMisc);
...
void PVMFJitterBufferNode::ProcessJBInfoEvent(PVMFAsyncEvent& aEvent)
{
    ...
    switch (aEvent.GetEventType())
    {
        ...
        case PVMFInfoDataReady:
        {
            ...
            uint32* pPlayerDriverUserData  = GetPlayerDriverUserData();
            if (pPlayerDriverUserData && (((uint32)*pPlayerDriverUserData) & PLAYER_DRIVER_STOPPED_RECEIVED))
            {
                LOGD ("PVMFInfoDataReady, rtsp has been stopped, flush jitter buffer!!!");
                FlushJitterBuffer();
            }
...
void PVMFJitterBufferNode::HandleEvent_IncomingMediaInactivityDurationExpired()
{
    ...
    {
// <--- Morris Yang ALPS00126647 [
        LOGE ("HandleEvent_IncomingMediaInactivityDurationExpired - FlushJitterBuffer");
        ...
        int32* aEventCode)
{
    ...
    {
// <--- Morris Yang ALPS00135691
// In poor network connection case (EDGE), the jitter delay cannot establish and may cause the delay percent reset to 0
// Modify to make sure the percentage is increasing.
        if (iIsBuffering && (iJitterDelayPercent > iLastDelayPercent)) {
        	    iLastDelayPercent = iJitterDelayPercent;
        }
        
        if (iJitterDelayPercent == 100)
        {
            iIsBuffering = false;
            iLastDelayPercent = 100;
        }
        else
        {
            if (iIsBuffering == false)
            {
                iIsBuffering = true;
                iLastDelayPercent = 0;
            }
        ...
        //LOGE ("@@ JBN iJitterDelayPercent=%d, iLastDelayPercent=%d", iJitterDelayPercent, iLastDelayPercent);
// --->
...
// <--- Morris Yang ALPS00224993
        if (iJitterDelayPercent > 0) {
            ...
            iConsective_zero_buffering_count++;
        }

        if ((iConsective_zero_buffering_count > 150) || ((iConsective_zero_buffering_count > 10) && (ipJitterBufferMisc->GetSessionDuration() > 47700000))) // 13.25 hours
        {
            ReportErrorEvent(PVMFErrTimeout, NULL);
            return;
        }
// --->
