...
#define LOG_TAG "RTPJitterBufferImpl"
...
{
    // <--- Demon Deng (ALPS00136669) purge before seqNumLock will generate a bad iReadOffset
    if (seqNumLock)
        ...
        uint32 aFragIndex)
{
    ...
    if (iInPlaceProcessing == false)
    {
        ...
        if (status == false)
        {
            ...
            LOGE ("0x%x RTPPacketHeaderParser::ParseRTPHeader: Jitter Buffer Full A", this);
        ...
        {
            //PVMF_JB_LOGERROR((0, "0x%x RTPPacketHeaderParser::ParseRTPHeader: Jitter Buffer Full", this));
            LOGE ("0x%x RTPPacketHeaderParser::ParseRTPHeader: Jitter Buffer Full B", this);
            return PVMF_JB_ERR_INSUFFICIENT_MEM_TO_PACKETIZE;
        }
    ...
    {
        LOGD ("RTPPacketHeaderParser::ParseRTPHeader: SSRC=%u, rtpSeqNum=%d, rtpTs=%u, rtpPacketLen=%d", SSRC, seqNum, rtpTimeStamp, rtpPacketLenExcludingHeader);
    }
...
OSCL_EXPORT_REF void PVMFRTPJitterBufferImpl::SetAllowRegOldPkt(bool value)
{
    if (iJitterBuffer)
    {
        iJitterBuffer->SetAllowRegOldPkt(value);
    }
}
// --->
...
bool PVMFRTPJitterBufferImpl::CanRetrievePacket(PVMFSharedMediaMsgPtr& aMediaOutMsg, bool& aCmdPacket)
{
    ...
    else
    {
        ...
        if (stats.currentOccupancy > 0)
        {
            ...
            uint32 prevPacketLoss = iJitterBuffer->getStats().totalPacketsLost;
            uint32 currPacketLoss;
            //--->
         ...
         // <--- Demon Deng 20101130 (ALPS00136734) only update iLastPacketOutTs when no packet loss between two frames
                currPacketLoss = iJitterBuffer->getStats().totalPacketsLost;
         // <--- Morris Yang 20100914 (ALPS00125050, ALPS00125146, ALPS00125063)
                if (converted_ts != iLastPacketOutTs && currPacketLoss == prevPacketLoss) {
			iLastFrameOutTsDiff = converted_ts - iLastPacketOutTs;	                
                }
         //--->
         //--->
		...
		*/
		if (_uDbgFlags & RTSP_SHOW_TIMESTAMP)
		{
			LOGD ("SendData Mime=%s, SSRC=%d, TS=%d, SEQNUM=%d, Occupancy=%u",
					     irMimeType.get_cstr(),
                                        aMediaOutMsg->getStreamID(),
                                        aMediaOutMsg->getTimestamp(),
                                        aMediaOutMsg->getSeqNum(),
