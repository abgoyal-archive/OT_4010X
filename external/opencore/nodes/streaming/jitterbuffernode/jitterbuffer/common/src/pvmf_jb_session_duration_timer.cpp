...
PVMFStatus PvmfJBSessionDurationTimer::Start(bool bStartFromSetPlayRange)
{
    ...
    if ((iSessionDurationInMS > 0) && (iCurrentMonitoringIntervalInMS > 0))
    {
        ...
        //LOGE ("@@ PvmfJBSessionDurationTimer::Start(%d) - SessionDurationInMS = %d, iCurrentMonitoringIntervalInMS = %d", bStartFromSetPlayRange, iSessionDurationInMS, iCurrentMonitoringIntervalInMS);
        iClock.Start();
        uint32 timebase32 = 0;
        iTimerStartTimeInMS = 0;
        iMonitoringIntervalElapsed = 0;
