...
uint64 getTickCountMs()
{
    ...
    return (uint64)(tv.tv_sec*1000LL + tv.tv_usec/1000);
}
...
{
// <--- Morris Yang ALPS00225213, ALPS00225214 move to unicast node
...
{
    LOGD ("StreamingSessionBufferingStart");
// <--- Morris Yang ALPS00225250
    iLastBufferingStartTime = getTickCountMs();
// --->
...
{
// <--- Morris Yang ALPS00225250
    if (0 != iLastBufferingStartTime)
    {
        iLastBufferingTime = Oscl_Int64_Utils::get_uint64_lower32(getTickCountMs() - iLastBufferingStartTime);
        LOGD ("StreamingSessionBufferingEnd iLastBufferingTime=%u", iLastBufferingTime);
    }

    if (ipSessionDurationTimer)
    {
        ComputeCurrentSessionDurationMonitoringInterval();
        ipSessionDurationTimer->Start(false);
    }
// --->
}
...
OSCL_EXPORT_REF bool PVMFJitterBufferMisc::SetSessionDurationExpired()
{
    ...
    LOGD ("SetSessionDurationExpired- Estimated Server Clock [%d] Client Clock[%d]", estServClock32, clientClock32);
...
OSCL_EXPORT_REF bool PVMFJitterBufferMisc::SetPlayRange(int32 aStartTimeInMS, int32 aStopTimeInMS, bool aPlayAfterASeek, bool aStopTimeAvailable)
{
    ...
    if (iPlayStopTimeAvailable == true)
    {
        ...
        if ((ipSessionDurationTimer) && (!iStreamingSessionExpired || (aPlayAfterASeek)))
        {
            ...
            ComputeCurrentSessionDurationMonitoringInterval();
            // <--- Demon Deng ALPS00139342, don't start timer here
#if 0
        // <--- Morris Yang ALPS00225250
            ipSessionDurationTimer->Start(true);
        // --->
#endif
            // --->
        }
...
OSCL_EXPORT_REF void PVMFJitterBufferMisc::SetServerInfo(PVMFJitterBufferFireWallPacketInfo& aServerInfo)
{
    if (iFireWallPacketsExchangeEnabled)
    {
        if (!ipFireWallPacketExchangerImpl)
        {
            ...
            LOGD ("ipFireWallPacketExchangerImpl created 0x%x", ipFireWallPacketExchangerImpl);
    ...
    // <--- Demon Deng ALPS00028091, in case we disable firewall after session node prepared
    else
    {
        ipObserver->MediaReceivingChannelPrepared(true);
    }
...
void PVMFJitterBufferMisc::PVMFJBSessionDurationTimerEvent()
{
    ...
    // Morris Yang <--- Morris Yang ALPS00225250
        ipSessionDurationTimer->Start(false);
    // --->
    }
    return;
