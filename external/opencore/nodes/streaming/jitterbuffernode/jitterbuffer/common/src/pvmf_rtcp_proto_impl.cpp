    ...
    for (uint32 i = 0; i < numFrags; i++)
    {
        ...
        for (int32 ii = 0; ii < filled_size; ii++)
        {
            ...
            if (BYE_RTCP_PACKET == array_of_packet_types[ii])
            {
                ...
                // <--- Morris Yang ALPS00225347 The Helix server sends abnormal consective BYE packets
                    iLastRTCPBYETick = getTickCountMs();
                    if ((getTickCountMs() - iLastRTCPBYETick) < 100)
                    {
                        iConsectiveRTCPByeCount++;
                    	}
                    else
                    	{
                    	    iConsectiveRTCPByeCount = 0;
                    ...
                    if ((iConsectiveRTCPByeCount > 6) && (0 == oscl_strncmp(irRTPDataJitterBuffer.GetMimeType(), "RTP/video/MP4V-ES", oscl_strlen("RTP/video/MP4V-ES"))))
                    {
                        LOGE ("Abnormal consective RTCP BYE received!!! (%s)", irRTPDataJitterBuffer.GetMimeType());
                        return PVMFFailure;
                    }
                // --->
...
PVMFStatus PVRTCPProtoImplementor::RTCPByeReceived(PVRTCPChannelController* aChannelController)
{
    ...
    LOGD ("RTCPByeReceived for mime %s", aChannelController->GetJitterBuffer().GetMimeType());
