...
#define LOG_TAG "JitterBufferImpl"
...
#endif
#define DEFAULT_PKT_NEEDED_FOR_VOTE 5
// <--- Demon Deng 20101130 (ALPS00136734)
#define DEFAULT_PKT_PACKET_DURATION_IN_MS 50
//--->
...
void PVMFJitterBufferImpl::ResetParams(bool aReleaseMemory)
{
    ...
    iLastPacketOutTs = 0;
    iLastFrameOutTsDiff = DEFAULT_PKT_PACKET_DURATION_IN_MS;
        ...
        , iObserverContext(aJBConstructParams.GetContextData())
{
    ...
    iConsecutiveLateMediaMsgCount = 0;

     PVMFJitterBufferMisc* _pJBMisc = aJBConstructParams.GetJitterBufferMisc();
     uint32 _session_duration = 0;
     
     if (_pJBMisc)
         _session_duration = _pJBMisc->GetSessionDuration();
     
     cPktNeededForVote = (_session_duration <= 1000 ? 2 : DEFAULT_PKT_NEEDED_FOR_VOTE);

     _uDbgFlags = 0;
     char value[PROPERTY_VALUE_MAX];
     property_get("rtsp.showts", value, "0");
     bool _res = atoi(value);
     if (_res) _uDbgFlags |= RTSP_SHOW_TIMESTAMP;
...
OSCL_EXPORT_REF PVMFJitterBufferRegisterMediaMsgStatus PVMFJitterBufferImpl::RegisterMediaMsg(PVMFSharedMediaMsgPtr& aMsg)
{
    ...
    else
    {
        ...
        if (!inputDataPacket->getMediaDataImpl(mediaDataIn))
        {
            ...
            LOGE ("PVMFJitterBufferImpl::RegisterMediaMsg: PVMF_JB_REGISTER_MEDIA_MSG_ERR_CORRUPT_PACKET A");
        ...
        else
        {
            if (iOnePacketPerMediaMsg)
            {
                ...
                if (PVMF_JB_PACKET_PARSING_SUCCESS == status)
                {
                    if (CanRegisterMediaMsg())
                    {
                        ...
                        else
                        {
                            ...
                            LOGE ("PVMFJitterBufferImpl::RegisterMediaMsg: PVMF_JB_REGISTER_MEDIA_MSG_FAILURE_JB_FULL A");
                        }
                    }
                    else
                    {
                        ...
                        LOGE ("PVMFJitterBufferImpl::RegisterMediaMsg: Invalid State to accept any packet A");
                        retval = PVMF_JB_REGISTER_MEDIA_MSG_ERR_CORRUPT_PACKET;
                    }
                }
                else
                {
                    ...
                    LOGE ("PVMFJitterBufferImpl::RegisterMediaMsg: ParsePacketHeader failed retval %d", retval);
                }
            }
            else
            {
                    ...
                    for (uint ii = 0; ii < numOfFragments ; ii++)
                    {
                        ...
                        if (PVMF_JB_PACKET_PARSING_SUCCESS == status)
                        {
                            ...
                            if (CanRegisterMediaMsg())
                            {
                                ...
                                {
                                    retval = PVMF_JB_REGISTER_MEDIA_MSG_FAILURE_JB_FULL;
                                    LOGE ("PVMFJitterBufferImpl::RegisterMediaMsg: PVMF_JB_REGISTER_MEDIA_MSG_FAILURE_JB_FULL B");
                                }
                            }
                            else
                            {
                                ...
                                //PVMF_JB_LOGERROR((0, "PVMFJitterBufferImpl::RegisterMediaMsg: Invalid State to accept any packet"));
                                //PVMF_JB_LOGDATATRAFFIC_IN((0, "PVMFJitterBufferImpl::RegisterMediaMsg: Invalid State to accept any packet"));
                                LOGE ("PVMFJitterBufferImpl::RegisterMediaMsg: Invalid State to accept any packet B");
                                retval = PVMF_JB_REGISTER_MEDIA_MSG_ERR_CORRUPT_PACKET;
                            }
                        }
                        else
                        {
                            ...
                            if (PVMF_JB_ERR_LATE_PACKET == status)
                            {
                                ...
                                //<--- Demon Deng do not raise a fatal error by 10 late packets (ALPS00136683)
#if 0
                                iConsecutiveLateMediaMsgCount++;
                                if (iConsecutiveLateMediaMsgCount >= 10) {                                	
                                    retval = PVMF_JB_REGISTER_MEDIA_MSG_ERROR_FATAL;
                                }
#endif
                                //--->
                            }
...
OSCL_EXPORT_REF PVMFTimestamp PVMFJitterBufferImpl::peekNextElementTimeStamp()
{
    if (iJitterBuffer->getNumElements() > 0)
    {
     ...
     // <--- Morris Yang add for preventing TS overflow
        PVMFTimestamp currTSOri = currTS;
        currTS = ((currTS >= prevTS) ? currTS : prevTS);
     // --->
        ts64 += (currTS - prevTS);
        PVMFTimestamp adjTS =
...
PVMFJitterBufferRegisterMediaMsgStatus PVMFJitterBufferImpl::AddPacket(PVMFSharedMediaDataPtr& aDataPacket)
{
    ...
    if (seqNumLock)
    {
        ...
        if (aDataPacket->getStreamID() == SSRCLock)
        {
            ...
            */
                //LOGE ("PVMFJitterBufferImpl::addPacket: MimeType=%s TS=%u, SEQNUM= %d, SIZE=%d", irMimeType.get_cstr(), aDataPacket->getTimestamp(), aDataPacket->getSeqNum(), aDataPacket->getFilledSize());
            ...
            /*
                PVMF_JB_LOGDATATRAFFIC_IN_E((0, "PVMFJitterBufferImpl::addPacket - OverWrite: MimeType=%s TS=%d, SEQNUM= %d",
                                             irMimeType.get_cstr(), aDataPacket->getTimestamp(), aDataPacket->getSeqNum()));
            */
                LOGE ("PVMFJitterBufferImpl::addPacket - OverWrite: MimeType=%s TS=%d, SEQNUM= %d", irMimeType.get_cstr(), aDataPacket->getTimestamp(), aDataPacket->getSeqNum());
    ...
    else
    {
        ...
        {//discard packet
            ...
            LOGE ("PVMFJitterBufferImpl::addPacket: ERROR wrong ssrc %d", aDataPacket->getStreamID());
            return PVMF_JB_REGISTER_MEDIA_MSG_SUCCESS;
        }
            ...
            if ((*it)->getStreamID() == SSRCLock)
            {
                ...
                if (iFirstSeqNum != seqNum)
                {
                    ...
                    if (iBroadCastSession == true)
                    {
                        ...
                        {
                            LOGE ("PVMF_JB_REGISTER_MEDIA_MSG_ERR_UNEXPECTED_DATA");
                    ...
                    {
                        LOGE ("PVMF_JB_REGISTER_MEDIA_MSG_ERR_LATE_MSG");
                ...
                if (! iJitterBuffer->addElement(*it, iFirstSeqNum))
                {
                    ...
                    LOGE ("PVMFJitterBufferImpl::addPacket: addElement failed");
                    bNoErr = false;
                }
                else
                {
            //     LOGE ("PVMFJitterBufferImpl::addPacket: MimeType=%s TS=%d, SEQNUM= %d, SIZE=%d", irMimeType.get_cstr(), (*it)->getTimestamp(), (*it)->getSeqNum(), (*it)->getFilledSize());
                    /*
    ...
    PVMFJitterBufferStats stats = getJitterBufferStats();
    if ((int32)stats.totalPacketsLost > 0)
    {
        //PVMF_JB_LOGWARNING((0, "Packet Loss: MimeType=%s, NumPacketsLost=%d", irMimeType.get_cstr(), stats.totalPacketsLost));
        LOGE ("Packet Loss: MimeType=%s, NumPacketsLost=%d", irMimeType.get_cstr(), stats.totalPacketsLost);
    }
...
// <--- Morris Yang 20100914 don't set the timestamp to iLastPacketOutTs, otherwise the last two frames will not be rendered. (ALPS00125050, ALPS00125146, ALPS00125063)
    // aMediaOutMsg->setTimestamp(iLastPacketOutTs);
        aMediaOutMsg->setTimestamp(iLastPacketOutTs+2*iLastFrameOutTsDiff);
// --->
        ...
        */
         ...
         LOGE ("GenerateAndSendEOSCommand: MimeType=%s, StreamID=%d, TS=%u", irMimeType.get_cstr(), aMediaOutMsg->getStreamID(), aMediaOutMsg->getTimestamp());
    ...
    else
    {
        ...
        if (diff32ms >= iDurationInMilliSeconds)
        {
            ...
            if ((diff32ms > JITTER_BUFFER_DURATION_MULTIPLIER_THRESHOLD*iDurationInMilliSeconds) && !iOverflowFlag && (consecutiveLowBufferCount > CONSECUTIVE_LOW_BUFFER_COUNT_THRESHOLD))
            {
                ...
                LOGE ("PVMFJitterBufferNode::IsDelayEstablished reporting PVMFInfoSourceOverflow");
            }
...
OSCL_EXPORT_REF PVMFSharedMediaDataPtr PVMFJitterBufferImpl::RetrievePacketPayload()
{
    ...
    if (elem.GetRep() != NULL)
    {
        ...
        // <--- Morris Yang 20100623 add for preventing TS overflow
        currTS = ((currTS >= iPrevTSOut) ? currTS : iPrevTSOut);
        // --->


// <--- Morris Yang ALPS00225346 protection for abnormal rtp timestamp
        if ((currTS > 0xFFFFF000) && (elem->getSeqNum() == 1) &&
        	((oscl_strncmp(irMimeType.get_cstr(), "RTP/audio/MP4A-LATM", oscl_strlen("RTP/audio/MP4A-LATM"))) == 0))
        {
            LOGD ("[Warning] Correct abnormal rtpTS=%u, seqNo=%u", currTS, elem->getSeqNum());
            currTS = iPrevTSOut;
        }
// --->
        ...
        PVMF_JB_LOGDATATRAFFIC_OUT((0, "PVMFJitterBufferImpl::retrievePacket: MimeType=%s, TS=%d, SEQNUM= %d",
                                    ...
                                    elem->getSeqNum()));
        */
        if (_uDbgFlags & RTSP_SHOW_TIMESTAMP)
        {
        	LOGD ("RetrievePacketPayload Mime=%s, Seq=%d, OriTS=%u, AdjTS=%u, Size=%u", irMimeType.get_cstr(), elem->getSeqNum(), currTS, adjustedTS, elem->getFilledSize());
        }
