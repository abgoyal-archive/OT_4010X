...
#define RTSP_SHOW_TIMESTAMP (1 << 0)
...
// --->
 ...
 // <--- Morris Yang ALPS00125526
            ...
            {
                iOldPktRegCount++;
                LOGD ("[Warning] Adding Old Pkt seqNum=%u, offset=%u, iOldPktRegCount=%u", seqNum, offset, iOldPktRegCount);
            }
// --->
            ...
            if (currElem.GetRep() == NULL)
            {
                ...
                //LOGE ("AddElement offset=%d, seqNum=%d, iNumElems=%d, iArraySize=%d", offset, seqNum, iNumElems, iArraySize);
                return (oRet);
            }
            ...
            /*
                PVMF_JB_LOGDATATRAFFIC_IN((0, "[0x%x] JB OW: iReadOffset=%d, iNumElemsLeft=%d, iLastRetrievedSeqNum=%d, old seqNum=%d, new seqNum=%d",
                                           ...
                                           currElem->getSeqNum()));
            */
                LOGE ("JB OW: iReadOffset=%d, iNumElemsLeft=%d, iLastRetrievedSeqNum=%d, old seqNum=%d, new seqNum=%d, SIZE=%d", iReadOffset,
                                           iNumElems,
                                           iLastRetrievedSeqNum,
                                           seqNum,
                                           currElem->getSeqNum(),
                                           currElem->getFilledSize());
...
// <--- Morris Yang ALPS00125526 if there is any old packet, retrieve the element with smallest sequence number
// <--- Morris Yang ALPS00131435
            if (iOldPktRegCount && iAllowRegOldPkt)
// --->
            {
                uint32 _ReadOffsetSmallestSeq = 0;
                uint32 _seq_smallest = 0xFFFFFFFF;
                typedef typename Oscl_Vector<PVMFSharedMediaDataPtr, Alloc>::iterator iterator_type;
                iterator_type it;
                uint32 _count = 0;
                for (it = iMediaPtrVec.begin(); it != iMediaPtrVec.end(); it++)
                {
                    ...
                    {
                        uint32 size = 0;
                        uint32 _seq = it->GetRep()->getSeqNum();
                        if (_seq < _seq_smallest)
                        {
                            _seq_smallest = _seq;
                            _ReadOffsetSmallestSeq = _count;
                        }
                    }
                    _count++;
                 }
                ...
                dataPkt = iMediaPtrVec[_ReadOffsetSmallestSeq];
                iNumElems--;
                /* Mark the retrieved element location as free */
                PVMFSharedMediaDataPtr retElem = iMediaPtrVec[_ReadOffsetSmallestSeq];
                retElem.Unbind();
                iMediaPtrVec[_ReadOffsetSmallestSeq] = retElem;
                ...
                iJitterBufferStats.packetSizeInBytesLeftInBuffer -= size;
                LOGD ("retrieveElement: _ReadOffsetSmallestSeq=%d, iNumElemsLeft=%d, SeqNum=%d, SIZE=%d, iOldPktRegCount=%d",
                  	                  _ReadOffsetSmallestSeq, iNumElems, iLastRetrievedSeqNum, dataPkt->getFilledSize(), iOldPktRegCount);  
                return (dataPkt);
            }
// --->
           ...
           /*
            ...
            LOGE ("PVMFDynamicCircularArray::retrieveElement: iReadOffset=%d, iNumElemsLeft=%d, SeqNum=%d, SIZE=%d", iReadOffset,
                                        iNumElems, iLastRetrievedSeqNum, dataPkt->getFilledSize());  
            */
            return (dataPkt);
        }
        ...
        // <--- Morris Yang ALPS00131435
        void SetAllowRegOldPkt(bool value) { iAllowRegOldPkt = value; }
        // --->
...
// <--- Morris Yang ALPS00125526
        uint32 iOldPktRegCount;
// --->

        // <--- Morris Yang ALPS00131435
        bool iAllowRegOldPkt;
        // --->
...
class PVMFJitterBuffer
{
    ...
    // <--- Morris Yang ALPS00131435
        virtual void SetAllowRegOldPkt(bool value) = 0;
    // --->
};
        ...
        , public PVMFJBEventNotifierObserver
{
    ...
    protected:
        ...
        uint32 _uDbgFlags;
        uint    cPktNeededForVote;
    // <--- Morris Yang ALPS00126055
        uint32 iConsecutiveLateMediaMsgCount;
    // --->
