...
static bool bShowVideoLog = false;
static bool bShowAudioLog = false;
#define LOGP(...) \
{\
    if (bShowLog)\
    {\
        if ((bShowVideoLog && !strcmp((char*)iName.Str(), "PVMFOMXVideoDecNode")) || \
            (bShowAudioLog && !strcmp((char*)iName.Str(), "PVMFOMXAudioDecNode")) || \
            (!bShowVideoLog && !bShowAudioLog))\
        {\
            LOGD(__VA_ARGS__);\
        }\
    }\
                                     ...
                                     OMX_OUT OMX_BUFFERHEADERTYPE* aBuffer)
{
    ...
    if (Node->IsComponentMultiThreaded())
    {
        ...
        // added by Ryan for profiling
#if 1                
        if (!strcmp((char*)Node->iName.Str(), "PVMFOMXVideoDecNode"))
        {               
            //LOGD("CFB(%d)", _SeqNum);
            if (Node->getSeqNum() == 0)
            {
                Node->iCBSeqNum = 0;
                OsclProfile::iCallbackFillBufferDoneInTime.clear();               
            }
            OsclProfile::Push(OsclProfile::iCallbackFillBufferDoneInTime, Node->iCBSeqNum);
            Node->iCBSeqNum ++;
        }
#endif
...
OSCL_EXPORT_REF OsclReturnCode PVMFOMXBaseDecNode::ProcessCallbackFillBufferDone_MultiThreaded(OsclAny* P)
{
    ...
    {
        OsclProfile::Push(OsclProfile::iProcessCallbackFillBufferDoneInTime, iSeqNum);
    }
...
OSCL_EXPORT_REF PVMFOMXBaseDecNode::PVMFOMXBaseDecNode(int32 aPriority, const char aAOName[], bool accelerated) :
        ...
        , iCBSeqNum(0)
{
    bShowLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_basedecnode");
    bShowVideoLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_videodecnode");
    bShowAudioLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_audiodecnode");
    ...
    switch (iProcessingState)
    {
        ...
        case EPVMFOMXBaseDecNodeProcessingState_PortReconfig:
        {
            ...
            else
            {
                ...
                // add by Ryan for return all buffer keeping by display
                if (!strcmp((char*)iName.Str(), "PVMFOMXVideoDecNode"))
                {
                    if (mSinkNode)
                    {
                        mSinkNode->ClearBuffer();
                    }
                } 
...
bool PVMFOMXBaseDecNode::SendOutputBufferToOMXComponent()
{
    ...
    // modify by Ryan
    uint32 _iSeqNum = m_iOutSeqNum;
    long long _time = OsclTickCount::getTimeMs();

    LOGP("OMX_FillThisBuffer(%d) addr: 0x%x", iNumOutstandingOutputBuffers, output_buf->pBufHdr->pBuffer);  
    OMX_ERRORTYPE err = OMX_FillThisBuffer(iOMXDecoder, output_buf->pBufHdr);
    if (err != OMX_ErrorNone)
    {
        LOGE("PVMFOMXBaseDecNode::SendOutputBufferToOMXComponent OMX_FillThisBuffer err: %x", err);
        return false;
    }

    if (!strcmp((char*)iName.Str(), "PVMFOMXVideoDecNode"))
    {
        LOGV("SendOutputBufferToOMXComponent: %d", iSeqNum);
#if 1   
        OsclProfile::Push(OsclProfile::iSendOutBufToOMXInTime, _iSeqNum, _time);
        m_iOutSeqNum++;
#endif                    
    }
    ...
    do
    {
        ...
        if (iObtainNewInputBuffer == true)
        {
            ...
            else
            {
                ...
                // modify by Ryan
                uint32 _iSeqNum = m_iInSeqNum;
                long long _time = OsclTickCount::getTimeMs();
                
                OMX_ERRORTYPE err = OMX_EmptyThisBuffer(iOMXDecoder, input_buf->pBufHdr);
                if (err != OMX_ErrorNone)
                {
                    LOGE("PVMFOMXBaseDecNode::SendOutputBufferToOMXComponent OMX_EmptyThisBuffer err: %x", err);
                    return false;
                }
                ...
                {
#if 1   
                    OsclProfile::Push(OsclProfile::iSendInBufToOMXInTime, _iSeqNum, _time);
                    m_iInSeqNum++;
#endif                    
                }
...
/////////////////////////////////////////////////////////////////////////////
OSCL_EXPORT_REF bool PVMFOMXBaseDecNode::SendConfigBufferToOMXComponent(uint8 *initbuffer, uint32 initbufsize, bool bLastConfigBuf)
...
//mtk80366 Remove start for alps00122003->
    	//input_buf->pBufHdr->nFlags |= OMX_BUFFERFLAG_LASTCODECCONFIG;
...
OSCL_EXPORT_REF bool PVMFOMXBaseDecNode::CreateOutMemPool(uint32 num_buffers)
{
   ...
   if (!strcmp((char*)iName.Str(), "PVMFOMXVideoDecNode"))
   {   
      LOGD("PVMFOMXBaseDecNode::CreateOutMemPool - name: %s, num_buffers: %d, Use Pmem to alloc buffer", iName.Str(), num_buffers);
      OSCL_TRY(leavecode, iOutBufMemoryPool = OSCL_NEW(OsclMemPoolFixedChunkAllocator, (num_buffers, 0, &iOutBufMemoryAlloc)););    
   }
   else
   {  
      // for audio path, don't use pmem
      LOGD("PVMFOMXBaseDecNode::CreateOutMemPool - name: %s, num_buffers: %d, Use malloc to alloc buffer", iName.Str(), num_buffers);
      OSCL_TRY(leavecode, iOutBufMemoryPool = OSCL_NEW(OsclMemPoolFixedChunkAllocator, (num_buffers)););
   }    
#else
    LOGV("PVMFOMXBaseDecNode::CreateOutMemPool - name: %s, num_buffers: %d, Use malloc to alloc buffer", iName.Str(), num_buffers);
//    OSCL_TRY(leavecode, iOutBufMemoryPool = OSCL_NEW(OsclMemPoolFixedChunkAllocator, (num_buffers, 0, &iOutBufMemoryAlloc)););    
    OSCL_TRY(leavecode, iOutBufMemoryPool = OSCL_NEW(OsclMemPoolFixedChunkAllocator, (num_buffers)););
#endif
...
#if 0
        OsclError::LeaveIfError(OsclErrNoMemory);
#endif        
        return false;
    }
...
OSCL_EXPORT_REF bool PVMFOMXBaseDecNode::CreateInputMemPool(uint32 num_buffers)
{
    ...
    if ((iOMXComponentSupportsExternalInputBufferAlloc && !iOMXComponentSupportsMovableInputBuffers))
    {
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                        (0, "%s::CreateInputMemPool() Allocating input buffers of size %d as well", iName.Str(), iOMXComponentInputBufferSize));
        ...
        OMX_OUT OMX_BUFFERHEADERTYPE* aBuffer)
{
    ...
    if (!strcmp((char*)iName.Str(), "PVMFOMXVideoDecNode"))
    {
        OsclProfile::Push(OsclProfile::iFillBufferDoneProcessingInTime, iSeqNum);
    }
...
// Ryan !!            
#if 0            
            if (!strcmp((char*)iName.Str(), "PVMFOMXAudioDecNode"))
            {
                if (aBuffer->nFilledLen)
                {
                    FILE *pf = fopen("/sdcard/test2.dat", "ab+");
                    fwrite(pBufdata, 1, aBuffer->nFilledLen, pf);
                    fclose(pf);
                }
            }
#endif            
...
//JJDBG
                ...
                {
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
                                    (0, "%s::Can't get handle for decoder!, err: 0x%X", iName.Str(), err));
                    iOMXDecoder = NULL;
                    oscl_free(aOutputParameters);
                    CommandComplete(iInputCommands, aCmd, PVMFErrResource);
                    return;
            ...
            iThreadSafeHandlerFillBufferDone = OSCL_NEW(FillBufferDoneThreadSafeCallbackAO, (this, iNumOutputBuffers, "FillBufferDoneAO", OsclActiveObject::EPriorityHighest));
            // modified by Ryan for performance tuning
...
PVMFStatus PVMFOMXBaseDecNode::ResetNodeParam()
{
    LOGV("***** ResetNodeParam() *****");
...
PVMFStatus PVMFOMXBaseDecNode::ClearBuffer()
{
    LOGV("***** %s ClearBuffer() *****", iName.Str());
#if 0
    OMX_ERRORTYPE  err = OMX_ErrorNone;
    // send command to flush all ports (arg is OMX_ALL)
    err = OMX_SendCommand(iOMXDecoder, OMX_CommandFlush, OMX_ALL, NULL);
