...
PVMFFileOutputNode::PVMFFileOutputNode(int32 aPriority)
		...
		, iHasWavHeaderInFile(false)			//added by HP Cheng
{
    ConstructL();
    int32 err;
    OSCL_TRY(err,
...
static void WAV_Put8(uint32 *pHeaderCount, uint8 *outbuf, uint8 data)
{
   *(uint8*)(outbuf+(*pHeaderCount)) = data;
   (*pHeaderCount) +=1;
}

static void WAV_Put16(uint32 *pHeaderCount, uint8 *outbuf, uint16 data)
{
   WAV_Put8(pHeaderCount, outbuf, (uint8)(data&0xFF));
   WAV_Put8(pHeaderCount, outbuf, (uint8)(data>>8));
}

static void WAV_Put32(uint32 *pHeaderCount, uint8 *outbuf, uint32 data)
{
   WAV_Put16(pHeaderCount, outbuf, (uint16)(data&0xFFFF));
   WAV_Put16(pHeaderCount, outbuf, (uint16)(data>>16));
}

uint32 GetWavHeaderLength(uint32 fmt)
{
	uint32 header_length;
   switch (fmt)   {
   case WAV_FORMAT_LPCM:     /* linear PCM */
      header_length = 44;
      break;
...
bool MakeWavHeader(void *out, uint32 data_length, uint32 fmt, uint32 chNum, uint32 sampleRate)
{
//   WAV_ENC_INTERNAL *ihdl = hdl->wav_enc_data;
   uint32 headerCount = 0;

   //trasforming parameters
   uint8* outbuf = (uint8*)out;
   int FormatTag = fmt;
   int ChannelNum = chNum;
   int SampleRate = sampleRate;
   int header_length;
   int SamplesWritten;
   int blockNum, resBytes, sampleNumInBlocks, sampleNumInRes;		//for SamplesWritten calculation
   ...
   WAV_Put32(&headerCount, outbuf, 0x46464952);            // RIFF      0
   WAV_Put32(&headerCount, outbuf, header_length + data_length);      // file size 4
   WAV_Put32(&headerCount, outbuf, 0x45564157);            // "WAVE"  8
   WAV_Put32(&headerCount, outbuf, 0x20746D66);            // "fmt"  12

   switch (FormatTag)
   {
   case WAV_FORMAT_LPCM:     /* linear PCM */
      WAV_Put32(&headerCount, outbuf, 16                  );   // fmt chunk size  16 
      WAV_Put16(&headerCount, outbuf, FormatTag     );   // FormatTag       20 
      WAV_Put16(&headerCount, outbuf, ChannelNum    );   // Channels        22 
      WAV_Put32(&headerCount, outbuf, SampleRate    );   // Samples Rate    24 
      WAV_Put32(&headerCount, outbuf, SampleRate*2*ChannelNum );   // Byte Rate       28 
      WAV_Put16(&headerCount, outbuf, 4                   );   // Block Align     32 
      WAV_Put16(&headerCount, outbuf, 16                  );   // Bits per sample 34 
      WAV_Put32(&headerCount, outbuf, 0x61746164          );   // "DATA"          36 
      WAV_Put32(&headerCount, outbuf, data_length         );   // data cksize,    40 
      break;
   case WAV_FORMAT_ALAW:     /* G.711 a-law */
   case WAV_FORMAT_ULAW:     /* G.711 u-law */
      // calculate SamplesWritten from data_length in A/ULaw
      SamplesWritten = data_length / 2;
      WAV_Put32(&headerCount, outbuf, 18                    ); // fmt chuck size  16
      WAV_Put16(&headerCount, outbuf, FormatTag       ); // FormatTag       20
      WAV_Put16(&headerCount, outbuf, ChannelNum      ); // Channels        22
      WAV_Put32(&headerCount, outbuf, SampleRate      ); // Samples Rate    24
      WAV_Put32(&headerCount, outbuf, SampleRate*ChannelNum      ); // Byte Rate       28
      WAV_Put16(&headerCount, outbuf, 2                     ); // Block Align     32
      WAV_Put16(&headerCount, outbuf, 8                     ); // Bits per sample 34
      WAV_Put16(&headerCount, outbuf, 0                     ); // ExtSize         36
      WAV_Put32(&headerCount, outbuf, 0x74636166            ); // "fact"          38
      WAV_Put32(&headerCount, outbuf, 4                     ); // Fact chunk Size 42
	  WAV_Put32(&headerCount, outbuf, SamplesWritten  ); // SamplesWritten  46
	  WAV_Put32(&headerCount, outbuf, 0x61746164            ); // data            50
	  WAV_Put32(&headerCount, outbuf, data_length           ); // data cksize     54
	  break;
   case WAV_FORMAT_DVIADPCM:    /* dvi adpcm */
	   // calculate SamplesWritten from data_length in ADPCM
	   // 505 samples -> 256 byte
	   // the resudule: 1st sample -> 4 bytes, others 0.5 bytes
	   blockNum = data_length / 256;
	   resBytes = data_length - blockNum * 256;
	   sampleNumInBlocks = blockNum * 505;
	   sampleNumInRes = (resBytes - 4) * 2 + 1;
	   SamplesWritten = sampleNumInBlocks + sampleNumInRes;

	   WAV_Put32(&headerCount, outbuf, 20                        ); // fmt chuck size  16
	   WAV_Put16(&headerCount, outbuf, FormatTag           ); // FormatTag       20
	   WAV_Put16(&headerCount, outbuf, ChannelNum          ); // Channels        22
	   WAV_Put32(&headerCount, outbuf, SampleRate          ); // Samples Rate    24
	   WAV_Put32(&headerCount, outbuf, (SampleRate<<8)*ChannelNum/505 ); // Byte Rate       28
	   if(1==ChannelNum)
	   {
		 WAV_Put16(&headerCount, outbuf, 256                       ); // Block Align     32
         WAV_Put16(&headerCount, outbuf, 4                         ); // Bits per sample 34
         WAV_Put16(&headerCount, outbuf, 2                         ); // ExtSize         36
         WAV_Put16(&headerCount, outbuf, 505                       ); // SamplesPerBlock 38
      }
      else
      {
         WAV_Put16(&headerCount, outbuf, /*2048*/512                      ); // Block Align     32
         WAV_Put16(&headerCount, outbuf, 4                         ); // Bits per sample 34
         WAV_Put16(&headerCount, outbuf, 2                         ); // ExtSize         36
         WAV_Put16(&headerCount, outbuf, /*2041*/505                      ); // SamplesPerBlock 38
      }
      WAV_Put32(&headerCount, outbuf, 0x74636166                ); // "fact"          40
      WAV_Put32(&headerCount, outbuf, 4                         ); // Fact chunk Size 44
      WAV_Put32(&headerCount, outbuf, SamplesWritten      ); // SamplesWritten  48
      WAV_Put32(&headerCount, outbuf, 0x61746164                ); // data            52
      WAV_Put32(&headerCount, outbuf, data_length               ); // data cksize     56
      break;
   default:
      return false;
   }
...
PVMFStatus PVMFFileOutputNode::WriteFormatSpecificInfo(OsclAny* aPtr, uint32 aSize)
{
    ...
    if (iFirstMediaData)
    {
        ...
        else if (((PVMFFileOutputInPort*)iInPort)->iFormat == PVMF_MIME_ADPCM ||
			     ((PVMFFileOutputInPort*)iInPort)->iFormat == PVMF_MIME_PCM16)
        {
			// PCM16, chNum and sampleRate is from Connect when direct connection
			// ADPCM, chNum and sampleRate is from FormatSpecificInfo
			
			//added by HP Cheng
			int wavHdrLen;
			void *pHdr;

			if ( aPtr )	{ // the case of PVMF_MIME_ADPCM, there is FormatSpecificInfo
				// extracting samplingRate from PV_AudioEncodeParam
				uint32 *pU32 = (uint32*)aPtr;
				iSamplingRate = pU32[0];

				// extracting channel Number from PV_AudioEncodeParam
				uint8 *pU8 = (uint8*)aPtr;
				iChannelNumber = pU8[4];

				// create wav header
				wavHdrLen = GetWavHeaderLength(WAV_FORMAT_DVIADPCM); //ask for the size of header
				pHdr = oscl_malloc(wavHdrLen);		
				MakeWavHeader(pHdr, 100, WAV_FORMAT_DVIADPCM, iChannelNumber, iSamplingRate);	//return header length
			}
			else {
				// the case of PVMF_MIME_PCM16, source node connect to composer node directly

				// create wav header
				wavHdrLen = GetWavHeaderLength(WAV_FORMAT_LPCM); //ask for the size of header
				pHdr = oscl_malloc(wavHdrLen);		
				MakeWavHeader(pHdr, 100, WAV_FORMAT_LPCM, iChannelNumber, iSamplingRate);	//return header length
			}

			// update aSize and aPtr
			aSize = wavHdrLen;
			aPtr = (OsclAny*)&pHdr;

			// update iHasWavHeaderInFile
			iHasWavHeaderInFile = true;
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO,(0, "hphp: PVMFFileOutputNode::DoStop"));
    switch (iInterfaceState)
    {
        case EPVMFNodeStarted:
        case EPVMFNodePaused:
			// added by HP Cheng, update WAV header
			if (((PVMFFileOutputInPort*)iInPort)->iFormat == PVMF_MIME_PCM16 )
			{
				// create wav header
				int wavHdrLen = GetWavHeaderLength(WAV_FORMAT_LPCM); //ask for the size of header
				void *pHdr = oscl_malloc(wavHdrLen);		
				MakeWavHeader(pHdr, iFileSize - wavHdrLen, WAV_FORMAT_LPCM, iChannelNumber, iSamplingRate);	//return header length
				iOutputFile.Seek(0,Oscl_File::SEEKSET);
				uint32 retSize = iOutputFile.Write( (OsclAny*)pHdr, sizeof(uint8), wavHdrLen );
				oscl_free(pHdr);
			}
			else if (((PVMFFileOutputInPort*)iInPort)->iFormat == PVMF_MIME_ADPCM )
			{
				// create wav header
				int wavHdrLen = GetWavHeaderLength(WAV_FORMAT_DVIADPCM); //ask for the size of header
				void *pHdr = oscl_malloc(wavHdrLen);		
				MakeWavHeader(pHdr, iFileSize - wavHdrLen, WAV_FORMAT_DVIADPCM, iChannelNumber, iSamplingRate);	//return header length
				iOutputFile.Seek(0,Oscl_File::SEEKSET);
				uint32 retSize = iOutputFile.Write( (OsclAny*)pHdr, sizeof(uint8), wavHdrLen );
				oscl_free(pHdr);
			}
...
void PVMFFileOutputNode::DoReset(PVMFFileOutputNodeCommand& aCmd)
{
    if (IsAdded())
    {
        ...
        {
			// added by HP Cheng, update WAV header
			// only when input is PCM, ans also Wav header has already been written to the file
			// note that iOutputFile might be closed when Calling PVMFFileOutputNode::DoStop()
			// we can still write to iOutputFile, but it will have no effect 
			if (((PVMFFileOutputInPort*)iInPort)->iFormat == PVMF_MIME_PCM16 && iHasWavHeaderInFile)
			{
				// create wav header
				int wavHdrLen = GetWavHeaderLength(WAV_FORMAT_LPCM); //ask for the size of header
				void *pHdr = oscl_malloc(wavHdrLen);		
			...
			else if (((PVMFFileOutputInPort*)iInPort)->iFormat == PVMF_MIME_ADPCM && iHasWavHeaderInFile)
			{
				// create wav header
				int wavHdrLen = GetWavHeaderLength(WAV_FORMAT_DVIADPCM); //ask for the size of header
				void *pHdr = oscl_malloc(wavHdrLen);		
