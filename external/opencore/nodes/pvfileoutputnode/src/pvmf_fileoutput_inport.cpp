...
bool PVMFFileOutputInPort::IsFormatSupported(PVMFFormatType aFmt)
{
    if ((aFmt == PVMF_MIME_AMR_IETF) ||
			...
			(aFmt == PVMF_MIME_ADPCM) ||		//added by HP Cheng
        ...
        PvmiCapabilityContext context)
{
    ...
    {
        num_parameter_elements = 17 + 1;					//added by HP Cheng for ADPCM recording
        ...
        else
        {
			...
			parameters[17].value.pChar_value = (char*)PVMF_MIME_ADPCM;			// added by HP Cheng
        }
    }
...
// added by HP Cheng
PVMFStatus PVMFFileOutputInPort::Connect(PVMFPortInterface* aPort)
{
	PvmiKvp* kvp = NULL;
    int numParams = 0;
	PVMFStatus status;
	...
	//
	PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "hphp: Inside PVMFFileOutputInPort::Connect"));

	// get config interface
	OsclAny* temp = NULL;
    aPort->QueryInterface(PVMI_CAPABILITY_AND_CONFIG_PVUUID, temp);
    PvmiCapabilityAndConfig* aConfig = (PvmiCapabilityAndConfig*) temp;
    if (!aConfig)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMFFileOutputInPort::Connect: Error - Peer port does not support capability interface"));
        return PVMFFailure;
    }

	// get sampling rate from the media input node through port
	...
	{
		PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMFFileOutputInPort::Connect: config->getParametersSync(sampling rate) not supported. Use default."));
		samplingRate = 8000;
	}
	else
	{
	...
	// get channel number from the media input node through port
	status = aConfig->getParametersSync(NULL, (PvmiKeyType)AUDIO_OUTPUT_NUM_CHANNELS_CUR_QUERY, kvp, numParams, NULL);
	uint32 channelNum = 0;
	if (status != PVMFSuccess || !kvp || numParams != 1)
	{
		PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMFFileOutputInPort::Connect: config->getParametersSync(channel number) not supported. Use default."));
		channelNum = 1;
	}
	else
	{
		channelNum = kvp[0].value.uint32_value;

		aConfig->releaseParameters(NULL, kvp, numParams);
	}


	// Forward settings to composer node
	iNode->SetSamplingRate(samplingRate);
	iNode->SetChannelNumber(channelNum);
	
	return PvmfPortBaseImpl::Connect(aPort);
}
