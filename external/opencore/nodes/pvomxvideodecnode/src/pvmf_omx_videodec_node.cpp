...
#undef NUMBER_OUTPUT_BUFFER
#define NUMBER_OUTPUT_BUFFER 19

#undef PVMF_OMXBASEDEC_NODE_COMMAND_VECTOR_RESERVE
#define PVMF_OMXBASEDEC_NODE_COMMAND_VECTOR_RESERVE 19

#define H264_DPB_MAX_SIZE 3110400   //for level 3.0
...
PVMFOMXVideoDecNode::PVMFOMXVideoDecNode(int32 aPriority, bool aHwAccelerated) :
        ...
        iNewHeight(0)
{
    bShowLog = Oscl_File_Cfg::LoadCfg(Oscl_File_Cfg::CFG_PLAYER, "log_videodecnode");
    ...
    OSCL_TRY(err,
			 ...
			 iCapability.iInputFormatCapability.push_back(PVMF_MIME_REAL_VIDEO); // by HungChih Lin
...
PVMFStatus PVMFOMXVideoDecNode::HandlePortReEnable()
{
    ...
    {
        LOGP("--------Format-------- = 0x%08x",iParamPort.format.video.eColorFormat);
        mColorFormat = iParamPort.format.video.eColorFormat;
        ...
        LOGD("--------Buffer Size-------- = %d", iOMXComponentOutputBufferSize);
        ...
        LOGD("PVMFOMXVideoDecNode::HandlePortReEnable() new output buffers %d, size %d", iNumOutputBuffers, iOMXComponentOutputBufferSize);
            ...
            if (fsiErrorCode == 0)
            {
                ...
                if (fsiInfo != NULL)
                {
                    ...
                    {        
                        ...
                        LOGP("!!!!! PVMFOMXVideoDecNode::HandlePortReEnable - %s !!!!!", PVMF_MIME_YUV420_PLANAR);
                    }
                    else if (mColorFormat == OMX_COLOR_FormatVendorMTKYUV)
                    {        
                        iYUVFormat = PVMF_MIME_YUV420_PLANAR_MTKYUV; // Y, U, V are separate - entire planes
                        LOGP("!!!!! PVMFOMXVideoDecNode::HandlePortReEnable - %s !!!!!", PVMF_MIME_YUV420_PLANAR_MTKYUV);
                    }
#endif                    
        ...
        LOGD("iNumOutputBuffers = %d",iNumOutputBuffers);

        /* Allocate output buffers */
        if (!CreateOutMemPool(iNumOutputBuffers))
        {
            LOGE("HandlePortReEnable() Port Reconfiguration -> Cannot allocate output buffers");
...
bool PVMFOMXVideoDecNode::NegotiateComponentParameters(OMX_PTR aOutputParameters)
{
    ...
    // check if params are OK. In case of H263 (and H.264) width/height cannot be obtained until
    // 1st frame (or SPS) is decoded, so read them from the output port.
	...
	if (iInPort && (((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H264_VIDEO || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H264_VIDEO_MP4 || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H264_VIDEO_RAW))
	{
        iYUVWidth =  pOutputParameters->width;
        iYUVHeight = pOutputParameters->height;
        LOGD("[1] FormatType = %s, nFrameWidth = %d, nFrameHeight = %d", ((PVMFOMXDecPort*)iInPort)->iFormat.getMIMEStrPtr(), iYUVWidth, iYUVHeight);
	}
    else if ((pOutputParameters->width != 0) && (pOutputParameters->height != 0) && iInPort && (((PVMFOMXDecPort*)iInPort)->iFormat != PVMF_MIME_H2631998 || ((PVMFOMXDecPort*)iInPort)->iFormat != PVMF_MIME_H2632000))
    {
        ...
        iYUVWidth  = pOutputParameters->width;
        iYUVHeight = pOutputParameters->height;
        LOGD("[2] FormatType = %s, nFrameWidth = %d, nFrameHeight = %d", ((PVMFOMXDecPort*)iInPort)->iFormat.getMIMEStrPtr(), iYUVWidth, iYUVHeight);
    }
    else
    {
        iYUVWidth =  iParamPort.format.video.nFrameWidth;
        iYUVHeight = iParamPort.format.video.nFrameHeight;
        LOGD("[3] FormatType = %s, nFrameWidth = %d, nFrameHeight = %d", ((PVMFOMXDecPort*)iInPort)->iFormat.getMIMEStrPtr(), iYUVWidth, iYUVHeight);
    }
...
#ifdef AVC_USE_EXT_BUF
    PVMFFormatType TempFormat = PVMF_MIME_FORMAT_UNKNOWN;
    uint32 MaxDPBSize;
    uint32 MaxDPBNumber;
    uint32 PicSize;
    ...
    if (iInPort != NULL)
    {
        TempFormat = ((PVMFOMXDecPort*)iInPort)->iFormat;
    }
    else
    {
        LOGE("[ERROR] iInPort = NULL");
    }

    if ((TempFormat == PVMF_MIME_H264_VIDEO) || (TempFormat == PVMF_MIME_H264_VIDEO_MP4) || (TempFormat == PVMF_MIME_H264_VIDEO_RAW))
    {
        MaxDPBSize = H264_DPB_MAX_SIZE;
        PicSize = (pOutputParameters->width*pOutputParameters->height*3)>>1;
        MaxDPBNumber = MaxDPBSize / PicSize;
        MaxDPBNumber = VIDEO_MIN(MaxDPBNumber,16);
        iNumOutputBuffers = MaxDPBNumber + 3;
        LOGD("[H264] MaxDPBNumber = %d, iNumOutputBuffers = %d", MaxDPBNumber, iNumOutputBuffers);
    }
#endif    
...
// For now, we just use the preferred one (with nIndex=0) assuming it is supported at MIO
    ...
    else if (iOMXVideoColorFormat == OMX_COLOR_FormatVendorMTKYUV)
    {        
        iYUVFormat = PVMF_MIME_YUV420_PLANAR_MTKYUV; // Y, U, V are separate - entire planes
    }
...
bool PVMFOMXVideoDecNode::InitDecoder(PVMFSharedMediaDataPtr& DataIn)
{
	...
	// Morris Yang 20100719 [
                if (!SendConfigBufferToOMXComponent(tmp_ptr, length, (size == initbufsize)))
	// ]
                {
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
    ...
    else if (Format == PVMF_MIME_REAL_VIDEO)
    {
        ...
        //PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFOMXVideoDecNode::InitDecoder() for RV Decoder. Initialization data Size %d.", initbufsize));
        LOGE ("@@ PVMFOMXVideoDecNode::InitDecoder() for RV Decoder. Initialization data Size %d.", initbufsize);
        ...
        OMX_OUT OMX_PTR aEventData)
{
    ...
    switch (aEvent)
    {
            ...
            switch (aData1)
            {
                ...
                case OMX_CommandFlush:
                {
                    ...
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_INFO,
                                    (0, "PVMFOMXVideoDecNode::EventHandlerProcessing: OMX_CommandFlush - completed on port %d", (int32)aData2));
                    ...
                    // the other port as well.
                {
                    ...
                    {
                        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_INFO,
                                        (0, "PVMFOMXVideoDecNode::EventHandlerProcessing: OMX_CommandPortEnable - completed on port %d, dynamic reconfiguration needed on port %d", (int32)aData2, iSecondPortToReconfig));
                    ...
                    {
                        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_INFO,
                                        (0, "PVMFOMXVideoDecNode::EventHandlerProcessing: OMX_CommandPortEnable - completed on port %d, resuming normal data flow", (int32)aData2));
                        iProcessingState = EPVMFOMXBaseDecNodeProcessingState_ReadyToDecode;
        ...
        case OMX_EventError:
        {
            ...
            LOGE("Ln %d OMX_EventError nData1 %d nData2 %d", __LINE__, (int32)aData1, (int32)aData2);
        ...
        case OMX_EventPortSettingsChanged:
        {
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                                (0, "PVMFOMXVideoDecNode::EventHandlerProcessing: OMX_EventPortSettingsChanged returned for port %d, dynamic reconfig already in progress", (int32)aData1));
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                                (0, "PVMFOMXVideoDecNode::EventHandlerProcessing: OMX_EventPortSettingsChanged returned for port %d", (int32)aData1));
...
bool PVMFOMXVideoDecNode::QueueOutputBuffer(OsclSharedPtr<PVMFMediaDataImpl> &mediadataimplout, uint32 aDataLen)
{
    ...
    if (OsclErrNone == leavecode)
    {
        ...
        if (fsiErrorCode == 0)
        {
            ...
            if (!strcmp((char*)iName.Str(), "PVMFOMXVideoDecNode"))
            {
                OsclProfile::Push(OsclProfile::iQueueOutputBufferInTime, iSeqNum - 1);
            } 
            ...
            if (numvalentries > starting_index)
            {
                ...
                else
                {
                 ...
                 (((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H2631998 || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H2632000 ||
                  ...
                  || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_REAL_VIDEO) // HungChih Lin
...

        {
            ...
            if (iAvgBitrateValue > 0)
                ++numvalentries;
                 ...
                 (((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_WMV || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_M4V || 
                 ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H2631998 || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H2632000 || 
                 ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H264_VIDEO || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H264_VIDEO_MP4 || 
                 ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_H264_VIDEO_RAW || ((PVMFOMXDecPort*)iInPort)->iFormat == PVMF_MIME_REAL_VIDEO) // HungChih Lin
                 )
        {
            // Format
            ++numvalentries;
    ...
    for (int32 paramind = 0; paramind < aNumElements; ++paramind)
    {
        ...
        if ((pv_mime_strcmp(compstr, _STRLIT_CHAR("x-pvmf/video/decoder")) < 0) || compcount < 4)
        {
            ...
            else
            {
                ...
                {
                    aInputParameters.cComponentRole = (OMX_STRING)"video_decoder.rv";
                }
