...
#define LOG_TAG "PVMF_MEDIA_INPUT_NODE_OUTPORT"
#include <utils/Log.h>
...
#define PVMIO_MEDIADATA_POOLNUM 20 //9
...
void PvmfMediaInputNodeOutPort::Start()
{
    ...
    PvmfPortBaseImpl::SetReserve(EPVIncomingDataQueue, 0);
    PvmfPortBaseImpl::SetThreshold(EPVIncomingDataQueue, 0);
    PvmfPortBaseImpl::SetCapacity(EPVOutgoingDataQueue, 20);
    PvmfPortBaseImpl::SetReserve(EPVOutgoingDataQueue, 20);
    PvmfPortBaseImpl::SetThreshold(EPVOutgoingDataQueue, 70);
        ...
        OsclAny* context)
{
    ...
    if (iWriteState == EWriteOK)
    {
        ...
        else if (PVMI_MEDIAXFER_FMT_TYPE_DATA == format_type)
        {
            ...
            //LOGD("TEST, seq_num:%d, iFormat:%s, len:%d, first4:%x", data_header_info.seq_num, iMimeType.get_cstr(), data_len, *((uint32 *)data));
            if(oscl_strncmp(iMimeType.get_cstr(), PVMF_MIME_M4V, oscl_strlen(PVMF_MIME_M4V)) == 0)
            {
                if(0 == data_header_info.seq_num)
                {
                    ...
                    volHeader.len = data_len; // vol header size should be (28)
                    ...
                    oscl_memcpy(volHeader.ptr, data, data_len);
#else             
                    ...
                    volHeader.ptr = NULL;
                    //volHeader.len = aBuffer->nFilledLen; // vol header size should be (28)
                    volHeader.len = data_header_info.vos_data_length; // vol header size should be (28)
                    uint8* memBuffer = (uint8*)iAlloc.allocate(refCounterSize + volHeader.len);
                    oscl_memset(memBuffer, 0, refCounterSize + volHeader.len);
                    OsclRefCounter* refCounter = OSCL_PLACEMENT_NEW(memBuffer, OsclRefCounterDA(memBuffer, (OsclDestructDealloc*) & iAlloc));
                    memBuffer += refCounterSize;
                    ...
                    oscl_memcpy(volHeader.ptr, data_header_info.vos_data_ptr, volHeader.len);
#endif               
                    // save in class variable
                    iVolHeader = OsclRefCounterMemFrag(volHeader, refCounter, volHeader.len);
                    LOGD("VOS in, seq_num:%d, iFormat:%s, len:%d, first4:%x", data_header_info.seq_num, iMimeType.get_cstr(), volHeader.len, *((uint32 *)volHeader.ptr));
                    mediaData->setFormatSpecificInfo(iVolHeader);
                    /*
                    if (iState == PvmfMediaInputNodeOutPort::PORT_STATE_STARTED)
                        RunIfNotReady();
                    return iCmdId++;
                ...
                /*
                else if(1 == data_header_info.seq_num)
                {
                     mediaData->setFormatSpecificInfo(iVolHeader);
                }
                mediaData->setSeqNum(data_header_info.seq_num-1);
                */
            }
            
            uint32 marker_bits = mediaData->getMarkerInfo();
		        if (data_header_info.iEndOfFrameFlagOut != 0)
		        {
		            marker_bits |= PVMF_MEDIA_DATA_MARKER_INFO_M_BIT;
		        }		
		        if (data_header_info.iKeyFrameFlagOut != 0)
		        {
		            marker_bits |= PVMF_MEDIA_DATA_MARKER_INFO_RANDOM_ACCESS_POINT_BIT;
		        }		
		        // attach marker bit
		        mediaData->setMarkerInfo(marker_bits);
