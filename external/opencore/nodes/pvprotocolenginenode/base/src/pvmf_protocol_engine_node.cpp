...
bool PVMFProtocolEngineNode::ProcessCommand(PVMFProtocolEngineNodeCommand& aCmd)
{
    ...
    //workaround for ALPS136685
    if (aCmd.iCmd == PVMF_GENERIC_NODE_STOP)
    {
        while (!iCurrentCommand.empty())
        {
            CancelClear();
            LOGE("ProtNode::ProcessCommand()--Warning: there is cmd when attemping to stop!");
            CommandComplete(iCurrentCommand, iCurrentCommand[0],PVMFErrCancelled);  
            LOGE("\t\t\t cmdid = %d canceled", iCurrentCommand[0].iCmd); 
        }
    }  
...
void PVMFProtocolEngineNode::SetCookie(OSCL_String& aCookie)
{
    if (iProtocolContainer) iProtocolContainer->setCookie(aCookie);
}
// --->
...
void PVMFProtocolEngineNode::forceResumeNotification()
{
    LOGINFODATAPATH((0, "PVMFProtocolEngineNode::forceResumeNotification"));
    LOGE("@info: PVMFProtocolEngineNode::forceResumeNotification");
    if (iDownloadControl != NULL)
    {
        iDownloadControl->forceResumeNotification();
    }
}
