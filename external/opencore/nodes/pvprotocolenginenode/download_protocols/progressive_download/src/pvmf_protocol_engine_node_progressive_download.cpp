        ...
        const uint32 aFileSize)
{
    ...
    uint32 aNPTInMS = 0;
    uint32 BufferTime = OSCL_MIN(PVPROTOCOLENGINE_JITTER_BUFFER_SIZE_TIME, (aDurationMsec>>5)/1000); //aDuration>>5 = 0.03125 = 3.125%
    BufferTime = OSCL_MAX(BufferTime, 1);//must not be 0
    ...
    if (iProgDownloadSI->convertSizeToTime(aCurrDownloadSize, aNPTInMS) == 0)
    {
        ...
        if (diffByterate < GET_10_PERCENT(iClipByterate) || // OSCL_ABS(aInstantByterate-iClipByterate)/iClipByterate < 1/8-1/64=0.109
                isBufferingEnoughTime(aCurrDownloadSize, BufferTime, aNPTInMS))  //PVPROTOCOLENGINE_JITTER_BUFFER_SIZE_TIME, aNPTInMS))
        {
            if (isBufferingEnoughTime(aCurrDownloadSize, BufferTime, aNPTInMS))  //PVPROTOCOLENGINE_JITTER_BUFFER_SIZE_TIME, aNPTInMS))
            {
                return 0;
            }
            return 1;
    ...
    else
    {
        ...
        if (isBufferingEnoughTime(aCurrDownloadSize, BufferTime/*PVPROTOCOLENGINE_INIT_DOWNLOAD_TIME_THRESHOLD_WITH_CLIPBITRATE*/)) return 0;
    }
        ...
        uint32 &aPlaybackRemainingTimeMsec)
{
    ...
    uint32 aNPTInMS = 0;
    uint32 BufferTimeInMs = OSCL_MIN(PVPROTOCOLENGINE_JITTER_BUFFER_SIZE_TIME*2000, (aDurationMsec>>5)); //aDuration>>5 = 0.03125 = 3.125%
    if (iProgDownloadSI->convertSizeToTime(aCurrDownloadSize, aNPTInMS) == 0)
    {
        ...
        if (aNPTInMS > BufferTimeInMs/*PVPROTOCOLENGINE_JITTER_BUFFER_SIZE_TIME*2000*/ + aPlaybackTimeMsec)
        {
