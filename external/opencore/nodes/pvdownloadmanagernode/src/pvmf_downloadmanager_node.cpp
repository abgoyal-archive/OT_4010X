...
// <--- Morris Yang ALPS00221177 add for m.youtube.com
    {"http-cookie", PVMI_KVPTYPE_VALUE, PVMI_KVPVALTYPE_CHARPTR},
// --->
...
// <--- Morris Yang ALPS00221177 add for m.youtube.com
    BASEKEY_SESSION_CONTROLLER_HTTP_COOKIE,
// --->
...
// <--- Morris Yang ALPS00133130
    if (iPlayBackClock && (iPlayBackClock->getAVSyncCtrl() != NULL))
    {
        iPlayBackClock->getAVSyncCtrl()->SetAccurateMediaSkipControlForAudio(true);
    }
// --->
    ...
    else
    {
        switch (aEvent.GetEventType())
        {
            ...
            case PVMFInfoDownloadingMoov:
                iContainer->iIgnoreMoovDataReady = true;
                filter = true;
                break;
            case PVMFInfoDownloadedMoov:
                iContainer->iIgnoreMoovDataReady = false;
                filter = true;
                break;
            // --->
            ...
            case PVMFInfoBufferingStatus:
                if (iContainer->iIgnoreMoovDataReady) {
                    if (aEvent.GetLocalBuffer()[0] == 100)
                        iContainer->iIgnoreMoovDataReady = false;
                    filter = true;
                }
                break;
        ...
        if (pv_mime_strcmp(compstr, _STRLIT_CHAR("net")) >= 0)
        {
            if (compcount == 3)
            {
                ...
                switch (i)
                {
                    ...
                    break;
                // <--- Morris Yang ALPS00221177 add for m.youtube.com
                    case BASEKEY_SESSION_CONTROLLER_HTTP_COOKIE:
                    {
                        OSCL_HeapString<OsclMemAllocator> cookie;
                        cookie = aParameters[paramind].value.pChar_value;
                        (iProtocolEngineNode.ProtocolEngineExtension())->SetCookie(cookie);
                    }
                    break;
                // --->
