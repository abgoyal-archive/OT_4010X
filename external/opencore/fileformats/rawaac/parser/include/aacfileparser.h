...
// sam sun for aac seek table gen--->
#define AAC_RP_TABLE_SIZE                			256 /* the max size of the iPRTable */ 
#define DEFOULT_FRAME_BETWEEN_TWO_ADJACENT_POINT	32
#define NUM_DATA_BLOCK								1 // for estimate ADTS duration
//	sam sun for aac rough duration --->
#define NUM_RANDOM_FRAMES							100 // for estimate ADTS duration 
#define NUM_REMOVE_FRAMES							30 // remove first 30 frames
// <---sam sun for aac rough duration 
...
{
    GENERIC_ERROR     = -4,
    AAC_INSUFFICIENT_DATA = -3, // sam sun for aac rough duration
    FILE_OPEN_ERROR = -2,
    MEMORY_ERROR    = -1,
    OK              = 1
};
...
// sam sun for aac seek table gen --->
typedef Int (*PPVMP4AudioDecodeFrameTwo)(tPVMP4AudioDecoderExternal*, void*);
typedef Int (*PPVMP4AudioDecoderConfig)(tPVMP4AudioDecoderExternal*, void*);
typedef Int (*PPVMP4AudioDecoderResetBuffer)(void*);
// <---sam sun for aac seek table gen 
...
class AACBitstreamObject
{
    public:
        ...
        */
        AACBitstreamObject(PVFile* file)
        {
			...
			ipMem = NULL;
			ipHandle = NULL;
			ipPVMP4AudioDecoderConfig = NULL;
			ipPVMP4AudioDecodeFrameTwo = NULL;
			ipPVMP4AudioDecoderResetBuffer = NULL;
			//  <---sam sun for aac seek table gen
        }
	...
	// sam sun for aac seek table gen--->
	int32 ParseAADTSFrame(int32 &aDirtyDataLen, int32 &aFrameLen);
	bool InitDecLibrary(uint32 aDesiredChannels);
	void ResetDecoder(); // for repositioning
	void DeinitAacDec();
	Int AacDecodeFrames(int16* aOutputBuffer,
                         uint32* aOutputLength, uint8** aInBuffer,
                            uint32* aInBufSize, uint32* aIsFirstBuffer,
                            uint32* aSamplesPerFrame);
	Int RetrieveDecodedStreamType();
	bool InitAACDec();
	int32 ParseAADIFFrame(int32 &aDirtyDataLen, int32 &aFrameLen);
	// sam sun for aac seek table gen --->
    ...
    private:
		...
		// sam sun for aac seek table gen --->
		int16 *iOutputBuffer;
        uint32 iOutputLength; 
        uint8 *iInBufferStart;
	    uint8 *iInBuffer;	
        uint32 iInBufSize; 
        uint32 iFrameNum;
        uint32 iSamplesPerFrame;
		bool ibFirstParseADIF;
		bool ibNewInputDataNeeded;

		uint32 iAacInitFlag;
        uint32 iInputUsedLength;

        void*    ipMem;
        tPVMP4AudioDecoderExternal  iExt;
        uint32 iNumOfChannels;
        uint32 iMemReq;
        uint32 iConfigUpSamplingFactor;

		void* ipHandle;
		PPVMP4AudioDecodeFrameTwo ipPVMP4AudioDecodeFrameTwo;
		PPVMP4AudioDecoderConfig  ipPVMP4AudioDecoderConfig;
		PPVMP4AudioDecoderResetBuffer ipPVMP4AudioDecoderResetBuffer;
		// sam sun for aac seek table gen --->
...
class CAACFileParser
{
	...
	// sam sun for aac seek table gen--->	
		OSCL_IMPORT_REF bool InitAACParserFile(OSCL_wString& aClip,  bool aInitParsingEnable = true, Oscl_FileServer* aFileSession = NULL, PVMFCPMPluginAccessInterfaceFactory* aCPMAccess = NULL, OsclFileHandle* aHandle = NULL);
		OSCL_IMPORT_REF void DeInitAACParserFile();
		OSCL_IMPORT_REF int32 ParseAACFile(int32 aParseNum);
		OSCL_IMPORT_REF void CalculateDuration();
		OSCL_IMPORT_REF bool IsADTS();
		OSCL_IMPORT_REF bool IsAACRPTableCreated();
	// <--- sam sun for aac seek table gen
    ...
    private:
		...
		// sam sun for aac seek table gen --->
		PVFile      iAACParserFile;
		AACBitstreamObject *ipParserBSO;
		uint32       iFrameNum;
		int32       iFilePos;
		int32 	    iFrameCount;
	    int32       iAACRPTableEntryCnt;  		        // for seek table entry count
        uint32      iFrameNumBetweenTwoAdjacentPoint;   // the number of frames between two adjacent point in the seek table(frame num)
		bool		ibAACRPTableCreated;
	    // <--- sam sun for aac seek table gen
