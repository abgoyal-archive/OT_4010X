...
class MediaAtom : public Atom
{
    ...
    public:
        ...
        int32 resetPlayBack(int32 time, bool oDependsOn)
        {
            if (_pmediaInformation != NULL)
            {
            // Morris Yang add sync window to improve video seek performance (ALPS00017047) [
                #if PARSER_SYNC_WINDOW
                    return _pmediaInformation->resetPlayBack(time, getMediaTimescale(), oDependsOn);
                #else
...
// Morris Yang add sync window to improve video seek performance (ALPS00017047) [
#if PARSER_SYNC_WINDOW
        int32 resetPlayBack(int32 time, uint32 timescale, bool oDependsOn)
        {
            if (_pmediaInformation != NULL)
            {
                return _pmediaInformation->resetPlayBack(time, timescale, oDependsOn);
            }
            else
            {
                return 0;
...
// ]
                ...
                uint32 sampleOffset = 0)
        {
            ...
            {
                LOGE ("READ_FAILED at %s, line %d", __FILE__, __LINE__);
                return (READ_FAILED);
            }
        }
