...
#define AMRRP_TABLE_SIZE              256 /*Max number of rt table */ 
#define PVAMRFF_DEFAULT_NUM_OF_FRAMES  80
#define PVAMRFF_DURATION_SCAN_AO_DELAY 1000
#define AMR_SAMPLE_FOR_AVGDURATION    200 /*frame count to read for evulate duration*/
#define AMR_SEEK_NODEDATAQUEUINGTIMEOUT_DEF (5*60*1000)  /* reference PVPLAYERENGINE_CONFIG_NODEDATAQUEUINGTIMEOUT_DEF 5*60*1000  in pv_player_config.h */
...
typedef enum
{
    ...
    AMR_FILE_READ_ERR = 4,
    AMR_ERR_NO_MEMORY = 9,
    AMR_NO_SYNC_FOUND = 10,
    AMR_FILE_OPEN_ERR = 11,
    AMR_DURATION_PRESENT = 16
} AMRErrorType;
//----------------------------------------------------------------------------
...
class bitstreamObject
{
    public:
        ...
        */
        ...
        /**
        * @brief Returns badBytes after getNextFrame(), used when build Random points table to skip bad bytes
        *
        * @param None
        * @returns iBadBytes
        */
	 ...
	 /**
        * @brief Returns current Head size,For IETF bitstream, iInitFilePos = IETF header size
        *
        * @param None
        * @returns  iInitFilePos
        */
        uint32  geInitFilePos()
        {
            return iInitFilePos;
        }
...
class CAMRFileParser
{
    public:
	 ...
	 /**
        ...
        * @param void
        * @returns iAMRDuration.
        */
	OSCL_IMPORT_REF	int32 GetDuration(){return iAMRDuration;}

 	/**
        * @brief  skip some frames in order to reach to acturate frame when seek .
        *
        * @param numtoskip num of frames to skip
        * @param aacturalSkip    actural num of frames that  skiped.
        * @returns Result of operation: EVERYTHING_OK, READ_FAILED, etc.
        */	
	OSCL_IMPORT_REF	int32 SkipFramesFromCurrent(int32 numtoskip, int32 & aacturalSkip);

	/**
        * @brief  scan Amr File to calculate acturate duration and build up random points table .
        *
        * @param aFrametoScan is num of frames  to scan every time. 
        * @param pAssistParser  ,ppAssistParser is pointer to pointer of  AMRFileparser which assist building random points table.
                    this AMRFileparser open the same file with the main AMRFileparser who call this function, and main fileparser just use assist parser to get every frame. 
        * @returns AMRErrorType.
        */	
	OSCL_IMPORT_REF	AMRErrorType  ScanAmrFile(int32 aFrametoScan,CAMRFileParser **ppAssistParser=NULL);


	/**
        * @brief  in ored to update countToClaculateRDATimeInterval from PVMFAMRParserNode.
        *
        * @param pointer to countToClaculateRDATimeInterval
        * @returns void.
        */	
	OSCL_IMPORT_REF	void SetCountInterval( uint32 * aCountInterval);
	...
	/**
        * @brief Resets the parser variables to allow start of playback at a new rough position
          *
        * @param aStartTime Time where to start playback from
        * @returns Result of operation: EVERYTHING_OK, READ_FAILED, etc.
        */
        OSCL_IMPORT_REF int32 ResetRoughPlayback(int32 aStartTime = 0);
	...
	int32 iAmrRPTableEntryCnt;  // for seek table entry count
	int32 iTempDuration;
	int32 iTempPos;
	bool  ibDurationCompleted;
	bool iFirstScan;
	int32 iFrameIntervalLeft;
	uint32 *iOutClaculateRDATimeInterval;
