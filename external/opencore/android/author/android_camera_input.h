...
#define ANDROID_VIDEO_FORMAT       PVMF_MIME_YUV422_INTERLEAVED_YUYV
    ...
    #define ANDROID_VIDEO_FORMAT_H263       PVMF_MIME_H2632000
    #define ANDROID_VIDEO_FORMAT_M4V        PVMF_MIME_M4V
  #else
...
//Start to modify by Mingliang.Zhong@2010-09-14(ISSUE:alps00121109)
class CameraInputMIONotify
{
public:
	CameraInputMIONotify(){}
	virtual ~CameraInputMIONotify(){}
	
	virtual void _CameraInputMIOCallback(void* p) = 0;
...
public:
	//Start to modify by Mingliang.Zhong@2010-09-14(ISSUE:alps00121109)
     void Check1stVideoFrameWrite(CameraInputMIONotify* pCallback,void*p);
 ...
 private:
 	bool iCheck1stVideoFrameWrite;
	void* iCheck1stVideoFrameWriteParams;
      bool i1stFramesend; 
	int32  iFrameCounter; 
      CameraInputMIONotify* ipCameraInputMIONotify;
	//End to modify by Mingliang.Zhong@2010-09-14(ISSUE:alps00121109)	  
...
private:
    ...
    nsecs_t    iStartTickCount;
    uint32     iAVTSOffset;
//uint32     iVideoFirstFrameTs;
    struct timeval iVideoFirstFrameTs;
	...
	sp<IMediaRecorderClient>  mNotifyListener;
...
char cVosData[128];
uint32 u4VosLen;
 sp<IMemory> VosBuf[16];
PVMFStatus postWriteAsyncSaveVos(const sp<IMemory>& frame);
PVMFStatus postWriteAsyncSkipVop(const sp<IMemory>& frame);
PVMFStatus postWriteAsyncOutputVos();
PVMFStatus SaveVos(const sp<IMemory>& frame);

uint32 u4NotReadyCnt;
