...
static const int kBufferSize = 640;//2048;
...
extern bool bCompressVideoStart;
extern bool bCompressVideoReady;
extern sp<android::Camera>     mCameraExtra;
extern bool bCompressVideoHasAudio;

static bool bCameraStart;
...
int AndroidAudioInput::audin_thread_func() {
...
#if 1
     // add by chipeng , adjust Audiooutput priority
     /*
     char thread_name[20]="AudioPolicyService";
     int result = 0;
     result = mtk_AdjustPrio(thread_name);	// 設定成功 return 0; 失敗 return -1
     LOGW("AudioCommandThread::threadLoop set priority = %d",result);
     */
     int result = -1;
     ...
     if(result == -1){
         ...
         sched_p.sched_priority = RTPM_PRIO_AUDIO_IN; // 40
         if(0 != sched_setscheduler(0, SCHED_RR, &sched_p)) {
 	    LOGE("[%s] failed, errno: %d", __func__, errno);
         }
        else {
            sched_p.sched_priority = RTPM_PRIO_AUDIO_IN;
            sched_getparam(0, &sched_p);
    ...
    LOGD("create AudioRecord %p", this);
    AudioRecord
    ...
    LOGD("AudioRecord created %p, this %p", record, this);
    ...
    if (res == NO_ERROR) {
        ...
        while (!iExitAudioThread) {
            ...
            if (iFirstFrameReceived == false) {
                ...
                LOGD("First Audio Frame received systime %d, recordLatency %d, iFirstFrameTs %d", systime, recordLatency, iFirstFrameTs);
            }
            ...
            if(false == bCompressVideoReady)
						{
							LOGD("AUDIO SKIP");								
	            MicData micdata(data, 0, iTimeStamp,
	                    dataDuration);
	            iWriteResponseQueueLock.Lock();
	            iWriteResponseQueue.push_back(micdata);
	            iWriteResponseQueueLock.Unlock();  
              if(false == bCameraStart && true == bCompressVideoStart && mCameraExtra!=0 && iAuthorClock->GetState() == PVMFMediaClock::RUNNING)
              {
                  if (mCameraExtra->startRecording() != NO_ERROR)
                  {
                  ...
                  {
                  	  LOGD("mCamera start recording OK");
                  }
                  bCameraStart = true;
              }
						}
        ...
        LOGD("record->stop %p, this %p", record, this);
        record->stop();
    }

    LOGD("delete record %p, this %p", record, this);
    delete record;
