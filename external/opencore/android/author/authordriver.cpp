...
extern bool bCompressVideoReady;
extern bool bIsMp4;
...
{
    int error = 0;
    LOGD("handleSetAudioSource:%d", ac->as);
    ...
    if (mAudioInputMIO != NULL) {
        ...
        if (mVideoInputMIO != NULL) {
            if(ac->as != AUDIO_SOURCE_MATV) {		//if it is MATV, do not do this force operation
               mAudioInputMIO->setAudioSource(AUDIO_SOURCE_CAMCORDER);
               LOGD("set force to CAMCORDER");
            }
        }
    }
...
void AuthorDriver::handleSetVideoSource(set_video_source_command *ac)
{
    ...
    case VIDEO_SOURCE_CAMERA: {
		...
		if(mListener.get()){
			cameraInput->SetNotifyListener(mListener);
		}
        if (cameraInput) {
...
//	 LOGD("hphp:AuthorDriver::handleSetOutputFormat, mComposerMimeType = /x-pvmf/ff-mux/wav");
        mComposerMimeType = "/x-pvmf/ff-mux/wav";
        break;
...
void AuthorDriver::handleSetAudioEncoder(set_audio_encoder_command *ac)
{
    ...
    case AUDIO_ENCODER_PCM:
        ...
        iAudioEncoderMimeType = "/x-pvmf/audio/encode/pcm";

//	 LOGD("hphp:AuthorDriver::handleSetAudioEncoder, set iAudioEncoderMimeType = /x-pvmf/audio/encode/pcm");

        // if mSamplingRate is zero, it means it is not set, use default = 8kHz
        if (mSamplingRate == 0)
        {
            mSamplingRate = 8000;
        }
        // if it is set, but out of range, make it error. we don't limit the sample rate to 8kHz or 16kHz
        else if (mSamplingRate < MIN_AUDIO_SAMPLING_RATE ||
                    mSamplingRate > 16000)
        {
            LOGE("Valid sampling rate for WAV: %d to %d \n",MIN_AUDIO_SAMPLING_RATE, MAX_AUDIO_SAMPLING_RATE);
            commandFailed(ac);
            return;
        }

        // WAV only supports mono (IE 1 channel)
        if (mNumberOfChannels == 0)
        {
            // Number of channels not set, use the default
            mNumberOfChannels = 1;
        ...
        {
            LOGE("Only valid number of channels for WAV is 1.");
            commandFailed(ac);
            return;
        }
        break;
    ...
    case AUDIO_ENCODER_ADPCM:
        ...
        iAudioEncoderMimeType = "/x-pvmf/audio/encode/adpcm";

//	 LOGD("hphp:AuthorDriver::handleSetAudioEncoder, set iAudioEncoderMimeType = /x-pvmf/audio/encode/adpcm");
...
void AuthorDriver::handleSetVideoEncoder(set_video_encoder_command *ac)
{
    ...
    if (mVideoInputMIO) {
        ...
        clipVideoBitrate();
        ((AndroidCameraInput *)mVideoInputMIO)->SetBitRate(mVideo_bitrate_setting);
    }
...
void AuthorDriver::handleSetOutputFile(set_output_file_command *ac)
{
    ...
    // OUTPUT_FORMAT_WAV added by HP cheng
    if (( OUTPUT_FORMAT_AMR_NB == mOutputFormat ) || ( OUTPUT_FORMAT_AMR_WB == mOutputFormat ) ||
        ...
        || ( OUTPUT_FORMAT_WAV == mOutputFormat ) ) {
...
PVMFStatus AuthorDriver::setMaxDurationOrFileSize(
    ...
    if (limit_is_duration) {
        ...
        if(limit > 16*1024)
            ...
            LOGW("setMaxDurationOrFileSize(), limit too small!!");       
        ret = durationConfig->SetMaxFileSize(
                limit > 0, static_cast<uint32>(limit) - reserve);
    }
    LOGD("setMaxDurationOrFileSize, limit_is_duration:%d, limit:%u", limit_is_duration, static_cast<uint32>(limit));
...
PVMFStatus AuthorDriver::setParameter(
	...
	else if(key == "video-param-camera-digizoom") {
        int64_t digizoom;
		LOGD(" key =  video-param-camera-digizoom ) \n");
        if (safe_strtoi64(value.string(), &digizoom)) {
            if(mCamera.get())
            {
			String8 string8 = mCamera->getParameters();
			CameraParameters camerapara(string8);
			camerapara.set(CameraParameters::KEY_ZOOM,(int)digizoom);				  
			string8 = camerapara.flatten();
			if(mCamera->setParameters(string8) == NO_ERROR)
			{
			       return PVMFSuccess;
			}
            }// end of  if(mCamera.get())
        	}
	}
	else if(key == "video-param-camera-smoothzoom") {
        int64_t digizoom;
		LOGD(" key =  video-param-camera-digizoom ) \n");
        if (safe_strtoi64(value.string(), &digizoom)) {
            if(mCamera.get())
            {
			if(mCamera->sendCommand(CAMERA_CMD_START_SMOOTH_ZOOM,digizoom,0) == NO_ERROR)
			{
			       return PVMFSuccess;
			}
            }// end of  if(mCamera.get())
...
//Start to modify, Mingliang.Zhong @2010-09-14 (ISSUE:alps00121109)
void AuthorDriver::_CameraInputMIOCallback(void* p)
{
	author_command *ac = (author_command *)p;

	handleFinalStop(ac);
}

void AuthorDriver::handleFinalStop(author_command *ac)
{
    LOGE("handleFinalStop");
    int error = 0;
...
void AuthorDriver::handleStop(author_command *ac)
{
	if(mVideoInputMIO)
	{
   ((AndroidCameraInput *)mVideoInputMIO)->Check1stVideoFrameWrite(this,ac);
}
	else
	{
		handleFinalStop(ac);
...
//End to modify, Mingliang.Zhong @2010-09-14 (ISSUE:alps00121109)
...
/*static*/ int AuthorDriver::startAuthorThread(void *cookie)
{
    ...
    // adjust author priority
    struct sched_param sched_p;
    sched_getparam(0, &sched_p);
    sched_p.sched_priority = RTPM_PRIO_PV_AUTHOR; //40;  // 40: temp solution, we will manage the priority in a general files
    ...
    if (0 != sched_setscheduler(0, SCHED_RR, &sched_p)) {
        ...
        sched_getparam(0, &sched_p);
        LOGD("Author Driver sched_setscheduler ok, priority: %d", sched_p.sched_priority);
    }
...
//--->Set video bitrate here! added by mingliang.zhong @2010-09-02.                   
                   else
                 {
#ifdef COMPRESS_INPUT
                   if(mCamera.get())
                   {
                            LOGD("!!!!!!!!!!Set Camera OutputBitRate");
                            String8 string8 = mCamera->getParameters();
                            CameraParameters camerapara(string8);
                            int ivalue = mVideo_bitrate_setting;
//                            if(ivalue >= 3400000) ivalue = 3400000;
//                            if(ivalue <= 192000)  ivalue = 192000; 
			    LOGD("!!!!!!!!!!, in Authordriver, bitrate setvalue:%d, mVideoEncoder:%d\n", ivalue, mVideoEncoder);
                            camerapara.setVideoEncBitrate(ivalue);
                            if(mVideoEncoder == VIDEO_ENCODER_H263)
                            {
                                camerapara.setVideoEncFormat(0);
                             }
                            else if(mVideoEncoder == VIDEO_ENCODER_MPEG_4_SP)
                            {
                                camerapara.setVideoEncFormat(1);
                             }

                            string8 = camerapara.flatten();
                            if(mCamera->setParameters(string8) != NO_ERROR)
                            {
                                    LOGD("!!!!!!!!!!Failed to set camera(%p) parameters", mCamera.get());
                            }
                }//end of if(mCamera->get())

#endif //COMPRESS_INPUT
                 }
//Set video bitrate here! added by mingliang.zhong @2010-09-02--->end.   
...
status_t AuthorDriver::setListener(const sp<IMediaRecorderClient>& listener) {
	...
	AndroidCameraInput *cameraInput = static_cast<AndroidCameraInput *>( mVideoInputMIO);
	if(cameraInput){
			cameraInput->SetNotifyListener(mListener);
	}
    return android::OK;
