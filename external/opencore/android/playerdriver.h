...
#include <media/Metadata.h> // for android::media::Metadata::Filter
#include <binder/Parcel.h> // for android::Parcel
...
// <--- Demon Deng
class PlayerGetMetadata: public PlayerCommand
{
  public:
    PlayerGetMetadata(const android::media::Metadata::Filter& ids, android::Parcel* records, media_completion_f cbf, void* cookie) :
            PlayerCommand(PLAYER_GET_METADATA, cbf, cookie), mIds(ids), mRecords(records) {}
    const android::media::Metadata::Filter& ids() const { return mIds; }
    android::Parcel* records() const { return mRecords; }
  private:
    PlayerGetMetadata();
    const android::media::Metadata::Filter& mIds;
    android::Parcel* mRecords;
};
// --->

// User data definition
#define PLAYER_DRIVER_STOPPED_RECEIVED (1 << 0)
