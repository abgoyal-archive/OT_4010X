...
#define LOG_TAG "RTSPEngine"
#include <cutils/log.h>
#include <cutils/properties.h>
#define RTSPENGINE_LOG_OPS        (1 << 0)
...
OSCL_EXPORT_REF PVRTSPEngineNode::PVRTSPEngineNode(int32 aPriority) :
        ...
        // <--- Demon Deng ALPS00138992, change all timeout to 14s to avoid ANR
        TIMEOUT_WATCHDOG(14),
        // --->
        TIMEOUT_WATCHDOG_TEARDOWN(2),
        TIMEOUT_WATCHDOG_PLAY(10),
        ...
        iExtensionInterface(NULL)
{
    ...
    iInterfaceState = EPVMFNodeCreated;
     ...
     property_get("rtspeng.logops", value, "0");
     bool _res = atoi(value);
     if (_res) _uDbgFlags |= RTSPENGINE_LOG_OPS;
}
...
OSCL_EXPORT_REF PVMFStatus PVRTSPEngineNode::GetServerInfo(PVRTSPEngineNodeServerInfo& aServerInfo)
{
    ...
    aServerInfo.iDisableFireWallPackets = iSessionInfo.iDisableFireWallPackets;
    // --->
    return PVMFSuccess;
}
...
// <--- Morris Yang ALPS00225312 Handle the server shutdown
        if ((REQ_SEND_SOCKET_ID == aId) && (EPVSocketFailure == aEvent))
        {
            LOGE ("[Warning] RTSP Server was gone!!!");
...
#if 1
	     if (_uDbgFlags & RTSPENGINE_LOG_OPS)
            {
                uint8* dbgBuf = OSCL_ARRAY_NEW(uint8, incomingMessageLen+1);
                if (dbgBuf) oscl_memcpy(dbgBuf, recvData, incomingMessageLen);
                dbgBuf[incomingMessageLen] = '\0';
                LOGE ("[S->C]\n=============================================================================================================");
                LOGE ("%s=============================================================================================================", dbgBuf);
// <--- Morris Yang 20100706 dump whole ops to file
			char path[PATH_MAX];
                     char* _op_buf = new char[(incomingMessageLen+1)+1024];
			sprintf (path, "/data/rtsp_op.log");
			sprintf(_op_buf, "[S->C]\n=============================================================================================================\n%s\n"
				                 "=============================================================================================================\n", dbgBuf);
			FILE* _rtsp_log_fp = NULL;
			_rtsp_log_fp = fopen(path, "a");
			
			if (_rtsp_log_fp)
			{
			 	fwrite(_op_buf, 1, strlen(_op_buf)+1, _rtsp_log_fp);
			 	fclose (_rtsp_log_fp);
			 	_rtsp_log_fp = NULL;
			}
			delete [] _op_buf;
...
PVMFStatus PVRTSPEngineNode::DispatchCommand(PVRTSPEngineCommand& aCmd)
{
    ...
    if (iRet != PVMFPending)
    {
        if (iRet != PVMFSuccess)
        {
            ...
            {
                // <--- Demon Deng collect more information about rtsp error
                LOGE("RTSPEngine: iCurrentErrorCode %d\n", iCurrentErrorCode); 
                // --->
...
PVMFStatus PVRTSPEngineNode::SendRtspPlay(PVRTSPEngineCommand &aCmd)
{
    ...
    switch (iState)
    {
        ...
        case PVRTSP_ENGINE_NODE_STATE_PAUSE_DONE:
        {
            ...
            //setup the watch dog for server response
        // <--- Morris Yang ALPS00225307 For some error bitstream the server does not respond RTSP 200 OK for the PLAY request (seek)
        // Change the RTSP PLAY response timeout to 10 seconds to avoid ANR
            //iWatchdogTimer->Request(REQ_TIMER_WATCHDOG_ID, 0, TIMEOUT_WATCHDOG);
            iWatchdogTimer->Request(REQ_TIMER_WATCHDOG_ID, 0, TIMEOUT_WATCHDOG_PLAY);
        // --->
            RunIfNotReady();
            break;
        }
            ...
            for (uint32 i = 0; i < iSessionInfo.iSelectedStream.size(); i++)
            {
                if (!oscl_strncmp(tmpOutgoingMsg->originalURI.c_str(), iSessionInfo.iSelectedStream[i].iMediaURI.get_cstr(), tmpOutgoingMsg->originalURI.length()))
                {
                    if (iIncomingMsg.numOfTransportEntries)
                    {
                        ...
                        // <--- Demon Deng ALPS00028091
                        if (iIncomingMsg.transport[0].client_portIsSet)
                        {
                            LOGD("client_port in response %d, request %d", iIncomingMsg.transport[0].client_port1, iSessionInfo.iSelectedStream[i].iCliRTPPort);
                            if (iSessionInfo.iSelectedStream[i].iCliRTPPort != iIncomingMsg.transport[0].client_port1)
                            {
                                iSessionInfo.iDisableFireWallPackets = true;
                                LOGW("NAT has modified our client_port, we will not send firewall packets, port mapping is handled by NAT");
                            }
                        }
                        // --->
...
PVMFStatus PVRTSPEngineNode::sendSocketOutgoingMsg(SocketContainer &aSock, RTSPOutgoingMessage &aMsg)
{
    ...
    {
    LOGE ("[C->S]\n=============================================================================================================");
    LOGE("%s=============================================================================================================", tmpStrPtrLen->c_str());
// <--- Morris Yang 20100706 dump whole ops to file
			char path[PATH_MAX];
                     char* _op_buf = new char[(strlen(tmpStrPtrLen->c_str())+1)+1024];
			sprintf (path, "/data/rtsp_op.log");
			sprintf(_op_buf, "[C->S]\n=============================================================================================================\n%s\n"
				                 "=============================================================================================================\n", tmpStrPtrLen->c_str());
			FILE* _rtsp_log_fp = NULL;
...
void PVRTSPEngineNode::ResetSessionInfo()
{
    ...
    iSessionInfo.iSDPinfo.Unbind();
    // <--- Demon Deng ALPS00028091
    iSessionInfo.iDisableFireWallPackets = false;
    // --->
}
