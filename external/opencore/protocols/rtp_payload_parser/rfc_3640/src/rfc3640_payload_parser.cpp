...
#define LOG_TAG "RFC3640PayloadParser"
...
// <--- Morris Yang add AAC-lbr mode
#define AAC_LBR_SIZELENGTH_DEFAULT_VALUE        6
#define AAC_LBR_INDEXLENGTH_DEFAULT_VALUE       2
#define AAC_LBR_INDEXDELTALENGTH_DEFAULT_VALUE  2
#define AAC_LBR_CTSDELTALENGTH_DEFAULT_VALUE    0
#define AAC_LBR_DTSDELTALENGTH_DEFAULT_VALUE    0
#define AAC_LBR_HEADERSLENGTH_DEFAULT_VALUE     8
#define AAC_LBR_AUXDATASIZELENGTH_DEFAULT_VALUE 0
// --->
...
OSCL_EXPORT_REF bool RFC3640PayloadParser::Init(mediaInfo* config)
{
    ...
    //LOGE ("RFC3640PayloadParser::Init Mode=%s, sizeLength(%d), indexLength(%d), indexDeltaLength(%d), CTSDeltaLength(%d), DTSDeltaLength(%d)", r3640m->getMode(), sizeLength, indexLength, indexDeltaLength, CTSDeltaLength, DTSDeltaLength);
...
// <--- Morris Yang 20100628 add AAC-lbr mode
    else if (!oscl_strncmp(r3640m->getMode(), "AAC-lbr", oscl_strlen("AAC-lbr")))
    {
        headersPresent         = true;
        headersLength          = AAC_LBR_HEADERSLENGTH_DEFAULT_VALUE;
        auxDataSizeLength      = AAC_LBR_AUXDATASIZELENGTH_DEFAULT_VALUE;

        if ((sizeLength != AAC_LBR_SIZELENGTH_DEFAULT_VALUE) ||
                (indexLength != AAC_LBR_INDEXLENGTH_DEFAULT_VALUE) ||
                (indexDeltaLength != AAC_LBR_INDEXDELTALENGTH_DEFAULT_VALUE))
        {
            retVal = true;
        }
    }
