...
#define LOG_TAG "H263MediaInfoParser"
                ...
                case 'a':
                {
                    ...
                    if (!oscl_strncmp(line_start_ptr, fmsize.c_str(), fmsize.length()))
                    {
                        ...
                        else
                        {
                            ...
                            payloadPtr2->setHasFrameSizeField(true);
                        }
                    }
                // <--- Morris Yang ALPS00225262 In case there is no a=framesize: field, looking for a=Height: and a=width:
                    StrPtrLen _height("a=Height:");
                    if ((!oscl_strncmp(line_start_ptr, _height.c_str(), _height.length())) && (false == framesize_found) &&
                    	    (h263V->getMediaPayloadNumberCount() == 1))
                    {
                            Oscl_Vector<PayloadSpecificInfoTypeBase*, SDPParserAlloc> payloadSpecificInfoVector =  h263V->getPayloadSpecificInfoVector();
                            H263PayloadSpecificInfoType* payloadPtr = (H263PayloadSpecificInfoType*)payloadSpecificInfoVector[0];

                            const char *sptr = oscl_strchr(line_start_ptr, ';');
                            sptr++;
                            const char *eptr = skip_to_whitespace(sptr, line_end_ptr);

                            uint32 uHeight = 0;
                            if (PV_atoi(sptr, 'd', eptr -sptr, uHeight) == false)
                            {
                                LOGE ("SDPH263MediaInfoParser::parseMediaInfo - Bad a=Height line format");
                                return SDP_BAD_MEDIA_HEIGHT;
                            }
                            payloadPtr->setFrameHeight(uHeight);
  			                    LOGD ("[Warning] a=framesize: does not exists, use a=Height: instead [%d]", uHeight);
                    }
                    StrPtrLen _width("a=Width:");
                    if ((!oscl_strncmp(line_start_ptr, _width.c_str(), _width.length())) && (false == framesize_found) &&
                    	    (h263V->getMediaPayloadNumberCount() == 1))
                    {
                            Oscl_Vector<PayloadSpecificInfoTypeBase*, SDPParserAlloc> payloadSpecificInfoVector =  h263V->getPayloadSpecificInfoVector();
                            H263PayloadSpecificInfoType* payloadPtr = (H263PayloadSpecificInfoType*)payloadSpecificInfoVector[0];
                            ...
                            uint32 uWidth = 0;
                            if (PV_atoi(sptr, 'd', eptr -sptr, uWidth) == false)
                            {
                                LOGE ("SDPH263MediaInfoParser::parseMediaInfo - Bad a=Width line format");
                                return SDP_BAD_MEDIA_WIDTH;
                            }
                            payloadPtr->setFrameWidth(uWidth);
  			                    LOGD ("[Warning] a=framesize: does not exists, use a=Width: instead [%d]", uWidth);
  		        }
                // --->]
                // <--- Morris Yang ALPS00136859 In case there is no a=framesize: field, also looking for a=cliprect:
                    StrPtrLen _cliprect("a=cliprect:");
                    if ((!oscl_strncmp(line_start_ptr, _cliprect.c_str(), _cliprect.length())) && (false == framesize_found) &&
                    	    (h263V->getMediaPayloadNumberCount() == 1))
                    {
                            Oscl_Vector<PayloadSpecificInfoTypeBase*, SDPParserAlloc> payloadSpecificInfoVector =  h263V->getPayloadSpecificInfoVector();
                            H263PayloadSpecificInfoType* payloadPtr = (H263PayloadSpecificInfoType*)payloadSpecificInfoVector[0];

                            const char *sptr = line_start_ptr + _cliprect.length();
                            for (; *sptr != ',' ; ++sptr);
                            sptr++;
                            for (; *sptr != ',' ; ++sptr);
                            sptr++;
                            ...
                            if (PV_atoi(sptr, 'd', eptr -sptr, uHeight) == false)
                            {
                                LOGE ("SDPH263MediaInfoParser::parseMediaInfo - Bad a=cliprect (height) line format");
                                return SDP_BAD_MEDIA_HEIGHT;
                            }
                            ...
                            if (PV_atoi(sptr, 'd', eptr -sptr, uWidth) == false)
                            {
                                LOGE ("SDPH263MediaInfoParser::parseMediaInfo - Bad a=cliprect (width) line format");
                                return SDP_BAD_MEDIA_WIDTH;
                            }
                            payloadPtr->setFrameHeight(uHeight);
                            payloadPtr->setFrameWidth(uWidth);
  			       LOGD ("[Warning] a=framesize: does not exists, use a=cliprect [Width=%d, Height=%d]", uWidth, uHeight);
  		        }
                // --->
