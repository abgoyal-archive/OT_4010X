...
#define LOG_TAG "SDPMediaInfoParser"
            ...
            switch (*line_start_ptr)
            {
                ...
                case 'b':
                {
                    if (!oscl_strncmp(line_start_ptr, "b=AS:", oscl_strlen("b=AS:")))
                    {
                        ...
                        {
                            // <--- Morris Yang ALPS00225117
                            int32 _bitRate = 1000*bitRate;
                            if (_bitRate < 0) {
                                LOGE ("SDPBaseMediaInfoParser::parseMediaInfo - Bad b=AS: bitrate overflow");
                         	    return SDP_BAD_MEDIA_FORMAT;
                            }
                            // --->
