...
SDPRFC3640MediaInfoParser::parseMediaInfo(const char *buff, const int index, SDPInfo *sdp, payloadVector payload_vec, bool isSipSdp, int alt_id, bool alt_def_id)
{
    ...
    else
    {
        ...
        if (!(indexDeltaLength_found_in_fmtp && sizelength_found_in_fmtp && indexlength_found_in_fmtp))
        {
            ...
            LOGE ("SDP_MISSING_MEDIA_DESCRIPTION!!! [RFC3640] mode=%s , AU (%d, %d, %d)", m3640media->getMode(), indexDeltaLength_found_in_fmtp, sizelength_found_in_fmtp, indexlength_found_in_fmtp);

            if (!oscl_strncmp(m3640media->getMode(), "generic", oscl_strlen("generic")))   // Morris Yang 20100910 ALPS00125548 [
            {
                LOGE ("[RFC3640] Unsupported mode=%s !!!!", m3640media->getMode());
                return SDP_BAD_MEDIA_FORMAT;
            }

            //Morris Yang 20100702 don't treat this as a fatal error, use AAC-hbr AU sizeLen, indexLen, and indexDeltaLen as default value
