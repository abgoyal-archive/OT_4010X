...
#define ENABLE_MEDIA_CLOCK_DEBUG
#ifdef ENABLE_MEDIA_CLOCK_DEBUG
#include <cutils/properties.h>
#define DEBUG_MEDIA_CLOCK        (1 << 0)
#endif
...
// <--- Morris 20100623 for RTSP streaming 
    bIsRtspStreamingClock = false;
// --->

// <--- Donglei Ji ALPS00131944
    iLatestClock  = 0;
    iTimebaseTime = 0;
    iPreTimebaseTickcount = 0;
    bEnableTimeAdjustment  = false;
// --->

#ifdef ENABLE_MEDIA_CLOCK_DEBUG
    _uDbgFlags = 0;
     char value[PROPERTY_VALUE_MAX];
     property_get("debug.mediaclock", value, "0");
     bool _res = atoi(value);
     if (_res) _uDbgFlags |= DEBUG_MEDIA_CLOCK;
    _log_counter = 0;
#endif

    bDisableBackwardAdjustment = false;
}
...
// <--- Morris 20100623 for RTSP streaming.   <--Jun 122346 extend to local playback, so delte.
    /*if (RUNNING == iState && (false == bIsRtspStreamingClock))
    {
...
// <--- Morris Yang 20100624 ALPS00008828
            if (bDisableBackwardAdjustment) {
...
#ifdef ENABLE_MEDIA_CLOCK_DEBUG
    if (_uDbgFlags & DEBUG_MEDIA_CLOCK) {
    	LOGE ("PVMFMediaClock::SetStartTime32 - Clock Start time set to %d, 0x%x (aOverFlow=%d)", iStartClockTime, iStartClockTime, aOverFlow);
    }
#endif
    //start scheduling afresh
    AdjustScheduling();
...
OSCL_EXPORT_REF void PVMFMediaClock::GetCurrentTime32(uint32& aClockTime, bool& aOverflow, PVMFMediaClock_TimeUnits aUnits, uint32& aTimebaseTime)
{
    ...
    {
        LOGV("!!! lock !!!");
        FromClockUnit(iLockClockTime, aClockTime, aUnits, aOverflow);
        return;
    }
    ...
    // Get the current timebase time
    LOGV("GetCurrentTick32: %d, iStartTimebaseTickValue: %d, iLatestRunningClockTime: %d, iLatestRunningTimebaseTime: %d", 
        temp, iStartTimebaseTickValue, iLatestRunningClockTime, iLatestRunningTimebaseTime);

#ifdef ENABLE_MEDIA_CLOCK_DEBUG
        if ((_uDbgFlags & DEBUG_MEDIA_CLOCK) && (0 == (_log_counter % 20))) {
    		LOGE("GetCurrentTick32: %d, iStartTimebaseTickValue: %d, iLatestRunningClockTime: %d, iLatestRunningTimebaseTime: %d", 
   		temp, iStartTimebaseTickValue, iLatestRunningClockTime, iLatestRunningTimebaseTime);
        }
#endif
    ...
    if (iState == STOPPED)
    {
        LOGV("PVMFMediaClock::GetCurrentTime32 - iState == STOPPED");
        // Return the specified start time
        FromClockUnit(iStartClockTime, aClockTime, aUnits, overflowFlag3);

#ifdef ENABLE_MEDIA_CLOCK_DEBUG
        if (_uDbgFlags & DEBUG_MEDIA_CLOCK) {
        	LOGE("PVMFMediaClock::GetCurrentTime32 - iState == STOPPED, aClockTime(%u), overflowFlag3(%d)", aClockTime, overflowFlag3);
        }
#endif
    }
    else if (iState == PAUSED)
    {
        LOGV("PVMFMediaClock::GetCurrentTime32 - iState == PAUSED");
        // Returned the paused time
        FromClockUnit(iPauseClockTime, aClockTime, aUnits, overflowFlag3);

#ifdef ENABLE_MEDIA_CLOCK_DEBUG
        if (_uDbgFlags & DEBUG_MEDIA_CLOCK) {
        	LOGE("PVMFMediaClock::GetCurrentTime32 - iState == PAUSED, aClockTime(%u), overflowFlag3(%d)", aClockTime, overflowFlag3);
        }
#endif
    }
    else
    {
    	    // <--- Donglei Ji ALPS00131944
    	    if (bEnableTimeAdjustment)
    	    {
          	uint32 timebaseOffset  = aTimebaseTime - iTimebaseTime;
          	if (0!=iLatestClock && 0!=iTimebaseTime && aTimebaseTime>iTimebaseTime && 2000<timebaseOffset)
         	{
              		UpdateLatestTimes(iLatestClock+50, aTimebaseTime);
         	}
    
         	if (0!=iLatestClock && 0!=iTimebaseTime && 0==aTimebaseTime)
         	{
              		iLastAdjustObsTimebaseTime = aTimebaseTime;
              		UpdateLatestTimes(iLatestClock+50, aTimebaseTime);
         	}
         	iTimebaseTime = aTimebaseTime;
        }
        // --->
        
        LOGV("PVMFMediaClock::GetCurrentTime32 - iState == RUNNING");
        // Running state
        uint32 currenttime;
...
#ifdef ENABLE_MEDIA_CLOCK_DEBUG
	 if ((_uDbgFlags & DEBUG_MEDIA_CLOCK) && (0 == (_log_counter % 20))) {
      		LOGE("PVMFMediaClock::GetCurrentTime32 - iState == RUNNING, aClockTime(%u), overflowFlag3(%d), aTimebaseTime(%d)", aClockTime, overflowFlag3, aTimebaseTime);
    }
#endif
    }
...
void PVMFMediaClock::GetScaledTimebaseTickCount(uint32& aScaledTickCount, bool& aOverFlow)
{
   ...
   // <---Donglei Ji ALPS00131944
    if (bEnableTimeAdjustment && iPreTimebaseTickcount>temp)
    {
    	    iStartTimebaseTickValue = temp;
    }
    iPreTimebaseTickcount = temp;
    // --->
...
#ifdef ENABLE_MEDIA_CLOCK_DEBUG
    if (_uDbgFlags & DEBUG_MEDIA_CLOCK) {
    	LOGE ("iLatestRunningClockTime=%d, iLatestRunningTimebaseTime=%d", iLatestRunningClockTime, iLatestRunningTimebaseTime);
    }
#endif
}

OSCL_EXPORT_REF void PVMFMediaClock::ResetClock(uint32 aTimeStamp)
{
	// Reset the time values
	uint32 tmp = 0;
	iStartClockTime = tmp;
	...
	{
		iClockTimebase->GetCurrentTick32(iStartTimebaseTickValue, overflowFlag);
        iStartTimebaseTickValue -= aTimeStamp;
	}
...
OSCL_EXPORT_REF void PVMFMediaClock::adjustStartTimebaseTickValue(uint32 aTimeStampOffset)
{
    bool overflowFlag = false;
	...
	{
		iClockTimebase->GetCurrentTick32(iStartTimebaseTickValue, overflowFlag);
		if (iStartTimebaseTickValue > aTimeStampOffset)
		{
			iStartTimebaseTickValue -= aTimeStampOffset;
		}
		else
...
#undef LOG_TAG
#define LOG_TAG "PVMFAVSyncbase"

#define WAIT_TIMEOUT 1000
OSCL_EXPORT_REF PVMFAVSyncbase::PVMFAVSyncbase() :
    m_bInit(false),
    m_iSetPlaybackRange(AVSYNC_TAG_NONE),
    m_iAudioInTs(0),
    m_iAudioFirstTimeStamp(0),
    m_bSetAudioFirstTimeStamp(false),
    m_bMutexLock(false),
    bAccurateMediaSkipControlForAudio(false)
{   
    LOGV("constructor");
}
...
OSCL_EXPORT_REF void PVMFAVSyncbase::Init()
{
    LOGV("init");
    m_iSetPlaybackRange = PVMFAVSyncbase::AVSYNC_TAG_NONE;
    m_iAudioInTs = 0;
    m_bInit = true;
    m_bMutexLock = false;
...
OSCL_EXPORT_REF void PVMFAVSyncbase::DeInit()
{
    LOGV("deinit");
    m_iSetPlaybackRange = PVMFAVSyncbase::AVSYNC_TAG_NONE;
    m_bInit = false;
...
OSCL_EXPORT_REF void PVMFAVSyncbase::LockSyncMutex()
{
    if (m_bInit == false)
    {
        return;
    ...
    m_bMutexLock = true;
    LOGV("***** Display Sem (Wait) *****");
#if WAIT_TIMEOUT
    OsclProcStatus::eOsclProcError err = mSem.Wait(WAIT_TIMEOUT);
#else
    OsclProcStatus::eOsclProcError err = mSem.Wait();
#endif
    if (err != OsclProcStatus::SUCCESS_ERROR)
    {
        LOGV("!!!!! LockSyncMutex warning: %d !!!!!", err);
        mSem.Close();
        mSem.Create();
    }
    LOGV("***** Display Sem (Release) *****");    
}

OSCL_EXPORT_REF void PVMFAVSyncbase::UnlockSyncMutex()
{
    if (m_bInit == false)
    {
        return;
    ...
    {
        m_bMutexLock = false;
        LOGV("***** Display Sem (Signal) *****");
        mSem.Signal();            
    }    
}

OSCL_EXPORT_REF bool PVMFAVSyncbase::isMutexLock()
{
    LOGV("isClock: %s", m_bMutexLock?"true":"false");
    return m_bMutexLock;
}

OSCL_EXPORT_REF void PVMFAVSyncbase::StartSeekTo()
{
    m_iSetPlaybackRange = AVSYNC_TAG_AUDIO | AVSYNC_TAG_VIDEO;
}

OSCL_EXPORT_REF bool PVMFAVSyncbase::CheckBufIn(uint32 aBufType)
{
    bool bRet = false;
    switch (aBufType)
    {
    case AVSYNC_TAG_NONE:
        if (m_iSetPlaybackRange == aBufType)
        {
            bRet = false;
        }
        else
    ...
    case AVSYNC_TAG_AUDIO:
        if (m_iSetPlaybackRange & AVSYNC_TAG_AUDIO)
        {
            bRet = true;
            m_iSetPlaybackRange -= aBufType;
        }
        break;

    case AVSYNC_TAG_VIDEO:
        if ((m_iSetPlaybackRange & AVSYNC_TAG_VIDEO) && !(m_iSetPlaybackRange & AVSYNC_TAG_AUDIO))
        {
            bRet = true;
            m_iSetPlaybackRange -= aBufType;
        }
...
OSCL_EXPORT_REF void PVMFAVSyncbase::SetAudioInTs(uint32 a_iAudioInTs)
{
    m_iAudioInTs = a_iAudioInTs;
}

OSCL_EXPORT_REF uint32 PVMFAVSyncbase::GetAudioInTs()
{
    uint32 ret = m_iAudioInTs;
    m_iAudioInTs = 0;
    return ret;
}

OSCL_EXPORT_REF void PVMFAVSyncbase::SetAudioDelayTime(uint32 aTime)
{
    if (aTime > m_iAudioDelayTime)
    {
        m_iAudioDelayTime = aTime;
    }
}

OSCL_EXPORT_REF uint32 PVMFAVSyncbase::GetAudioDelayTime()
{
    uint32 ret = m_iAudioDelayTime;
    m_iAudioDelayTime = 0;
    return ret;
}

OSCL_EXPORT_REF void PVMFAVSyncbase::SetAudioFirstTs(uint32 a_iAudioInTs)
{
	if (false == m_bSetAudioFirstTimeStamp)
	{
    	m_iAudioFirstTimeStamp = a_iAudioInTs;
    	m_bSetAudioFirstTimeStamp = true;
	}
}

OSCL_EXPORT_REF uint32 PVMFAVSyncbase::GetAudioFirstTs()
{
    return m_iAudioFirstTimeStamp;
}
