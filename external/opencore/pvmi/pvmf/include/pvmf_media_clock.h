...
class PVMFAVSyncbase
{
    public:
        enum AVSYNC_TAG
        {
            AVSYNC_TAG_NONE = 0x00,
            AVSYNC_TAG_AUDIO = 0x01,
            AVSYNC_TAG_VIDEO = 0x02,

            AVSYNC_TAG_MAX
        };
        
        OSCL_IMPORT_REF PVMFAVSyncbase();
        OSCL_IMPORT_REF ~PVMFAVSyncbase();
        
        OSCL_IMPORT_REF void Init();
        OSCL_IMPORT_REF void DeInit();
        OSCL_IMPORT_REF void LockSyncMutex();
        OSCL_IMPORT_REF void UnlockSyncMutex();
        OSCL_IMPORT_REF bool isMutexLock();

        OSCL_IMPORT_REF void StartSeekTo();
        OSCL_IMPORT_REF bool CheckBufIn(uint32 aBufType = AVSYNC_TAG_NONE);

        OSCL_IMPORT_REF void SetAudioInTs(uint32 aiAudioInTs);
        OSCL_IMPORT_REF uint32 GetAudioInTs();

        OSCL_IMPORT_REF void SetAudioDelayTime(uint32 aTime);
        OSCL_IMPORT_REF uint32 GetAudioDelayTime();

        // Honda add
        OSCL_IMPORT_REF void SetAudioFirstTs(uint32 aiAudioInTs);
        OSCL_IMPORT_REF uint32 GetAudioFirstTs();

     // <--- Morris Yang ALPS00122159
        OSCL_IMPORT_REF void SetAccurateMediaSkipControlForAudio(bool bEnable) { bAccurateMediaSkipControlForAudio = bEnable; };
        OSCL_IMPORT_REF bool AccurateMediaSkipControlForAudio() { return bAccurateMediaSkipControlForAudio; };
     // --->
    ...
    protected:
        ...
        bool m_bMutexLock;
        OsclSemaphore mSem;
        uint32 m_iSetPlaybackRange;
        uint32 m_iAudioInTs;

        uint32 m_iAudioDelayTime;

        // Honda add
        uint32 m_iAudioFirstTimeStamp;
        bool m_bSetAudioFirstTimeStamp;

     // <---  Morris Yang ALPS00122159
        bool bAccurateMediaSkipControlForAudio;
     // --->
};
        ...
        public PVMFMediaClockNotificationsObs /*Media clock uses itself to set callback*/
{
    ...
    public:
        // Add by Ryan for video playback to reset the clock to do av sync
        void ResetClock(uint32 aTimeStamp);

        void setAVSyncCtrl(PVMFAVSyncbase* aAVSyncCtrl){mAVSyncCtrl = aAVSyncCtrl;}
        PVMFAVSyncbase* getAVSyncCtrl(){return mAVSyncCtrl;}

        // Add by Honda to adjust iStartTimebaseTickValue
        // for compensate video/audio first frame timeStamp difference
        void adjustStartTimebaseTickValue(uint32 aTimeStampOffset);

        // <--- Morris 20100623 for RTSP streaming 
        void SetRtspStreaming(bool flag) { bIsRtspStreamingClock = flag; }
        bool IsRtspStreamingClock() { return bIsRtspStreamingClock; }
        // --->
        // <--- Morris Yang 20100624 ALPS00008288
        void DisableBackwardAdjustment(bool value) { bDisableBackwardAdjustment = value; }
        // --->
        // <--- Donglei Ji ALPS00131944
        void EnableTimeAdjustment(bool value) { bEnableTimeAdjustment = value; }
        // --->
    protected:
        ...
        // add by Ryan for av sync
        bool m_bLockClock;
        uint32 iLockClockTime;

        // <--- Morris 20100623 for RTSP streaming 
        bool bIsRtspStreamingClock;
        // --->

        // <--- Morris Yang 20100624 ALPS00008288
        bool bDisableBackwardAdjustment;
        // --->
        
        // <--- Donglei Ji ALPS00131944
        uint32 iLatestClock;
        uint32 iTimebaseTime;
        uint32 iPreTimebaseTickcount;
        bool bEnableTimeAdjustment;
        // --->
