...
#endif
// ---------------------------------------------------------------------
//#include <utils/Log.h>		//by HP Cheng
//#define LOG_TAG "Pv_omxcore"
...
OMX_ERRORTYPE RvRegister();			// by HungChih Lin

OMX_ERRORTYPE AacRegister();
OMX_ERRORTYPE RaRegister();             //by Morris Yang
OMX_ERRORTYPE AmrRegister();
OMX_ERRORTYPE Mp3Register();
OMX_ERRORTYPE WmaRegister();
OMX_ERRORTYPE WavEncRegister();		//by HP Cheng
...
//unconditional registration
    ...
    Status = RvRegister();
	LOGE ("RvRegister A return %d", Status);
    if (Status != OMX_ErrorNone)
        return Status;
	// HungChih Lin (end)

	LOGE ("after RvRegister A return %d", Status);
...
#if REGISTER_OMX_WAVENC_COMPONENT
    //WAV ENCODER
//    LOGD("hphp : calling WavEncRegister()");
    Status = WavEncRegister();
    if (Status != OMX_ErrorNone)
        return Status;
#endif
        ...
        OMX_IN  OMX_CALLBACKTYPE* pCallBacks)
{
    ...
    *pHandle = NULL;
    ...
    else if (!data) // singleton object has been destroyed
    {
        ...
        OsclSingletonRegistry::registerInstanceAndUnlock(data, OSCL_SINGLETON_ID_OMX, error);
        _Cleanup_Component(pTempProxyTerm, *pHandle, cComponentName);
        LOGE("singleton object has been destroyed");
        return OMX_ErrorInvalidState;
    }
    LOGV("4. err = 0x%X", ErrorType);
    if ((NULL != *pHandle) && (OMX_ErrorNone == ErrorType))
    {
        OMX_U32* componentIndex = &(data->iComponentIndex);
        OMX_HANDLETYPE* componentHandle = data->iComponentHandle;
        LOGV("4.1 err = 0x%X", ErrorType);
