...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "OmxComponentBase"
    ...
    OMX_IN  size_t size)
{
    ...
    if (iState != OMX_StateLoaded &&
			...
			OMX_FALSE != ipPorts[nPortIndex]->PortParam.bEnabled)
    {
...
OMX_ERRORTYPE OmxComponentBase::FlushPort(OMX_S32 PortIndex)
{
    ...
    pOpenmaxAOType->FlushPortFinal(PortIndex); //handle unflushed buffer in component.
    ...
    if (OMX_PORT_OUTPUTPORT_INDEX == PortIndex || OMX_PORT_ALLPORT_INDEX == PortIndex)
    {
        ...
        while ((GetQueueNumElem(pOutputQueue) > 0))
        {
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_NOTICE, (0, "OmxComponentBase : FlushPort ERROR DeQueue() returned NULL"));
                LOGD("OmxComponentBase : FlushPort MAYBE ERROR DeQueue(), returned NULL");
                return OMX_ErrorUndefined;
            }
...
OMX_ERRORTYPE OmxComponentBase::DoStateSet(OMX_U32 aDestinationState)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_NOTICE,
                    (0, "OmxComponentBase : DoStateSet IN : iState (%i) aDestinationState (%i)", iState, (int32)aDestinationState));
        ...
        switch (iState)
        {
            ...
            case OMX_StatePause:
            {
                ...
                while (iOutBufferCount > 0)
                {
                    ...
                    {
                        LOGD("flush output, sleep a while..., tid:%d", gettid());
...
pOpenmaxAOType->getInputDeQueueTime(ipInputBuffer);
            ...
            else
            {
                ...
                ReturnInputBuffer(ipInputBuffer, pInPort);
pOpenmaxAOType->getReturnInputBufferTime(ipInputBuffer);
                ipInputBuffer = NULL;
            }
    ...
    OMX_INOUT OMX_PTR ComponentParameterStructure)
{
	...
	OMX_AUDIO_PARAM_ADPCMTYPE* pAudioAdpcmMode;			//by HP Cheng
    ...
    OMX_AUDIO_PARAM_RATYPE* pAudioRa;                                  //by Morris Yang
    ...
    switch (nParamIndex)
    {
		...
		case OMX_IndexParamAudioAdpcm:		//by HP Cheng
        {
            pAudioAdpcmMode = (OMX_AUDIO_PARAM_ADPCMTYPE*) ComponentParameterStructure;
            if (pAudioAdpcmMode->nPortIndex > 1)
            {
                return OMX_ErrorBadPortIndex;
            }
            PortIndex = pAudioAdpcmMode->nPortIndex;
            oscl_memcpy(pAudioAdpcmMode, &ipPorts[PortIndex]->AudioAdpcmParam, sizeof(OMX_AUDIO_PARAM_ADPCMTYPE));
            SetHeader(pAudioAdpcmMode, sizeof(OMX_AUDIO_PARAM_ADPCMTYPE));
        }
        break;
    ...
    OMX_IN  OMX_PTR ComponentParameterStructure)
{
    ...
    OMX_AUDIO_PARAM_PORTFORMATTYPE* pAudioPortFormat;
    OMX_AUDIO_PARAM_PCMMODETYPE* pAudioPcmMode;
	OMX_AUDIO_PARAM_ADPCMTYPE* pAudioAdpcmMode;      // HP Cheng
    OMX_PARAM_BUFFERSUPPLIERTYPE* pBufSupply;
    OMX_PARAM_PORTDEFINITIONTYPE* pPortDef ;
    OMX_PARAM_COMPONENTROLETYPE* pCompRole;
    ComponentPortType* pComponentPort;
    ...
    switch (nParamIndex)
    {
		...
		case OMX_IndexParamAudioAdpcm:			//by HP Cheng
        {
            ...
            PortIndex = pAudioAdpcmMode->nPortIndex;
            /*Check Structure Header and verify component State*/
            ErrorType = ParameterSanityCheck(hComponent, PortIndex, pAudioAdpcmMode, sizeof(OMX_AUDIO_PARAM_ADPCMTYPE));
            oscl_memcpy(&ipPorts[PortIndex]->AudioAdpcmParam, pAudioAdpcmMode, sizeof(OMX_AUDIO_PARAM_ADPCMTYPE));
        ...
        case OMX_IndexParamAudioRa:
        {
            ...
            if (ErrorType != OMX_ErrorNone)
            {
                LOGE ("ParameterSanityCheck error on OMX_IndexParamAudioRa");
                return ErrorType;
            }
    ...
    OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    OMX_ERRORTYPE ReturnValue;
pOpenmaxAOType->getInputQueueTime(pBuffer);
    ...
    OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    OMX_ERRORTYPE ReturnValue;
pOpenmaxAOType->getOutputQueueTime(pBuffer);
