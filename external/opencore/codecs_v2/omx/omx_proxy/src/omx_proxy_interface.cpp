...
// a) total number of omx output buffers (ex. NUMBER_OUTPUT_BUFFER_AVC , NUMBER_OUTPUT_BUFFER_MP4 ...)
// b) total number of omx input buffers (ex. NUMBER_INPUT_BUFFER_AVC , NUMBER_INPUT_BUFFER_MP4 ...)
...
#define MAX_NUMBER_OF_OMX_PROXY_MSGS 200
...
{
#if (VR_PROFILE_OPT)
    fp_getProcMsgTime = NULL;
    fp_getProcMsgTime=fopen("/getProcMsgTime.txt", "ab");
#endif
...
#if (VR_PROFILE_OPT)
    if(fp_getProcMsgTime)
        fclose(fp_getProcMsgTime);
    fp_getProcMsgTime = NULL;
#endif
...
void ProxyApplication_OMX::ProcessMessage(TPVCommandId cmdid, OsclAny* cmd)
{
...
#if (VR_PROFILE_OPT)
    {
        ...
        if(fp_getProcMsgTime)
        {
            fprintf(fp_getProcMsgTime, "proc msg %08X %08X %04d %04d %08X %08X\n", t1.tv_sec, t1.tv_usec, gettid(), iNumMessage, cmdid, cmd);    
        }
    }
#endif
    ...
    switch (cmdid)
    {
        ...
        case EMPTY_BUF:
        {
            ...
            OmxComponentBase* pOpenmaxAOType = (OmxComponentBase*)((OMX_COMPONENTTYPE *)Command->hComponent)->pComponentPrivate;
            pOpenmaxAOType->getInputQueueStartTime(Command->pBuffer);
            ...
            pOpenmaxAOType->getInputQueueEndTime(Command->pBuffer);
        ...
        case FILL_BUF:
        {
            ...
            OmxComponentBase* pOpenmaxAOType = (OmxComponentBase*)((OMX_COMPONENTTYPE *)Command->hComponent)->pComponentPrivate;
            pOpenmaxAOType->getOutputQueueStartTime(Command->pBuffer);

            // Do not use wait/signal semaphore for queuing input/output buffers
            // allow the client thread to keep going without having to check the status (assume its OK)
            ReturnValueOmxApi = ComponentFillThisBuffer(Command->hComponent, Command->pBuffer);

            pOpenmaxAOType->getOutputQueueEndTime(Command->pBuffer);
    ...
    OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
	...
	LOGD("ProxyFillThisBuffer:%08X, pBuffer:%08X, tid:%d", hComponent, pBuffer, gettid());
    iMemCmd++;
    iNumClientMsg++;
