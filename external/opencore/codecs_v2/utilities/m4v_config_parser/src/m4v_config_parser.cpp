...
void movePointerTo(mp4StreamType *psBits, int32 pos)
{
    ...
    // <--- Demon Deng, ALPS00029585, weird clip will cause infinite loop ...
    ...
    *profilelevel = 0x0000FFFF; // init to some invalid value. When this value is returned, then no profilelevel info is available
    ...
    if (codeword == VISUAL_OBJECT_SEQUENCE_START_CODE)
    {
        ...
        *profilelevel = (int) codeword;
        ...
        //add by Qian for alps136934
        //Find visual_object_start_code
        status = SearchNextM4VFrame(psBits); /* search 0x00 0x00 0x01 */
        if (status != 0)
        {
            LOGE("iDecodeVOLHeader error: SearchNextM4VFrame(1)");
            return MP4_INVALID_VOL_PARAM;
        }
        //add end by Qian for alps136934
        ShowBits(psBits, 32, &codeword);
        if (codeword == USER_DATA_START_CODE)
        {
            ...
            if (iErrorStat)
            {
                LOGD("iDecodeVOLHeader error: DecodeUserData(1)");
            	return MP4_INVALID_VOL_PARAM;
            }
        }
        ...
        {
            LOGD("iDecodeVOLHeader error: VISUAL_OBJECT_START_CODE");
        	return MP4_INVALID_VOL_PARAM;
        }
        ...
        else
        {
            ...
            do
            {
                ...
                if (status != 0)
                {
                    LOGD("iDecodeVOLHeader error: SearchNextM4VFrame(1)");
                    return MP4_INVALID_VOL_PARAM;
                }
        ...
        //add by Qian for alps136934
        //start to find video_object_start_code  00  00  01 00-1F
        //int16 status = 0;
        status = SearchNextM4VFrame(psBits); /* search 0x00 0x00 0x01 */
        if (status != 0)
        {
            LOGE("iDecodeVOLHeader error: SearchNextM4VFrame LINE(3)");
            return MP4_INVALID_VOL_PARAM;
        }
        //add end
        ShowBits(psBits, 32, &codeword);
        if (codeword == USER_DATA_START_CODE)
        {
            ...
            if (iErrorStat)
            {
                LOGD("iDecodeVOLHeader error: DecodeUserData(2)");
            	return MP4_INVALID_VOL_PARAM;
            }
        }
    ...
    if (codeword == VO_START_CODE)
    {
        ...
        //add by Qian for alps136934
        //next should be video_object_layer_start_code 0x00 0x00 0x01 20 through 2F,skip right 4 variant bit
        //int16 status = 0;
        status = SearchNextM4VFrame(psBits); /* search 0x00 0x00 0x01 */
        if (status != 0)
        {
            LOGE("iDecodeVOLHeader error: SearchNextM4VFrame(4)");
            return MP4_INVALID_VOL_PARAM;
        }
        //add end
        ...
        if (codeword != VOL_START_CODE)
        {
            ...
            {
                LOGD("iDecodeVOLHeader error: SHORT_HEADER_MODE");
            ...
            else
            {
              //  int16 status = 0;                                    
                ...
                do
                {
                    ...
                    if (status != 0)
                    {
                        LOGD("iDecodeVOLHeader error: SearchNextM4VFrame(2)");
                        return MP4_INVALID_VOL_PARAM;
                    }
                    status = ReadBits(psBits, VOL_START_CODE_LENGTH, &codeword);
                }
...
decode_vol:
        ...
        {
        	//<-- Honda
            LOGD("iDecodeVOLHeader error: Video Object Type Indication= %d", codeword);
            //return MP4_INVALID_VOL_PARAM;
            //-->
        }
        ...
        if (codeword)
        {
            ...
            {
				//<-- Honda
                LOGD("iDecodeVOLHeader error: 4, low_delay");
                //return MP4_INVALID_VOL_PARAM;
				//-->
            }
        ...
        if (codeword != 0)
        {
            LOGD("iDecodeVOLHeader error: 10");
            return MP4_INVALID_VOL_PARAM;
        }
        ...
        if (codeword != 1)
        {
            LOGD("iDecodeVOLHeader error: 11");
            return MP4_INVALID_VOL_PARAM;
        }
        ...
        if (codeword != 1)
        {
            LOGD("iDecodeVOLHeader error: 12");
            return MP4_INVALID_VOL_PARAM;
        }
        ...
        ReadBits(psBits, 1, &codeword);
		//<-- Honda
		// Remove time_increment_resolution check
        ...
        if (codeword != 1)
        {
            LOGD("iDecodeVOLHeader error: 13");
            return MP4_INVALID_VOL_PARAM;
        }
        ...
        *display_width = (int32)codeword;
        ...
        if (codeword != 1)
        {
            ...
            LOGD("iDecodeVOLHeader error: 14");
            //return MP4_INVALID_VOL_PARAM;
        }
        ...
        *height = (*display_height + 15) & -16;
        ...
        //--->
        //[Jackal][ALPS00006582] [MPEG4 playback]Can not shown the tumbnails of these files
        //Add header parsing code to parse header until quant_type
        
        ReadBits(psBits, 1, &codeword); //marker_bit
        ReadBits(psBits, 1, &codeword); //interlaced
        ReadBits(psBits, 1, &codeword); //obmc_disable

        if (video_object_layer_verid == 1)
        {
            ReadBits(psBits, 1, &codeword); //sprite_enable
        }
        else
        {
            ReadBits(psBits, 2, &codeword); //sprite_enable
        }

        if (codeword != 0)
        {
            LOGE("[ERROR] sprite_enable != 0 !!");
            return MP4_INVALID_VOL_PARAM;
        }

        ReadBits(psBits, 1, &codeword); //not_8_bit
        
        if (codeword != 0)
        {
            LOGE("[ERROR] not_8_bit != 0 !!");
            return MP4_INVALID_VOL_PARAM;
        }

        ReadBits(psBits, 1, &codeword); //quant_type
        
        if (codeword != 0)
        {
            LOGE("[ERROR] quant_type != 0 !!");
            return MP4_INVALID_VOL_PARAM;
        }
    ...
    else
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (status != 0)
                {
                    LOGD("iDecodeVOLHeader error: 15");
                    return MP4_INVALID_VOL_PARAM;
                }
                status = ReadBits(psBits, VOL_START_CODE_LENGTH, &codeword);
            }
...
OSCL_EXPORT_REF int16 iGetAVCConfigInfo(uint8 *buffer, int32 length, int32 *width, int32 *height, int32 *display_width, int32 *display_height, int32 *profile_idc, int32 *level_idc)
{
    int16 status;
    mp4StreamType psBits;
    uint16 sps_length = 0, pps_length = 0;
    int32 size;
    int32 i = 0;
    uint8* sps = NULL;
    ...
    *width = *height = *display_height = *display_width = 0;
    ...
    else
    {
        ...
        pps += 2;
#else
        // [2010/05/03][Jackal] Some bitstream have multiple SPS and PPS
        // so we need to find first SPS and PPS
        ...
        while(1)
        {
            ...
            {
                pps_length = (uint16)(pps[1] << 8) | pps[0];
                pps = pps + pps_length + 2;
            }
            else if((pps[2] & 0x08) == 0x08)
            {
                pps_length = (uint16)(pps[1] << 8) | pps[0];
                pps += 2;
                break;
            ...
            {
                LOGE("[ERROR] Can't Find SPS and PPS");
                return MP4_INVALID_VOL_PARAM;
            }
        }  
#endif
