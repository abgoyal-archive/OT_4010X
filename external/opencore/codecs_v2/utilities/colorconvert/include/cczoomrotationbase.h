...
class ColorConvertBase
{
    ...
    public:
        enum PixelFormat
        {
            PIXEL_FORMAT_YUV_420_PLANER = 0,
            PIXEL_FORMAT_YUV_420_PLANER_MTK = 1,
        };
        OSCL_IMPORT_REF void SetInputBufFormat(PixelFormat aPixelFormat);

        void SetCropSize(uint32 aCropWidth, uint32 aCropHeight);

        // [20100423] Jackal
        void SetPmemSrcBufVA(uint32 aPmemSrcBufVA);
        void SetPmemDstBufPA(uint32 aPmemDstBufPA);

    protected:
        uint32 _mCrop_width, _mCrop_height;
        // [20100423] Jackal
        uint32 _mPmemSrcBufVA;
        uint32 _mPmemDstBufPA;
